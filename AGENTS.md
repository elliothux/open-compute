...
- Keep only active implementation designs directly under `docs/` (plus its index); the user-designated `docs/workerd/` directory owns active native workerd P1/P2 designs. Put qualification-only plans for completed implementations under `docs/acceptance/`; put continuously maintained references, API/deviation matrices, testing policy, deployment guides, and runbooks under `docs/references/`. Put completed stage designs, stage-specific matrices, and completed investigation/verification records under `docs/implemented/`, and maintain every directory index.
...
- Separate outstanding release qualification or long-running verification from completed core implementation into an explicit plan under `docs/acceptance/`, then archive the completed implementation with its actual evidence and limitations. If qualification reveals unfinished implementation work, move that work into an active design under `docs/` instead of hiding it in the acceptance plan. A concluded investigation may be archived with a No-Go or Conditional Go verdict; that does not mean its unsupported capability has been implemented or its unrun checks passed.
