...
- Final Gate: three fresh-process rounds only as the last acceptance step after implementation, review, and fixes are complete. The retired G0 capability investigation is not a required recurring Gate; the default-one/explicit-three contract and audited process concurrency are documented in `docs/references/testing.md` and `docs/implemented/runtime-and-test-layout.md`.
