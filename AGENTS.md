...
- Keep only active implementation designs directly under `docs/` (plus its index). Put qualification-only plans for completed implementations under `docs/acceptance/`; put continuously maintained references, API/deviation matrices, testing policy, deployment guides, and runbooks under `docs/references/`. Put completed stage designs, stage-specific matrices, and completed investigation/verification records under `docs/implemented/`, and maintain every directory index.
