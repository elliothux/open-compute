...
#[test]
fn r2_injection_sources_are_descriptor_bound_and_reserved_modules_fail_closed() {
    let account = AccountId::generate();
    let worker = WorkerId::generate();
    let deployment = DeploymentId::generate();
    let bundle = CanonicalBundle::build(
    ...
    let binding = BindingDescriptorV1::new(
        ...
        "BUCKET".to_owned(),
        open_compute_core::BindingKind::R2Bucket,
    ...
    let collision = CanonicalBundle::build(
        ...
        vec![
            module("index.js", b"export default {}"),
            module(R2_FACADE_MODULE_NAME, b"export class R2Bucket {}"),
        ],
        BundleLimits::default(),
    )
    .unwrap();
