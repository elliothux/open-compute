...
fn p1_schema_inspection_checks_current_kv_and_d1_files_without_mutation() {
    ...
    // Current Vectorize and AI Search files exercise both inspection arms.
    ...
    .unwrap();
    let mut vectorize_connection = Connection::open(vectorize_root.join("data.sqlite")).unwrap();
    crate::schema_migrations::migrate_to_for_test(
        &mut vectorize_connection,
    ...
    vectorize_connection
        .execute(
            "INSERT INTO index_meta(singleton, resource_id, dimensions, metric, quota_vectors,
               quota_bytes) VALUES(1, ?1, 32, 'cosine', 100, 16777216)",
            rusqlite::params![vectorize.id.to_string()],
        )
        .unwrap();
    ...
    .unwrap();
    let mut ai_search_connection = Connection::open(ai_search_root.join("data.sqlite")).unwrap();
    crate::schema_migrations::migrate_to_for_test(
        &mut ai_search_connection,
    ...
    ai_search_connection
        .execute(
            "INSERT INTO instance_meta(singleton, resource_id, model_contract_sha256,
             ...
             VALUES(1, ?1, ?2, X'7B7D', X'7B7D', 0, 0, 1, 1, 1, 1, 0, 0)",
            rusqlite::params![ai_search.id.to_string(), [7u8; 32].as_slice()],
        )
        .unwrap();
    ...
    let kv_file = kv_paths.database_path(account, kv.id);
    let foreign = InstanceId::generate().to_string();
    let connection = Connection::open(&kv_file).unwrap();
    connection
        .execute(
            "UPDATE kv_meta SET value=?1 WHERE key='instance_id'",
            [foreign.as_bytes()],
        )
        .unwrap();
    drop(connection);
    assert_eq!(
        crate::inspect_current_schema(storage.data_dir(), storage.db(), 5_000)
            .unwrap_err()
            .code(),
    ...
    let unchanged: Vec<u8> = connection
        ...
        .unwrap();
    assert_eq!(unchanged, foreign.as_bytes());
    connection
        .execute(
            "UPDATE kv_meta SET value=?1 WHERE key='instance_id'",
