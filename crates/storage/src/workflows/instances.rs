...
impl WorkflowRepository<'_> {
    ...
    pub fn reserve_instance(
        ...
        self.db.with_immediate(|tx| {
            ...
            let (total,active,definition_total): (u64,u64,u64) = tx.query_row(
                "SELECT COUNT(*),coalesce(SUM(r.state!='released'),0),coalesce(SUM(r.definition_id=?2),0)
    ...
    pub fn instance_referrers_intact(
        ...
        self.db.with_read(|conn|conn.query_row("SELECT EXISTS(SELECT 1 FROM deployment_referrers
            WHERE deployment_id=?1 AND kind='workflow_instance' AND ref_id=?2)
            AND EXISTS(SELECT 1 FROM workflow_referrers WHERE definition_id=?3 AND referrer_kind='instance' AND referrer_id=?2)",
            params![reservation.target.deployment_id.to_string(),reservation.instance_id.to_string(),reservation.target.definition_id.to_string()],
...
const RESERVATION_SELECT: &str = "SELECT f.account_id,r.definition_id,r.definition_name,r.version_id,v.worker_id,r.deployment_id,
...
fn reservation_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<WorkflowReservation> {
