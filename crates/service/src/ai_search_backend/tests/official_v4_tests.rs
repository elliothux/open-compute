...
async fn official_ai_search_routes_cover_the_frozen_30_operation_surface() {
    ...
    exercise_jobs_and_items(&state, &instances, &docs).await;
}

async fn exercise_tokens_and_namespaces(state: &HttpState, account: &str) -> String {
    ...
    let list = official_ai_send(state, "GET", &namespaces, "read-token", None, Body::empty()).await;
...
async fn exercise_instances_and_search(state: &HttpState, main: &str) -> (String, String) {
...
async fn exercise_jobs_and_items(state: &HttpState, instances: &str, docs: &str) {
