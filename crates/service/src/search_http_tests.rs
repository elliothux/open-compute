...
async fn operator_can_create_vector_metadata_and_ai_namespace_resources() {
    ...
    let indexes = format!("/operator/api/v1/accounts/{account}/vectorize/indexes");
    ...
    let namespaces = format!("/operator/api/v1/accounts/{account}/ai-search/namespaces");
    let (status, created) = body(
        router
            .clone()
            .oneshot(request(
...
async fn operator_routes_fail_closed_without_authority_or_valid_ids() {
    ...
    assert_eq!(
        router
            ...
            .oneshot(request(
                "GET",
                "/operator/api/v1/accounts/not-an-account/vectorize/indexes",
                &Value::Null,
                None,
            ))
            .await
    ...
    let missing = format!(
        "/operator/api/v1/accounts/{account}/vectorize/indexes/{}",
        ResourceId::generate()
    );
    assert_eq!(
        router
