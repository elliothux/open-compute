...
async fn wait_signals_and_servers(
    ...
    gateway_task: Option<tokio::task::JoinHandle<Result<(), PlatformError>>>,
    challenge_task: Option<tokio::task::JoinHandle<Result<(), PlatformError>>>,
    provider_task: Option<tokio::task::JoinHandle<Result<(), PlatformError>>>,
    gateway_process_task: Option<tokio::task::JoinHandle<Result<(), PlatformError>>>,
    ...
    let mut challenge_task = challenge_task;
    let mut provider_task = provider_task;
    let mut gateway_process_task = gateway_process_task;
    ...
    'wait: loop {
        tokio::select! {
            ...
            res = async {
                match gateway_process_task.as_mut() {
                    Some(task) => task.await,
                    None => std::future::pending().await,
                }
            } => {
    ...
    if let Some(task) = gateway_process_task.as_mut()
        && !task.is_finished()
    {
        let _ = task.await;
    }
