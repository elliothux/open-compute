...
class Target extends RpcTarget {
  ...
  nested() {
    return new Target(`${this.value}:nested`);
  }
  [Symbol.dispose]() {
    targetDisposals += 1;
  }
...
class BackendLogic {
  ...
  response() {
    return new Response("native-response", { status: 201 });
  }
  readable() {
  ...
  writable() {
    return new WritableStream({
      write(chunk) {
        writableText += new TextDecoder().decode(chunk);
      },
    });
  }
  written() {
  ...
  targetCallback(callback, value) {
    return callback.ping(value);
  }
  invalid() {
    return new WeakMap();
  }
  disposalCount() {
    return targetDisposals;
  }
  fetch(request) {
...
export class Backend extends WorkerEntrypoint {
  async invoke(method, rawArgs, controller) {
    ...
    return encode(
      await Reflect.apply(backend[method], backend, args),
      controller,
    );
  }
...
class SourceCapability extends RpcTarget {
  constructor(target, controller) {
    ...
    this.target = target;
    this.controller =
      typeof controller.dup === "function" ? controller.dup() : controller;
  ...
  [Symbol.dispose]() {
    ...
    this.retention = undefined;
    if (retention)
      Promise.resolve(retention.release()).finally(() =>
        retention[Symbol.dispose](),
      );
    if (typeof this.controller[Symbol.dispose] === "function")
      this.controller[Symbol.dispose]();
    if (typeof this.target[Symbol.dispose] === "function")
      this.target[Symbol.dispose]();
  }
}

async function activateNested(
  value,
  controller,
  operationHandle,
  seen = new WeakSet(),
  ...
  if (value.capability === 1) {
    await value.handle.activate(
      await controller.retainCapability(operationHandle),
    );
    return;
  }
...
function encode(value, controller, seen = new WeakMap()) {
  ...
  for (const [key, item] of Object.entries(value))
    output[key] = encode(item, controller, seen);
  return output;
}
    ...
    get(target, key, receiver) {
      if (key === "then") {
        return (resolve, reject) =>
          Promise.resolve(handle.call("get", property, [])).then(
            (value) => resolve(decode(value)),
            reject,
          );
      }
  ...
  if (envelope.kind === "function") {
    ...
    return new Proxy(fn, {
      get(target, property, receiver) {
        ...
        if (property === "dup")
          return () => capability({ ...envelope, handle: handle.dup() });
...
export class Transport extends WorkerEntrypoint {
  async call(method, rawArgs) {
    ...
    const result = await this.env.BACKEND.invoke(
      method,
      rawArgs,
      new Controller(),
    );
  ...
  async get(property) {
    const result = await this.env.BACKEND.getProperty(
      property,
      new Controller(),
    );
    await activate(result);
  ...
  counts() {
    return { retentionBegins, retentionCompletes, retentionReleases };
  }
}
...
function result(raw) {
  return new Proxy(raw, {
    get(target, property) {
      if (property === "then") {
        return (resolve, reject) =>
          Reflect.apply(Reflect.get(target, "then"), target, [
            (value) => resolve(decode(value)),
            reject,
          ]);
      }
...
function service(transport) {
  return new Proxy(Object.create(null), {
    get(_target, property) {
      ...
      const item = (...args) =>
        result(transport.call(property, encode(args, transport)));
      return new Proxy(item, {
        get(target, key, receiver) {
          if (key === "then")
            return (resolve, reject) =>
              Promise.resolve(transport.get(property)).then(
                (value) => resolve(decode(value)),
                reject,
              );
          return Reflect.get(target, key, receiver);
...
export default {
  async test(_controller, env) {
    ...
    assert(
      structured.bytes instanceof Uint8Array && structured.bytes[2] === 3,
      "binary changed",
    );
    assert(
      structured.when instanceof Date &&
        structured.when.getTime() === 1_700_000_000_000,
      "Date changed",
    );
    const request = await binding.request(
      new Request("https://original.example/path", { method: "POST" }),
    );
    assert(
      request.url === "https://original.example/path" &&
        request.method === "POST",
      "Request changed",
    );
    const response = await binding.response();
    assert(
      response.status === 201 && (await response.text()) === "native-response",
      "Response changed",
    );
    assert(
      (await new Response(await binding.readable()).text()) === "stream",
      "ReadableStream failed",
    );
    const writer = (await binding.writable()).getWriter();
    await writer.write(new TextEncoder().encode("native-write"));
    await writer.close();
    assert(
      (await binding.written()) === "native-write",
      "WritableStream failed",
    );
    assert(
      (await binding.callback((value) => `function:${value}`, "ok")) ===
        "function:ok",
      "function callback failed",
    );
    assert(
      (await binding.targetCallback(new CallbackTarget(), "ok")) ===
        "target-callback:ok",
      "RpcTarget callback failed",
    );
    const upgrade = await binding.fetch(
      new Request("https://application.example/websocket", {
        headers: { Upgrade: "websocket" },
      }),
    );
    assert(
      upgrade.status === 101 && upgrade.webSocket,
    ...
    assert(
      (await echoed) === "echo:native-websocket",
      "WebSocket transport failed",
    );
    socket.close(1000, "done");

    assert(
      (await binding.capability("pipe").ping("ok")) === "pipe:ok",
      "promise pipeline failed",
    );
    assert(
      (await binding.capability("pipe-get").label) === "label:pipe-get",
      "pipelined getter failed",
    );
    const target = await binding.capability("target");
    const duplicate = target.dup();
    assert(
      (await target.ping("one")) === "target:one",
      "returned RpcTarget failed",
    );
    target[Symbol.dispose]();
    assert(
      (await duplicate.ping("two")) === "target:two",
      "dup released the shared target",
    );
    const nested = await duplicate.nested();
    const nestedLabel = await nested.label;
    assert(
      nestedLabel === "label:target:nested",
      `nested capability/getter failed: ${nestedLabel}`,
    );
    nested[Symbol.dispose]();
    duplicate[Symbol.dispose]();
    for (
      let attempt = 0;
      attempt < 20 && (await env.BACKEND.targetDisposals()) < 2;
      attempt += 1
    ) {
    ...
    assert(
      disposalCount >= 1,
      `target disposer did not run: ${disposalCount}; ${JSON.stringify(counts)}`,
    );
    assert(
      counts.retentionBegins >= 4,
      "capability operations bypassed admission",
    );
    assert(
      counts.retentionCompletes >= counts.retentionBegins,
      "capability completion was lost",
    );
    assert(
      counts.retentionReleases >= 3,
      "capability retention did not release",
    );

    let rejected = false;
    try {
      await binding.invalid();
    } catch {
      rejected = true;
    }
