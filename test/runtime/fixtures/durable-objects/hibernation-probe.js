...
export class Room extends DurableObject {
  ...
  async inspect() {
    ...
    return {
      ...
      attachment: first
        ? first.deserializeAttachment()
        : this.ctx.storage.kv.get("attachment"),
...
export default {
  async test(_controller, env) {
    ...
    assert(
      before.constructs === 1,
      `expected 1 construct before eviction, got ${before.constructs}`,
    );
    assert(before.sockets === 1, "socket missing before eviction");
    assert(
      Array.isArray(before.tags) && before.tags.includes("echo"),
      "tags missing before eviction",
    );
    assert(
      before.attachment?.n === 1 && before.attachment?.tag === "echo",
      "attachment missing before eviction",
    );
    ...
    assert(
      afterMessage.constructs === 2,
      `expected exactly one reconstruction, got ${afterMessage.constructs}`,
    );
    assert(
      afterMessage.handled === 1,
      `auto-response must not run the handler, handled=${afterMessage.handled}`,
    );
    assert(afterMessage.sockets === 1, "hibernated socket was not restored");
    assert(
      Array.isArray(afterMessage.tags) && afterMessage.tags.includes("echo"),
      "tags were not restored",
    );
    assert(
      afterMessage.attachment?.n === 1 &&
        afterMessage.attachment?.tag === "echo",
      "attachment was not restored",
    );
    assert(
      afterMessage.autoRequest === "ping" &&
        afterMessage.autoResponse === "pong",
      "auto-response pair missing",
    );
    ...
    assert(
      afterClose.close?.code === 1000,
      `close handler missing: ${JSON.stringify(afterClose.close)}`,
    );
