#!/bin/sh
# Validate durable Workflow behavior with a verified offline runtime and real authorities.
set -eu
...
if [ ! -f "$workerd" ]; then
  echo "OPEN_COMPUTE_TEST_WORKERD is missing; the durable Workflow Gate refuses to skip" >&2
  exit 1
fi
...
while [ "$round" -le "$rounds" ]; do
  printf 'P2.5 stock-workerd fresh-process round %s/%s\n' "$round" "$rounds"
  cargo test -p open-compute-service --all-features \
    --test p2_5_workflow_hard_gate --test p2_5_workflow_product_gate \
    -- --test-threads=1
  cargo test -p open-compute-service --all-features \
    --test p2_4_workflow_product_gate snapshot_restore -- --test-threads=1
  round=$((round + 1))
done
