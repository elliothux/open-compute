...
function compileFixture(root: string, id: string): void {
  if (!id.startsWith("ts::"))
    throw new Error(`invalid compile case identity: ${id}`);
  const source = id.slice(4);
  if (
    !source.startsWith("test/conformance/fixtures/") ||
    !source.endsWith(".ts")
  ) {
  ...
  if (!contained(fixtures, path))
    throw new Error(`compile case escapes fixture root: ${id}`);
  let regular = false;
  try {
...
export function validateCaseEvidence(
  ...
  if (registry.schemaVersion !== 1)
    throw new Error("unsupported Gate registry schema");
  ...
  if (!Array.isArray(catalog.contracts))
    throw new Error("catalog.contracts must be an array");
  for (const [index, raw] of catalog.contracts.entries()) {
    ...
    for (const field of ["positiveCases", "negativeCases"] as const) {
      for (const id of strings(contract[field], `contract[${index}].${field}`))
        runtime.add(id);
    }
  }

  if (!Array.isArray(catalog.memberEvidence))
  ...
  for (const [index, raw] of catalog.memberEvidence.entries()) {
    ...
    for (const id of strings(
      item.compileCases ?? [],
      `memberEvidence[${index}].compileCases`,
    )) {
    ...
    for (const id of strings(
      item.runtimeCases ?? [],
      `memberEvidence[${index}].runtimeCases`,
    ))
      runtime.add(id);
  }
  ...
  if (missing.length)
    throw new Error(
      `catalog references unregistered Gate cases: ${missing.join(", ")}`,
    );
  for (const id of runtime) {
    if (!/^[a-z0-9][a-z0-9-]*::\S+$/.test(id))
      throw new Error(`invalid Gate case identity: ${id}`);
  }
