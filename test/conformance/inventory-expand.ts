...
const OBJECT_UTILITY_TYPES = new Set([
  "Pick",
  "Omit",
  "Partial",
  "Required",
  ...
  const visit = (statements: readonly Statement[], prefix: string): void => {
    for (const statement of statements) {
      ...
      if (
        ...
        visit(
          (statement.body as { statements: readonly Statement[] }).statements,
          `${qualify(prefix, name)}.`,
        );
      }
    }
    ...
    if (isVariableStatement(statement)) {
      for (const declaration of statement.declarationList.declarations) {
        if (isIdentifier(declaration.name))
          names.push(qualify(prefix, declaration.name.text));
      }
      continue;
    }
    const name = declarationName(statement);
...
function isOptionalMember(node: Node): boolean {
  if (
    "postfixToken" in node &&
    (node as { postfixToken?: Node }).postfixToken?.kind ===
      SyntaxKind.QuestionToken
  ) {
    return true;
  }
  if (
    "questionToken" in node &&
    (node as { questionToken?: Node }).questionToken !== undefined
  ) {
    return true;
  }
  return false;
...
function isNonPublic(node: Node): boolean {
  return (
    hasModifier(node, ModifierFlags.Private) ||
    hasModifier(node, ModifierFlags.Protected)
  );
}
...
function literalKeys(node: TypeNode): Set<string> | undefined {
  ...
  if (isLiteralTypeNode(node)) {
    if (isStringLiteral(node.literal) || isNumericLiteral(node.literal))
...
function pushMember(
  ...
  if (
    isNonPublic(node) ||
    state.seen.has(node) ||
    (state.allowedMembers !== undefined && !state.allowedMembers.has(member))
  )
    return;
  state.seen.add(node);
  state.added += 1;
  ...
  state.members.push({
    ...
    readonly:
      flags?.readonly ??
      state.readonlyOverride ??
      hasModifier(node, ModifierFlags.Readonly),
    optional:
      flags?.optional ?? state.optionalOverride ?? isOptionalMember(node),
    static: isStatic,
    node,
  });
}
...
function expandTypeElements(state: ExpandState, nodes: readonly Node[]): void {
  for (const node of nodes) {
    if (!state.includeStatic && hasModifier(node, ModifierFlags.Static))
      continue;
    if (isMethodSignatureDeclaration(node) || isMethodDeclaration(node)) {
      ...
      if (state.includeStatic)
        pushMember(state, "constructor", "constructor", node);
...
function runWith<T extends keyof ExpandState>(
  state: ExpandState,
  patch: Pick<ExpandState, T>,
  run: () => void,
): void {
...
function expandHeritage(
  state: ExpandState,
  statement: InterfaceDeclaration | ClassDeclaration,
): void {
...
function expandObjectUtility(
  ...
  for (const item of collected) {
    const keep =
      name === "Pick" ? keys.has(item.member) : !keys.has(item.member);
...
function expandResolved(
  ...
  for (const row of rows) {
    runWith(state, { prefix: row.prefix, includeStatic: false }, () =>
      expandDeclarationShape(state, row.statement),
    );
  }
}
...
function expandType(
  ...
  if (
    ...
    runWith(state, { readonlyOverride: true }, () =>
      expandType(state, node.type),
    );
    return;
  }
  const name =
    referenceName ??
    (isTypeReferenceNode(node) ? entityNameText(node.typeName) : undefined);
  ...
  if (OBJECT_UTILITY_TYPES.has(name) && isTypeReferenceNode(node)) {
    expandObjectUtility(
      state,
      name,
      node.typeArguments === undefined ? undefined : [...node.typeArguments],
      node,
    );
  }
}

function expandDeclarationShape(
  state: ExpandState,
  statement: Statement,
): void {
  if (isInterfaceDeclaration(statement)) {
    if (statement.members.length || statement.heritageClauses !== undefined)
      state.surface = true;
...
export function expandTargetDeclaration(
  ...
  if (isFunctionDeclaration(statement)) {
    ...
    const state = createState(
      members,
      memberProduct(symbol, exported, product),
      symbol,
      prefix,
      index,
      allowedMembers,
  ...
  if (isVariableStatement(statement)) {
    ...
    for (const declaration of statement.declarationList.declarations) {
      ...
      const exported =
        aliases.get(declaration.name.text) ?? declaration.name.text;
      const state = createState(
        members,
        memberProduct(symbol, exported, product),
        symbol,
