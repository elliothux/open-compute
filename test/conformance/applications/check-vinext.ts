...
function fixtureTreeDigest(): string {
  ...
  for (const name of names) {
    ...
    if (!lstatSync(path).isFile())
      throw new Error(`fixture entry is not a regular file: ${name}`);
...
function validatePackages(manifest: JsonRecord): void {
  ...
  for (const [name, version] of Object.entries(declared)) {
    ...
    if (installed.version !== version)
      throw new Error(`installed fixture package identity drift: ${name}`);
  }
}

function validateCases(manifest: JsonRecord, matrix: JsonRecord): JsonRecord[] {
  if (matrix.schemaVersion !== 1 || matrix.application !== "vinext")
    throw new Error("unsupported vinext case matrix");
  const catalogIds = new Set(
    array(
      json("test/conformance/catalog.json").contracts,
      "catalog.contracts",
    ).map((item, index) =>
      string(record(item, `catalog.contracts[${index}]`).id, "contract.id"),
    ),
  );
  const cases = array(matrix.cases, "matrix.cases").map((item, index) =>
    record(item, `matrix.cases[${index}]`),
  );
  ...
  for (const entry of cases) {
    ...
    if (!/^vinext\/[a-z0-9-]+\/[a-z0-9-]+$/.test(id) || ids.has(id))
      throw new Error(`duplicate or invalid case id: ${id}`);
    ids.add(id);
    if (
      !["build", "production-http", "browser"].includes(
        string(entry.mode, `${id}.mode`),
      )
    )
      throw new Error(`${id}: invalid mode`);
    if (
      !["ONCE", "TIMING"].includes(string(entry.repetition, `${id}.repetition`))
    )
      throw new Error(`${id}: invalid repetition`);
    if (
      !["platform-contract", "application-behavior", "toolchain-only"].includes(
        string(entry.classification, `${id}.classification`),
      )
    ) {
      throw new Error(`${id}: invalid classification`);
    }
    const fixture = string(entry.fixture, `${id}.fixture`);
    if (!fixture.startsWith(`${APPLICATION}/`) && fixture !== APPLICATION)
      throw new Error(`${id}: fixture escapes the application`);
    if (!existsSync(join(ROOT, fixture)))
      throw new Error(`${id}: fixture path is missing`);
    if (!strings(entry.requestSequence, `${id}.requestSequence`).length)
      throw new Error(`${id}: request sequence is empty`);
    ...
    for (const contract of strings(entry.contracts, `${id}.contracts`)) {
      if (!catalogIds.has(contract))
        throw new Error(`${id}: unknown contract ${contract}`);
    }
  ...
  if (
    !sameStrings(
      strings(manifest.selectedCases, "manifest.selectedCases"),
      selected,
    ) ||
    !sameStrings(
      strings(manifest.optionalCases, "manifest.optionalCases"),
      optional,
    ) ||
    !sameStrings(
      strings(manifest.excludedCases, "manifest.excludedCases"),
      excluded,
    )
  ) {
...
function validateBrowser(manifest: JsonRecord): void {
  ...
  const playwrightPackage = record(
    fixtureRequire("@playwright/test/package.json"),
    "@playwright/test/package.json",
  );
  if (
    string(playwrightPackage.version, "@playwright/test.version") !==
    string(browser.playwrightVersion, "browser.playwrightVersion")
  ) {
    throw new Error("Playwright package version is missing or changed");
  }
  const playwrightRequire = createRequire(
    fixtureRequire.resolve("@playwright/test/package.json"),
  );
  const browsersManifest = record(
    createRequire(playwrightRequire.resolve("playwright-core/package.json"))(
      "./browsers.json",
    ),
  ...
  const revision =
    typeof chromium.revision === "number"
      ? String(chromium.revision)
      : string(chromium.revision, "chromium.revision");
  if (
    revision !==
    string(browser.playwrightRevision, "browser.playwrightRevision")
  ) {
    throw new Error("Playwright Chromium revision is missing or changed");
  }
  if (
    string(chromium.browserVersion, "chromium.browserVersion") !==
    string(browser.browserVersion, "browser.browserVersion")
  ) {
...
function validateRunner(cases: readonly JsonRecord[]): void {
  const raw = execFileSync(
    "bun",
    [`${APPLICATION}/tests/qualification.ts`, "--list"],
    {
      cwd: ROOT,
  ...
  if (report.schemaVersion !== 1)
    throw new Error("unsupported qualification runner inventory");
  const expected = cases
    .filter(
      (entry) =>
        entry.selection === "mandatory" &&
        !ORCHESTRATION_CASES.has(string(entry.id, "case.id")),
    )
...
function validateHistoricalGo(manifest: JsonRecord): void {
  ...
  if (
    status.verdict !== "go" ||
    status.cloudflareSemantics !== "worker-version-and-deployment"
  ) {
  ...
  if (
    evidence.sourceBuildReproducibility !== "non-blocking-upstream-deviation" ||
  ...
  const first = string(
    evidence.firstSourceInventorySha256,
    "first source inventory digest",
  );
  const second = string(
    evidence.secondSourceInventorySha256,
    "second source inventory digest",
  );
  if (first === second)
    throw new Error("source-build deviation evidence was lost");
  if (
    evidence.importedModuleCount !== 79 ||
    evidence.wranglerModuleCount !== 79 ||
    evidence.importedModuleNamesSha256 !== evidence.wranglerModuleNamesSha256
  ) {
    throw new Error(
      "frozen artifact importer does not match Wrangler module discovery",
    );
  }
  for (const key of [
    "localBundleSha256",
...
function main(): void {
  if (process.argv.slice(2).join(" ") !== "--list")
    throw new Error("use --list");
  const manifest = json(MANIFEST);
  const matrix = json(CASES);
  if (manifest.schemaVersion !== 2 || manifest.application !== "vinext")
    throw new Error("unsupported vinext manifest");
  if (
    digest("bun.lock") !==
    string(manifest.rootLockSha256, "manifest.rootLockSha256")
  )
    throw new Error("root lock digest drift");
  if (
    fixtureTreeDigest() !==
    string(manifest.fixtureTreeSha256, "manifest.fixtureTreeSha256")
  )
    throw new Error("fixture tree digest drift");
  if (digest(CASES) !== string(manifest.casesSha256, "manifest.casesSha256"))
    throw new Error("case matrix digest drift");
  ...
  console.log(
    JSON.stringify({
      ...
      historicalRootLockSha256: record(
        record(manifest.p4Status, "manifest.p4Status").evidence,
        "historical evidence",
      ).rootLockSha256,
      selected: selected.length,
      mandatory: selected.filter((entry) => entry.selection === "mandatory")
        .length,
      optional: selected.filter(
        (entry) => entry.selection === "optional-partial",
      ).length,
