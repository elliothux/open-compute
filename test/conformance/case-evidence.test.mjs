...
test("case evidence accepts exact registered runtime and contained compile fixtures", () => {
  const value = fixture();
  assert.doesNotThrow(() =>
    validateCaseEvidence(value.root, value.catalog, value.registry),
  );
});
...
test("case evidence rejects stale, duplicate, missing, and traversing identities", () => {
  ...
  assert.throws(
    () =>
      validateCaseEvidence(value.root, value.catalog, {
        schemaVersion: 1,
        cases: [],
      }),
    /unregistered/,
  ...
  assert.throws(
    () =>
      validateCaseEvidence(value.root, value.catalog, {
        schemaVersion: 1,
        cases: [value.runtime, value.runtime],
      }),
    /duplicate evidence/,
  );
  value.catalog.memberEvidence[0].compileCases = [
    "ts::test/conformance/fixtures/kv/missing.ts",
  ];
  assert.throws(
    () => validateCaseEvidence(value.root, value.catalog, value.registry),
    /missing/,
  ...
  value.catalog.memberEvidence[0].compileCases = [
    "ts::test/conformance/fixtures/../outside.ts",
  ];
  assert.throws(
    () => validateCaseEvidence(value.root, value.catalog, value.registry),
    /outside|escapes/,
