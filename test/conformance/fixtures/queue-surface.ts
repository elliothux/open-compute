...
export default {
  async fetch(_request: Request, env: Env): Promise<Response> {
    ...
    const text: QueueSendResponse = await env.EVENTS.send("plain", {
      contentType: "text",
      delaySeconds: 0,
    });
    const bytes: QueueSendResponse = await env.EVENTS.send(new Uint8Array(1), {
      contentType: "bytes",
    });
    ...
    const sendOldest: Date | undefined =
      send.metadata.metrics.oldestMessageTimestamp;
    const batchOldest: Date | undefined =
      batch.metadata.metrics.oldestMessageTimestamp;
    const backlog: number =
      metrics.backlogCount +
      metrics.backlogBytes +
      send.metadata.metrics.backlogCount +
      text.metadata.metrics.backlogBytes +
      bytes.metadata.metrics.backlogCount +
      v8.metadata.metrics.backlogBytes +
      batch.metadata.metrics.backlogCount;
    return new Response(
      JSON.stringify({ backlog, oldest, sendOldest, batchOldest }),
    );
  },
  async queue(
