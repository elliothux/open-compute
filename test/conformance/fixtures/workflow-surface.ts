...
export class PortableWorkflow extends WorkflowEntrypoint<Env, Params> {
  async run(
      ...
      "dynamic",
      {
        retries: {
          ...
          delay: async ({ ctx, error }) =>
            error.name && ctx.attempt > 1 ? "2 seconds" : 1000,
        },
        timeout: "1 minute",
        sensitive: "output",
      },
    ...
    const waiting = step.waitForEvent<{ value: number }>("event", {
      type: "ready",
      timeout: "1 hour",
    });
...
export default {
  async fetch(_request: Request, env: Env): Promise<Response> {
    ...
    const deleted: WorkflowBatchDeleteResult = await env.FLOW.deleteBatch(
