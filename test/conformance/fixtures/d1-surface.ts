...
export default {
  async fetch(_request: Request, env: Env): Promise<Response> {
    const prepared: D1PreparedStatement = env.DB.prepare(
      "SELECT 1 AS value, 2 AS value",
    );
    const bound: D1PreparedStatement = prepared.bind(
      null,
      true,
      1,
      1.5,
    ...
    const all: D1Result<{ value: number }> = await bound.all<{
      value: number;
    }>();
    ...
    const rawNamed: [string[], ...unknown[][]] = await bound.raw({
      columnNames: true,
    });
    ...
    const unconstrained: D1DatabaseSession = env.DB.withSession(
      "first-unconstrained",
    );
    ...
    const batched: D1Result<unknown>[] = await env.DB.batch([
      bound,
      sessionPrepared,
    ]);
    const sessionBatch: D1Result<unknown>[] = await session.batch([
    ...
    return new Response(
      JSON.stringify({
        ...
        sessionBatch: sessionBatch.length,
        dump: typeof dump.then,
        bookmark,
        primary: typeof primary.prepare,
