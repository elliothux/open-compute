...
async function reset(bucket: R2Bucket): Promise<Response> {
  ...
  return Response.json({
    reset: (await bucket.list({ prefix: "portable:" })).objects.length === 0,
  });
}
...
async function surface(bucket: R2Bucket): Promise<Response> {
  ...
  for (const key of ["", ".", "..", "a/../b", exactKey])
    await bucket.put(key, "key");
  const keyShapes = (
    await Promise.all(
      ["", ".", "..", "a/../b", exactKey].map(
        async (key) => (await bucket.head(key))?.key === key,
      ),
    )
  ).every(Boolean);

  const head = await bucket.head("portable:a");
  const conditional = await bucket.get("portable:a", {
    onlyIf: { etagMatches: put.etag },
  });
  const notModified = await bucket.get("portable:a", {
    onlyIf: { etagDoesNotMatch: put.etag },
  });
  const failedPut = await bucket.put("portable:a", "changed", {
    onlyIf: { etagMatches: "missing" },
  });
  const ranged = await bucket.get("portable:a", {
  ...
  const json = await (await bucket.get("portable:json"))!.json<{
    ok: boolean;
  }>();
  ...
  return Response.json({
    object: {
      ...
      uploaded:
        put.uploaded instanceof Date && Number.isFinite(put.uploaded.getTime()),
    ...
    deleted:
      (await bucket.head("portable:delete-a")) === null &&
...
async function multipart(bucket: R2Bucket): Promise<Response> {
  ...
  return Response.json({
    identity:
      upload.key === resumed.key && upload.uploadId === resumed.uploadId,
    part: {
      number: part.partNumber,
      etag: part.etag === "5d41402abc4b2a76b9719d911017c592",
    },
    object: {
      key: completed.key,
      size: completed.size,
...
async function errors(bucket: R2Bucket): Promise<Response> {
  ...
  await body!.text();
  const listedWithoutInclude = await bucket.list({
    prefix: "portable:error-seed",
  });
  const listedHead = listedWithoutInclude.objects[0]!;
  const observed = {
    getUnknownOption: await capture(() =>
      invoke(bucket.get, bucket, ["portable:error-seed", { unknown: true }]),
    ),
    putUnknownOption: await capture(() =>
      invoke(bucket.put, bucket, [
        "portable:unknown",
        "value",
        { unknown: true },
      ]),
    ),
    listUnknownOption: await capture(() =>
      invoke(bucket.list, bucket, [{ unknown: true }]),
    ),
    getNumberKey: await capture(() => invoke(bucket.get, bucket, [1])),
    getSymbolKey: await capture(() =>
      invoke(bucket.get, bucket, [Symbol("key")]),
    ),
    negativeOffset: await capture(() =>
      bucket.get("portable:error-seed", { range: { offset: -1 } }),
    ),
    fractionalOffset: await capture(() =>
      bucket.get("portable:error-seed", { range: { offset: 0.5 } }),
    ),
    negativeLength: await capture(() =>
      bucket.get("portable:error-seed", { range: { length: -1 } }),
    ),
    fractionalLength: await capture(() =>
      bucket.get("portable:error-seed", { range: { length: 0.5 } }),
    ),
    negativeSuffix: await capture(() =>
      bucket.get("portable:error-seed", { range: { suffix: -1 } }),
    ),
    fractionalSuffix: await capture(() =>
      bucket.get("portable:error-seed", { range: { suffix: 0.5 } }),
    ),
    suffixOffset: await capture(() =>
      invoke(bucket.get, bucket, [
        "portable:error-seed",
        { range: { suffix: 1, offset: 0 } },
      ]),
    ),
    suffixLength: await capture(() =>
      invoke(bucket.get, bucket, [
        "portable:error-seed",
        { range: { suffix: 1, length: 1 } },
      ]),
    ),
    emptyRange: await capture(() =>
      bucket.get("portable:error-seed", { range: {} as R2Range }),
    ),
    malformedRangeHeader: await capture(() =>
      bucket.get("portable:error-seed", {
        range: new Headers({ range: "bananas" }),
      }),
    ),
    multiRangeHeader: await capture(() =>
      bucket.get("portable:error-seed", {
        range: new Headers({ range: "bytes=0-1,3-4" }),
      }),
    ),
    quotedMatch: await capture(() =>
      bucket.get("portable:error-seed", {
        onlyIf: { etagMatches: '"quoted"' },
      }),
    ),
    malformedMatchHeader: await capture(() =>
      bucket.get("portable:error-seed", {
        onlyIf: new Headers({ "if-match": "bad" }),
      }),
    ),
    md5Bytes: await capture(() =>
      bucket.put("portable:error-seed", "x", { md5: new Uint8Array(15) }),
    ),
    md5Length: await capture(() =>
      bucket.put("portable:error-seed", "x", { md5: "00" }),
    ),
    md5Hex: await capture(() =>
      bucket.put("portable:error-seed", "x", { md5: "z".repeat(32) }),
    ),
    multipleHashes: await capture(() =>
      bucket.put("portable:error-seed", "x", {
        md5: "00".repeat(16),
        sha1: "00".repeat(20),
      }),
    ),
    ssecFormat: await capture(() =>
      bucket.get("portable:error-seed", { ssecKey: "Z".repeat(64) }),
    ),
    ssecLength: await capture(() =>
      bucket.get("portable:error-seed", { ssecKey: "00" }),
    ),
    invalidStorageClass: await capture(() =>
      bucket.put("portable:storage", "value", { storageClass: "Banana" }),
    ),
    invalidInclude: await capture(() =>
      invoke(bucket.list, bucket, [{ include: ["etag"] }]),
    ),
    writeUnknownMetadata: await capture(() =>
      listedHead.writeHttpMetadata(new Headers()),
    ),
    bodySecondUse: await capture(() => body!.bytes()),
    unpairedKey: await capture(() =>
      invoke(bucket.put, bucket, ["portable:\ud800", "value"]),
    ),
  };
  const zero = await bucket.list({ prefix: "portable:limit-", limit: 0 });
  const negative = await bucket.list({ prefix: "portable:limit-", limit: -1 });
  const fractional = await bucket.list({
    prefix: "portable:limit-",
    limit: 1.5,
  });
  ...
  const multipartErrors = {
    ...
    uploadPartFractional: await capture(() =>
      invoke(upload.uploadPart, upload, [1.5, "x"]),
    ),
    completePartZero: await capture(() =>
      invoke(upload.complete, upload, [[{ partNumber: 0, etag: "x" }]]),
    ),
    completeNonArray: await capture(() =>
      invoke(upload.complete, upload, [{}]),
    ),
    resumeEmpty: await capture(() =>
      invoke(bucket.resumeMultipartUpload, bucket, [
        "portable:error-multipart",
        "",
      ]),
    ),
  };
  ...
  return Response.json({
    ...
    limits: {
      zero: {
        count: zero.objects.length,
        truncated: zero.truncated,
        hasCursor: "cursor" in zero,
      },
      negative: {
        keys: negative.objects.map((object) => object.key),
        truncated: negative.truncated,
        hasCursor: "cursor" in negative,
      },
      fractional: {
        keys: fractional.objects.map((object) => object.key),
        truncated: fractional.truncated,
        hasCursor: "cursor" in fractional,
      },
      high: {
        keys: high.objects.map((object) => object.key),
        truncated: high.truncated,
        hasCursor: "cursor" in high,
      },
    },
