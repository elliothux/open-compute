...
export default {
  async fetch(_request: Request, env: Env): Promise<Response> {
    ...
    const putSha256: R2Object = await env.BUCKET.put(key, new ArrayBuffer(0), {
      sha256: "00".repeat(32),
    });
    const putSha384: R2Object = await env.BUCKET.put(key, null, {
      sha384: new Uint8Array(48),
    });
    const putSha512: R2Object = await env.BUCKET.put(key, new Blob(), {
      sha512: "00".repeat(64),
    ...
    const conditional: R2ObjectBody | R2Object | null = await env.BUCKET.get(
      key,
      {
        onlyIf: headers,
    ...
    const ranged: R2ObjectBody | null = await env.BUCKET.get(key, {
      range: { suffix: 1 },
    });
    ...
    const created: R2MultipartUpload = await env.BUCKET.createMultipartUpload(
      key,
      {
    ...
    const resumed: R2MultipartUpload = env.BUCKET.resumeMultipartUpload(
      created.key,
      created.uploadId,
    );
    const uploaded: R2UploadedPart = await resumed.uploadPart(1, body, {
      ssecKey: "11".repeat(32),
    });
    ...
    const bytes: Uint8Array = got
      ? await (await env.BUCKET.get(key))!.bytes()
      : new Uint8Array();
    const buf: ArrayBuffer = got
      ? await (await env.BUCKET.get(key))!.arrayBuffer()
      : new ArrayBuffer(0);
    const blob: Blob = got
      ? await (await env.BUCKET.get(key))!.blob()
      : new Blob();
    const parsed: unknown = got
      ? await (await env.BUCKET.get(key))!.json()
      : null;
