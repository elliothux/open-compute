...
function metadata(
  response: QueueSendResponse | QueueSendBatchResponse,
): boolean {
...
async function surface(queue: Queue<unknown>): Promise<Response> {
  ...
  responses.push(
    await queue.send("text", { contentType: "text", delaySeconds: 0 }),
  );
  responses.push(
  ...
  responses.push(
    await queue.send(
      { when: new Date(0), values: new Map([["a", new Set([1, 2])]]) },
      {
        contentType: "v8",
      },
    ),
  ...
  const counts = responses.map(
    (response) => response.metadata.metrics.backlogCount,
  );
...
async function errors(queue: Queue<unknown>): Promise<Response> {
  const invalidContentType = await capture(() =>
    queue.send("x", { contentType: "invalid" as QueueContentType }),
  );
  const negativeDelay = await capture(() =>
    queue.send("x", { delaySeconds: -1 }),
  );
  const highDelay = await capture(() =>
    queue.send("x", { delaySeconds: 86_401 }),
  );
  const emptyBatch = await capture(() => queue.sendBatch([]));
  const highBatch = await capture(() =>
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === "/surface" && request.method === "POST")
      return surface(env.EVENTS);
    if (path === "/errors" && request.method === "GET")
      return errors(env.EVENTS);
