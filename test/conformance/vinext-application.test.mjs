...
test("vinext current inputs are verified offline without renewing the historical P4 verdict", () => {
  const result = spawnSync(
    "bun",
    ["test/conformance/applications/check-vinext.ts", "--list"],
    {
      cwd: root,
      encoding: "utf8",
  ...
  assert.equal(
    report.rootLockSha256,
    createHash("sha256")
      .update(readFileSync(resolve(root, "bun.lock")))
      .digest("hex"),
  );
  assert.equal(
    report.historicalRootLockSha256,
    "c99e951a642e668e1826f56289b02c03abcb3d084675d3cd117205e25b49d3d0",
  );
...
test("vinext offline checks reject changed frozen inputs and installed packages", async (t) => {
  ...
  files.push(
    ...
    "test/conformance/applications/check-vinext.ts",
    "test/conformance/applications/vinext.json",
  ...
  const dependencies = resolve(
    sandbox,
    "test/applications/vinext/node_modules",
  );
  symlinkSync(
    resolve(root, "test/applications/vinext/node_modules"),
    dependencies,
    "dir",
  );
  const run = () =>
  ...
  await t.test("installed package drift", () => {
    ...
    assert.ok(
      result.stderr.includes(
        "installed fixture package identity drift: @cloudflare/vite-plugin",
      ),
      result.stderr,
    );
  });
