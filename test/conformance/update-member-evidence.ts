...
interface Product {
  readonly status:
    "supported" | "supported_with_deviation" | "unsupported" | "blocked";
...
function contractStatus(
  ...
  if (statuses.includes("supported_with_deviation"))
    return "supported_with_deviation";
  return "supported";
}

function main(args: string[]): void {
  if (args.length !== 1 || args[0] !== "--apply")
    throw new Error("use --apply");
  const inventory = record(
    JSON.parse(readFileSync(INVENTORY, "utf8")),
    "inventory",
  );
  const products = record(inventory.products, "inventory.products") as Record<
    string,
    Product
  >;
  const catalog = record(JSON.parse(readFileSync(CATALOG, "utf8")), "catalog");
  const members = Object.values(products).flatMap(
  ...
  const evidence = catalog.memberEvidence
    .map((raw, index) => {
  ...
  for (const member of members) {
    ...
    else if (DO_RPC_BLOCKED_IDS.has(member.id))
      add("durable-objects.rpc-observable-boundary", member.id);
