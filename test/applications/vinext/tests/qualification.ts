...
function url(base: URL, path: string): URL {
  if (!path.startsWith("/"))
    throw new Error("qualification path must be absolute");
  const result = new URL(path.slice(1), base);
  if (result.origin !== base.origin)
    throw new Error("qualification path escaped the target");
  return result;
}
...
async function request(targetValue: Target, path: string): Promise<Response> {
  return fetch(url(targetValue.base, path), {
    headers:
      targetValue.requestHost === undefined
        ? {}
        : { host: targetValue.requestHost },
    redirect: "manual",
...
const implementations: Record<CaseId, (targetValue: Target) => Promise<void>> =
  {
    ...
    "vinext/app/hydration-navigation": async (targetValue) => {
      ...
      try {
        ...
        const response = await page.goto(targetValue.browserBase.href, {
          waitUntil: "networkidle",
        });
        assert.equal(response?.status(), 200);
        assert.equal(
          await page.getByTestId("counter").textContent(),
          "count:0",
        );
    ...
    "vinext/app/rsc-flight": async (targetValue) => {
      ...
      try {
        const page = await browser.newPage();
        await page.goto(targetValue.browserBase.href, {
          waitUntil: "networkidle",
        });
        const flight = page.waitForResponse((response) => {
          ...
          return (
            headers.rsc === "1" &&
            new URL(response.url()).pathname === "/navigation"
          );
        });
      ...
      try {
        ...
        const action = page.waitForRequest(
          (requestValue) =>
            ...
            requestValue.headers()["next-action"] !== undefined,
        );
        await page.getByTestId("server-action").click();
        await action;
        await page.getByTestId("action-result").waitFor();
        assert.equal(
          await page.getByTestId("action-result").textContent(),
          "action:qualified",
        );
        assert.equal(
          await page.getByTestId("action-cookie").textContent(),
          "cookie:qualified",
        );
      } finally {
        await browser.close();
    ...
    "vinext/app/metadata-assets": async (targetValue) => {
      ...
      assert.match(
        root.body,
        /<title>open-compute P4 vinext qualification<\/title>/,
      );
      assert.match(root.body, /Fixed Next\.js 16 production workload/);
      const source = root.body.match(
    ...
    "vinext/pages/gsp-paths": async (targetValue) => {
      assert.match(
        (await text(targetValue, "/static-qualification/alpha", 200)).body,
        /gsp:(?:<!-- -->)?alpha/,
      );
    ...
    "vinext/bindings/env-context": async (targetValue) => {
      ...
      assert.deepEqual(JSON.parse(env.body), {
        marker: "p4-public-marker",
        serverOnlyCanaryExposed: false,
        serverOnlyCanaryLength: 23,
      });
    },
      ...
      while (directories.length > 0) {
        ...
        for await (const entry of listing) {
          ...
          if (entry.isDirectory()) directories.push(filename);
            ...
            assert(
              !Buffer.from(await readFile(filename)).includes(
                Buffer.from(canary),
              ),
            );
        }
    ...
    "vinext/security/browser-context-isolation": async (targetValue) => {
      ...
      try {
        ...
        await page.getByTestId("action-result").waitFor();
        assert.equal(
          (await first.cookies()).find((cookie) => cookie.name === "p4-action")
            ?.value,
          "qualified",
        );
        assert.equal(
          (await second.cookies()).find(
            (cookie) => cookie.name === "p4-action",
          ),
          undefined,
        );
        await first.close();
...
async function main(): Promise<void> {
  ...
  for (let index = 0; index < args.length; index += 2) {
    if (
      ...
      !CASES.includes(args[index + 1] as CaseId)
    )
      throw new Error("use --case <known-id>");
    requested.push(args[index + 1] as CaseId);
  }
  if (new Set(requested).size !== requested.length)
    throw new Error("duplicate qualification case");
  ...
  process.stdout.write(
    `${JSON.stringify({ schemaVersion: 1, status: passed ? "passed" : "failed", results })}\n`,
  );
