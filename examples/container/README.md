...
`ocd` supervises one verified stock workerd child, keeps structured authority in the mounted
SQLite data directory, and uses the configured external S3-compatible provider for artifacts and
objects. The build context contains one native Linux release file named `ocd`; use the matching
...
- Mount the operator config read-only at `/etc/open-compute/config.toml`. Generate its initial
  content using `ocd config init --data-dir /var/lib/open-compute`, then set S3 and listeners.
