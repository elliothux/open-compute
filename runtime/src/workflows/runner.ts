...
import type { WorkflowCallback, WorkflowClass, WorkflowControllerV1, WorkflowEventWire, WorkflowProtocolError, WorkflowRunResult } from "./execution-protocol.js";
...
export async function runWorkflow(target: unknown, ctx: ExecutionContext, env: Record<string, unknown>, event: WorkflowEventWire, backend: WorkflowControllerV1): Promise<WorkflowRunResult> {
  ...
  const step = Object.freeze({
    async do(name: string, callback: WorkflowCallback<null>, ...extra: unknown[]): Promise<unknown> {
      ...
      } catch (error) { failure ||= "WORKFLOW_SERIALIZATION_UNSUPPORTED"; throw error; }
      active = true;
      const index = ordinal++;
      const count = (counts.get(name) || 0) + 1;
      counts.set(name, count);
      try {
        const identity = { ordinal: index, name, nameCount: count, configJson: "null" as const };
        ...
        try {
          value = await callback(Object.freeze({
            step: Object.freeze({ name, count }), attempt: 1, config: null,
          }));
        } catch {
          failure ||= "WORKFLOW_EXECUTION_FAILED";
          await rpc(() => backend.failure({ ordinal: index, error: workflowFailure() }));
          throw workflowError("Workflow execution failed");
        }
        if (closed) throw workflowError("WORKFLOW_RUN_STALE");
        try { outputJson = workflowJson(value); }
        catch (error) {
          const code = error instanceof Error && error.message === "WORKFLOW_RESULT_TOO_LARGE"
            ? "WORKFLOW_RESULT_TOO_LARGE" : "WORKFLOW_SERIALIZATION_UNSUPPORTED";
          failure ||= code;
          await rpc(() => backend.failure({ ordinal: index, error: workflowFailure(), errorCode: code }));
          throw workflowError(code);
        }
        await rpc(() => backend.success({ ordinal: index, outputJson }));
        return JSON.parse(outputJson);
      } finally { active = false; }
    },
    sleep: unsupported, sleepUntil: unsupported, waitForEvent: unsupported,
  });
  ...
  try {
    ...
    try { outputJson = workflowJson(value); }
      ...
      reject(error instanceof Error && error.message === "WORKFLOW_RESULT_TOO_LARGE"
        ? "WORKFLOW_RESULT_TOO_LARGE" : "WORKFLOW_SERIALIZATION_UNSUPPORTED");
    }
  } catch { failure ||= "WORKFLOW_EXECUTION_FAILED"; }
  finally { closed = true; }
  if (unknown) throw workflowError("WORKFLOW_RUNTIME_UNAVAILABLE");
  return failure
    ? { outcome: "errored", errorCode: failure, error: workflowFailure(), finalOrdinal: ordinal }
