...
export class DoHost extends DurableObject {
  ...
  async #tenant(authority) {
    ...
    const code = {
      ...
      env: tenantEnv(snapshot, this.ctx, authority.deploymentId, doPolicy(this.env)),
  ...
  async fetch(request) {
    ...
    const init = { method: tenantMethod, headers, body: request.body, redirect: "manual" };
