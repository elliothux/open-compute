...
function readRow(storage) {
  ...
  return {
    id: 1,
    scheduledTimeMs: Number(rows[0].scheduled_time_ms),
    retryCount: Number(rows[0].retry_count),
    inFlight: Number(rows[0].in_flight) === 1,
    rowToken: rows[0].row_token,
...
function wrapStorage(storage, index, transaction = null, rootStorage = storage) {
  return new Proxy(storage, {
    get(target, property) {
      const alarmStorage = transaction ? rootStorage : target;
      ...
      if (property === "transactionSync") {
        return callback => target.transactionSync(() => callback(
      ...
      if (property === "transaction") {
        return async callback => {
          ...
          const result = await target.transaction(async nativeTransaction => {
            ...
            committed = { mutated: attempt.mutated, final: readRow(rootStorage) };
            return value;
          });
          if (committed?.mutated && !sameProjection(initial, committed.final)) {
            await flushTransaction(rootStorage, index, initial, committed.final);
          }
          return result;
        };
      }
...
export function prepareDurableObjectContext(ctx, index) {
  ...
  const storage = wrapStorage(ctx.storage, index);
  let context = ctx;
  try {
...
export async function dispatchDurableObjectAlarm(instance, handler, prepared, payload) {
  if (!payload || typeof payload.rowToken !== "string" || !ROW_TOKEN.test(payload.rowToken)
      || !Number.isSafeInteger(payload.retryCount) || payload.retryCount < 0
