...
import { workflowError, workflowJson, workflowString, workflowSerializationCode } from "./__open_compute_workflow_json__.js";
...
export async function runWorkflow(target, ctx, env, event, backend) {
  ...
  const reject = code => { failure ||= code; throw workflowError(code); };
  const check = () => {
  ...
  const execute = async batch => {
    ...
    active = true;
    try {
      const reply = await rpc("claimBatch", { steps: batch.map(item => ({
        ...
        if (grant.state === "run") {
          ...
          const callback = (async () => {
            ...
            try {
              ...
              await report("failure", { ordinal: indexOrdinal, code });
              return;
            }
            let outputJson;
            try { outputJson = workflowJson(result); }
              ...
              await report("failure", { ordinal: indexOrdinal, code: failure });
              return;
            }
            await report("success", { ordinal: indexOrdinal, outputJson });
          })();
          callback.catch(() => {});
        }
        try { return { ok: true, value: value(await rpc("result", indexOrdinal)) }; }
        catch (error) { return { ok: false, error }; }
      }));
      const drain = await rpc("drain");
  ...
  const wait = async (kind, name, config) => {
    ...
    active = true;
    try {
      const reply = await rpc("registerWait", { ...item, batchFirstOrdinal: item.ordinal, batchSize: 1 });
      frontier = [item.ordinal];
      const result = value(reply);
      return kind === "wait_event" ? { type: result.type, payload: result.payload,
        timestamp: new Date(result.timestampMs) } : undefined;
    } finally { active = false; }
  };
  const step = Object.freeze({
    do(name, config, callback, ...extra) {
      if (typeof config === "function" && callback === undefined) { callback = config; config = {}; }
      if (typeof callback !== "function" || extra.length) reject("WORKFLOW_STEP_CONFIG_UNSUPPORTED");
      const item = descriptor("do", name, config);
      let resolve, rejectResult;
      const result = new Promise((yes, no) => { resolve = yes; rejectResult = no; });
  ...
  try {
    ...
    if (error !== suspension) failure ||= recalledFailure(error) || "WORKFLOW_EXECUTION_FAILED";
  }
  ...
  return suspended
    ...
    : failure ? { outcome: "errored", errorCode: failure, finalOrdinal: ordinal }
      : { outcome: "complete", outputJson, finalOrdinal: ordinal };
}
