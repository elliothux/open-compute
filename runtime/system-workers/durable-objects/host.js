// Generated from runtime/src/durable-objects/host.ts by Rolldown. Do not edit.
import { DurableObject } from "cloudflare:workers";
import { PROFILE, bindingError, doPolicy, modulesFor, resolveSnapshot, tenantEnv } from "../loader/host.js";
const INTERNAL = [
...
function tenantCompatibilityFlags(flags) {
...
function decodeWire(value) {
	...
	switch (value[0]) {
		...
		case "a":
			if (Array.isArray(value[1])) return value[1].map(decodeWire);
			break;
		case "o": {
...
function encodeWire(value, seen = new WeakSet()) {
	...
	if (!value || typeof value !== "object" || value instanceof Promise || value instanceof ReadableStream || seen.has(value)) {
		throw bindingError("DO_RPC_UNSUPPORTED");
	}
	...
	seen.add(value);
	const encoded = Array.isArray(value) ? ["a", value.map((item) => encodeWire(item, seen))] : ["o", Object.entries(value).map(([key, item]) => [key, encodeWire(item, seen)])];
	seen.delete(value);
	return encoded;
}
function required(headers, name, pattern) {
	const value = headers.get(name) || "";
	if (!pattern.test(value)) throw bindingError("DO_INTERNAL_PROTOCOL_ERROR");
	return value;
}
function authorityFromHeaders(headers) {
	...
	const workerCodeSha256 = required(headers, "x-open-compute-worker-code-sha256", /^[0-9a-f]{64}$/);
	const objectId = required(headers, "x-open-compute-object-id", /^[0-9a-f]{64}$/);
	const namespaceResourceId = required(headers, "x-open-compute-namespace-resource-id", /^[0-9a-f-]{36}$/);
	const className = required(headers, "x-open-compute-class-name", /^[A-Za-z_$][A-Za-z0-9_$]{0,127}$/);
	...
	if (!Number.isSafeInteger(routeGeneration) || routeGeneration < 1 || !Number.isSafeInteger(objectGeneration) || objectGeneration < 1) {
...
function deleteAuthorityFromHeaders(headers) {
	const objectId = required(headers, "x-open-compute-object-id", /^[0-9a-f]{64}$/);
	const objectGeneration = Number(headers.get("x-open-compute-object-generation"));
...
export class DoHost extends DurableObject {
	constructor(ctx, env) {
		super(ctx, env);
	...
	#meta() {
		const rows = this.ctx.storage.sql.exec("SELECT route_generation, deployment_id, object_generation, data_format_version " + "FROM open_compute_host_meta WHERE singleton = 1").toArray();
		return rows.length ? rows[0] : null;
	}
	async #tenant(authority) {
		...
		if (prior && authority.routeGeneration === Number(prior.route_generation) && authority.deploymentId !== prior.deployment_id) {
			throw bindingError("DO_INTERNAL_PROTOCOL_ERROR");
		}
		if (prior && authority.routeGeneration > Number(prior.route_generation)) {
		...
		const snapshot = await resolveSnapshot(this.env, envelope, false, false, this.env.INTERNAL_TOKEN);
		...
		const code = {
			...
			globalOutbound: this.ctx.exports.OutboundGateway({ props: {
		...
		if (!prior || authority.routeGeneration > Number(prior.route_generation)) {
			this.ctx.storage.sql.exec("INSERT OR REPLACE INTO open_compute_host_meta " + "(singleton, route_generation, deployment_id, object_generation, data_format_version) " + "VALUES (1, ?, ?, ?, 1)", authority.routeGeneration, authority.deploymentId, authority.objectGeneration);
		}
		return facet;
	}
	async fetch(request) {
		...
		if (operation === "alarm" || operation === "alarm-repair") {
			...
			const result = operation === "alarm" ? await facet.__openComputeAlarm(payload) : await facet.__openComputeAlarmRepair();
	...
	async #dispatchRpc(authority, method, args) {
		if (!authority || typeof authority !== "object" || typeof method !== "string" || FORBIDDEN_RPC.has(method) || method.startsWith("__openCompute") || !PUBLIC_METHOD.test(method) || !Array.isArray(args)) {
			throw bindingError("DO_RPC_UNSUPPORTED");
		}
		const facet = await this.#tenant(authority);
		const target = Reflect.get(facet, method);
