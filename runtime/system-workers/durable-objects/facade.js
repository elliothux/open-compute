// Generated from runtime/src/durable-objects/facade.ts by Rolldown. Do not edit.
...
export class DurableObjectId {
	constructor(marker, value, name) {
		...
		idState.set(this, Object.freeze({
			value,
			name
		}));
		Object.freeze(this);
...
function stubProxy(id, raw) {
	...
	const proxy = new Proxy(target, { get(owner, property, receiver) {
		...
		return async (...args) => {
			assertPlain(args);
			try {
				return decodeWire(await enqueueStubOperation(state, () => raw.dispatchRpc(id.toString(), property, encodeWire(args))));
			} catch (error) {
...
export class DurableObjectStub {
	...
	async fetch(input, init) {
		...
		try {
			...
			const outbound = new Request(`https://do-transport.invalid/${state.id.toString()}`, transport);
...
function namespaceCapability(value) {
	if (value === null || typeof value !== "object" || !("schemaVersion" in value) || value.schemaVersion !== 1 || !("namespacePrefix" in value) || typeof value.namespacePrefix !== "string" || !/^[0-9a-f]{16}$/.test(value.namespacePrefix) || !("namespaceNameKey" in value) || typeof value.namespaceNameKey !== "string" || !("maxObjectNameBytes" in value) || typeof value.maxObjectNameBytes !== "number" || !Number.isSafeInteger(value.maxObjectNameBytes) || value.maxObjectNameBytes < 1 || value.maxObjectNameBytes > 1024 || !("transport" in value) || value.transport === null || typeof value.transport !== "object") return false;
	const raw = value.transport;
	return "dispatchRpc" in raw && typeof raw.dispatchRpc === "function" && "fetch" in raw && typeof raw.fetch === "function";
}
