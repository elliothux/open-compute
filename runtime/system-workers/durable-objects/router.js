// Generated from runtime/src/durable-objects/router.ts by Rolldown. Do not edit.
export { DoHost } from "./host.js";
import { currentStartupGeneration, doPolicy, stableCode } from "../loader/host.js";
export { AlarmIndex, D1Transport, DoTransport, KVNamespace, OutboundGateway, QueueTransport, R2Transport, WorkflowBindingTransport, WorkflowBindingTransportV2 } from "../loader/host.js";
...
function value(headers, name, pattern) {
...
function admitted(env, operation) {
	const policy = doPolicy(env);
	if (activeDispatches >= policy.maxInFlightDispatches) {
		throw stableFailure("DO_STORAGE_LIMIT");
	}
	activeDispatches += 1;
	const pending = Promise.resolve().then(() => operation(policy)).finally(() => {
		activeDispatches -= 1;
	});
	return Promise.race([pending, scheduler.wait(policy.dispatchTimeoutMs).then(() => {
		throw stableFailure("DO_DISPATCH_TIMEOUT");
	})]);
}
function backendHeaders(request, env) {
	return {
		[TOKEN_HEADER]: env.BINDING_BACKEND_TOKEN,
		"x-open-compute-startup-generation": value(request.headers, "x-open-compute-startup-generation", /^[^\x00-\x1f]{1,128}$/),
		"x-open-compute-deployment-id": value(request.headers, "x-open-compute-deployment-id", /^[0-9a-f-]{36}$/),
		"x-open-compute-descriptor-sha256": value(request.headers, "x-open-compute-descriptor-sha256", /^[0-9a-f]{64}$/),
		"x-open-compute-route-generation": value(request.headers, "x-open-compute-route-generation", /^[1-9][0-9]{0,19}$/),
		"x-open-compute-request-id": value(request.headers, "x-open-compute-request-id", /^[0-9a-f-]{36}$/),
		"x-open-compute-do-operation": value(request.headers, "x-open-compute-do-operation", /^(fetch|rpc)$/),
...
async function authorize(request, env) {
	...
	const response = await env.BINDING_BACKEND.fetch(`http://binding-backend/internal/bindings/v1/do/${bindingId}/resolve`, {
		method: "POST",
		headers,
...
async function acknowledge(request, env, authority) {
	const bindingId = value(request.headers, "x-open-compute-binding-id", /^[0-9a-f-]{36}$/);
	const response = await env.BINDING_BACKEND.fetch(`http://binding-backend/internal/bindings/v1/do/${bindingId}/ready`, {
		method: "POST",
...
function hostHeaders(request, authority) {
...
function host(env, authority) {
	return env.DO_HOST.get(env.DO_HOST.idFromName(authority.hostKey));
}
async function dispatchFetch(request, env, policy) {
...
async function dispatchRpc(request, env, policy) {
	const declared = Number(request.headers.get("content-length") || 0);
	...
	try {
		...
		return error("DO_RPC_UNSUPPORTED", 400);
	}
	if (!record(payload) || typeof payload.method !== "string" || !PUBLIC_METHOD.test(payload.method) || FORBIDDEN_RPC.has(payload.method) || payload.method.startsWith("__openCompute") || !Array.isArray(payload.args)) return error("DO_RPC_UNSUPPORTED", 400);
	const authority = await authorize(request, env);
	const headers = hostHeaders(request, authority);
	headers.set("x-open-compute-do-operation", "rpc");
	headers.set("content-type", "application/json");
	const response = await host(env, authority).fetch(new Request("http://do-host/internal/rpc", {
		method: "POST",
		headers,
...
async function deleteAuthorized(request, env) {
	const authority = await request.json();
	if (!record(authority) || typeof authority.hostKey !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(authority.hostKey) || typeof authority.objectId !== "string" || !/^[0-9a-f]{64}$/.test(authority.objectId) || typeof authority.objectGeneration !== "number" || !Number.isSafeInteger(authority.objectGeneration)) {
...
async function alarmAuthority(request, env) {
	...
	if (!record(body) || typeof body.namespaceResourceId !== "string" || typeof body.objectId !== "string" || !/^[0-9a-f]{64}$/.test(body.objectId) || typeof body.objectGeneration !== "number" || !Number.isSafeInteger(body.objectGeneration) || body.objectGeneration < 1) {
		throw stableFailure("SCHEDULER_INTERNAL_PROTOCOL_ERROR");
	}
	const response = await env.BINDING_BACKEND.fetch("http://binding-backend/internal/alarms/v1/resolve", {
		method: "POST",
		headers: {
			[TOKEN_HEADER]: env.BINDING_BACKEND_TOKEN,
...
async function dispatchAlarm(request, env, repair) {
	...
	const payload = repair ? {} : {
		rowToken: body.rowToken,
		retryCount: body.retryCount
	};
	return host(env, authority).fetch(new Request(repair ? "http://do-host/internal/alarm-repair" : "http://do-host/internal/alarm", {
		method: "POST",
		headers,
		body: JSON.stringify(payload)
	}));
