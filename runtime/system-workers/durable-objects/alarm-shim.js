// Generated from runtime/src/durable-objects/alarm-shim.ts by Rolldown. Do not edit.
...
function alarmFailure(code, type = Error) {
...
function validRow(row) {
	return row !== undefined && Number(row.id) === 1 && Number.isSafeInteger(Number(row.scheduled_time_ms)) && Number(row.scheduled_time_ms) > 0 && Number.isSafeInteger(Number(row.retry_count)) && Number(row.retry_count) >= 0 && Number(row.retry_count) <= 6 && (Number(row.in_flight) === 0 || Number(row.in_flight) === 1) && typeof row.row_token === "string" && ROW_TOKEN.test(row.row_token) && (row.last_error_code === null || typeof row.last_error_code === "string") && Number.isSafeInteger(Number(row.updated_at_ms));
}
function readRow(storage) {
	ensureTable(storage);
	const rows = storage.sql.exec(`SELECT id, scheduled_time_ms, retry_count, in_flight, row_token,
            last_error_code, updated_at_ms FROM ${TABLE} WHERE id = 1`).toArray();
...
function exactDelete(storage, rowToken) {
	ensureTable(storage);
	storage.sql.exec(`DELETE FROM ${TABLE} WHERE id = 1 AND row_token = ?`, rowToken);
}
function restoreIfAbsent(storage, row) {
	if (!row) return;
	ensureTable(storage);
	storage.sql.exec(`INSERT INTO ${TABLE}
       (id, scheduled_time_ms, retry_count, in_flight, row_token, last_error_code, updated_at_ms)
     VALUES (1, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING`, row.scheduledTimeMs, row.retryCount, row.inFlight ? 1 : 0, row.rowToken, row.lastErrorCode, row.updatedAtMs);
}
...
function setAlarm(storage, index, transaction, value) {
	if (transaction?.sync) {
	...
	return (async () => {
       ...
       ON CONFLICT(id) DO UPDATE SET scheduled_time_ms = excluded.scheduled_time_ms,
         ...
         last_error_code = NULL, updated_at_ms = excluded.updated_at_ms`, scheduledTimeMs, rowToken, Date.now());
		if (transaction) {
...
function getAlarm(storage, index, transaction) {
	return (async () => {
...
function deleteAlarm(storage, index, transaction) {
	if (transaction?.sync) {
...
async function deleteAllStorage(storage, transaction) {
	...
	const objects = [...storage.sql.exec("SELECT type, name FROM sqlite_master " + "WHERE type IN ('trigger', 'view', 'table') " + "ORDER BY CASE type WHEN 'trigger' THEN 0 WHEN 'view' THEN 1 ELSE 2 END, rowid DESC")];
...
async function flushTransaction(rootStorage, index, initial, final) {
	try {
		if (final) {
...
function sameProjection(left, right) {
	...
	return left.rowToken === right.rowToken && left.scheduledTimeMs === right.scheduledTimeMs && left.retryCount === right.retryCount;
}
function wrapStorage(storage, index, transaction, rootStorage) {
	return new Proxy(storage, { get(target, property) {
		const alarmStorage = rootStorage;
		if (property === "setAlarm") return (value) => setAlarm(alarmStorage, index, transaction, value);
		...
		if (property === "transactionSync") {
			...
			return (callback) => target.transactionSync(() => callback(wrapStorage(target, index, {
				sync: true,
				mutated: false
			}, rootStorage)));
		}
		if (property === "transaction") {
			...
			return async (callback) => {
				...
				const result = await target.transaction(async (nativeTransaction) => {
					...
					const value = await callback(wrapStorage(nativeTransaction, index, attempt, rootStorage));
					committed.value = {
...
export function prepareDurableObjectContext(ctx, index) {
...
export function activateDurableObjectAlarm(prepared) {
...
export async function repairDurableObjectAlarm(prepared) {
	const row = readRow(prepared.storage);
...
export async function dispatchDurableObjectAlarm(instance, handler, prepared, payload) {
	if (payload === null || typeof payload !== "object" || !("rowToken" in payload) || !("retryCount" in payload) || typeof payload.rowToken !== "string" || !ROW_TOKEN.test(payload.rowToken) || typeof payload.retryCount !== "number" || !Number.isSafeInteger(payload.retryCount) || payload.retryCount < 0 || payload.retryCount > 6) {
	...
	prepared.storage.sql.exec(`UPDATE ${TABLE} SET in_flight = 1, retry_count = ?, updated_at_ms = ?
     WHERE id = 1 AND row_token = ?`, payload.retryCount, now, payload.rowToken);
	try {
		...
		if (payload.retryCount >= 6) {
			...
			return {
				outcome: "exhausted",
				errorCode: "DO_RUNTIME_EXCEPTION"
			};
		}
		const retryCount = payload.retryCount + 1;
		const scheduledTimeMs = Date.now() + 2e3 * 2 ** payload.retryCount;
		prepared.storage.sql.exec(`UPDATE ${TABLE} SET scheduled_time_ms = ?, retry_count = ?, in_flight = 0,
              last_error_code = 'DO_RUNTIME_EXCEPTION', updated_at_ms = ?
       WHERE id = 1 AND row_token = ?`, scheduledTimeMs, retryCount, Date.now(), payload.rowToken);
		const retry = {
			scheduledTimeMs,
			retryCount,
		...
		try {
			await prepared.index.upsert(retry);
		} catch {}
		return {
			outcome: "retry",
			scheduledTimeMs,
