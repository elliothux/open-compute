...
async function authorize(request, env) {
  ...
  if (!response.ok) {
    const failure = new Error(response.headers.get(ERROR_HEADER) || "DO_STORAGE_UNAVAILABLE");
    failure.stableCode = failure.message;
    throw failure;
  }
...
async function deleteAuthorized(request, env) {
  ...
  if (!authority || !/^[A-Za-z0-9_-]{43}$/.test(authority.hostKey || "")
