// Generated from runtime/src/loader/modules.ts by Rolldown. Do not edit.
...
import { D1_FACADE_MODULE, DO_ALARM_SHIM_MODULE, DO_FACADE_MODULE, DO_ID_CODEC_MODULE, DO_WRAPPER_MODULE, INTERNAL_MODULE_PREFIX, LOADED_ISOLATE_WRAPPER_MODULE, QUEUE_FACADE_MODULE, R2_FACADE_MODULE, VALIDATION_MODULE, WORKFLOW_FACADE_MODULE, WORKFLOW_JSON_MODULE, WORKFLOW_JSON_V2_MODULE, WORKFLOW_RUNNER_MODULE, WORKFLOW_RUNNER_V2_MODULE, WORKFLOW_V2_FACADE_MODULE, WORKFLOW_WRAPPER_MODULE, WRAPPER_RUNTIME_MODULE, generateBindingWrapper, generateValidationWrapper } from "./wrappers/generator.js";
...
function moduleValue(module) {
	...
	switch (module.type) {
		case "esModule": return { js: new TextDecoder("utf-8", {
			fatal: true,
			ignoreBOM: false
		}).decode(raw) };
		case "commonJsModule": return { cjs: new TextDecoder("utf-8", {
			fatal: true,
			ignoreBOM: false
		}).decode(raw) };
		case "text": return { text: new TextDecoder("utf-8", {
			fatal: true,
			ignoreBOM: false
		}).decode(raw) };
		case "json": return { json: JSON.parse(new TextDecoder("utf-8", {
			fatal: true,
			ignoreBOM: false
		}).decode(raw)) };
		case "data": return { data: raw.buffer };
		case "wasm": return { wasm: raw };
		default: throw new Error("unsupported module representation");
	}
}
export function modulesFor(snapshot, validation, entrypointName, durableObject = false, workflow = false, workflowCapability = 1) {
	...
	const has = (kind, version = 1) => snapshot.bindings.some((binding) => binding.kind === kind && binding.capabilityVersion === version);
	if (snapshot.bindings.some((binding) => binding.kind === "workflow" && binding.capabilityVersion !== 1 && binding.capabilityVersion !== 2)) throw bindingError("WORKFLOW_CAPABILITY_MISMATCH");
	...
	if (workflow) {
		...
		modules[workflowCapability === 2 ? WORKFLOW_RUNNER_V2_MODULE : WORKFLOW_RUNNER_MODULE] = { js: workflowCapability === 2 ? workflowRunnerV2Source : workflowRunnerSource };
	}
	...
	modules[LOADED_ISOLATE_WRAPPER_MODULE] = { js: generateBindingWrapper({
		mainModule: snapshot.mainModule,
		bindings: snapshot.bindings,
		entrypointName,
		durableObject,
		workflowCapability: workflow ? workflowCapability : undefined
	}) };
	if (validation) {
		modules[VALIDATION_MODULE] = { js: generateValidationWrapper(entrypointName) };
		return {
			modules,
			mainModule: VALIDATION_MODULE
		};
	}
	return {
		modules,
