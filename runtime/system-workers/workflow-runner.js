...
import { workflowError, workflowFailure, workflowJson, workflowString } from "./__open_compute_workflow_json__.js";
...
export async function runWorkflow(target, ctx, env, event, backend) {
  ...
  const step = Object.freeze({
    async do(name, callback, ...extra) {
      ...
      try {
        const identity = { ordinal: index, name, nameCount: count, configJson: "null" };
        const grant = await rpc("claim", identity);
        ...
        try {
          ...
          await rpc("failure", { ordinal: index, error: workflowFailure() });
        ...
        try { outputJson = workflowJson(value); }
          ...
          const code = error?.message === "WORKFLOW_RESULT_TOO_LARGE"
            ? "WORKFLOW_RESULT_TOO_LARGE" : "WORKFLOW_SERIALIZATION_UNSUPPORTED";
          failure ||= code;
          await rpc("failure", { ordinal: index, error: workflowFailure(), errorCode: code });
          throw workflowError(code);
        }
        await rpc("success", { ordinal: index, outputJson });
  ...
  try {
    ...
    try { outputJson = workflowJson(value); }
      ...
      reject(error?.message === "WORKFLOW_RESULT_TOO_LARGE"
        ? "WORKFLOW_RESULT_TOO_LARGE" : "WORKFLOW_SERIALIZATION_UNSUPPORTED");
    }
