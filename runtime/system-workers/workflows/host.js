// Generated from runtime/src/workflows/host.ts by Rolldown. Do not edit.
...
function assertActivation(value) {
	for (const name of [
		"instanceId",
		"runToken",
		"externalInstanceId",
	...
	if (typeof value.instanceGeneration !== "number" || !Number.isSafeInteger(value.instanceGeneration) || typeof value.createdAtMs !== "number" || !Number.isSafeInteger(value.createdAtMs)) throw new Error("invalid activation");
}
export class WorkflowBindingTransport extends WorkerEntrypoint {
	async #request(operation, body) {
		...
		const response = await this.env.BINDING_BACKEND.fetch(`http://binding-backend/internal/bindings/v1/workflow/${props.bindingId}/${operation}`, {
			method: "POST",
			headers: {
	...
	async status(id) {
		return readWorkflowStatus(await this.#request("status", { id }));
	}
}
...
class WorkflowRunController extends RpcTarget {
	...
	async #request(operation, body) {
		...
		const response = await this.#env.BINDING_BACKEND.fetch(`http://binding-backend/internal/workflows/v1/runs/${operation}`, {
			method: "POST",
			headers: {
				"content-type": "application/json",
				"x-open-compute-binding-token": this.#env.BINDING_BACKEND_TOKEN,
	...
	async claim(body) {
		...
		if (reply?.state !== "run" || typeof reply.stepToken !== "string" || !/^[0-9a-f]{64}$/.test(reply.stepToken)) {
			throw new Error("WORKFLOW_RUNTIME_UNAVAILABLE");
		}
		this.#grant = {
	...
	async #commit(operation, body) {
		...
		const reply = await this.#request(operation, {
			...body,
...
export async function handleWorkflow(request, env, ctx, validation, capability = 1) {
	try {
		...
		if (!/^[A-Za-z_$][A-Za-z0-9_$]{0,127}$/.test(className) || !/^[0-9a-f]{64}$/.test(expected) || loaderKey.split("/").length !== 3) throw new Error("invalid envelope");
		const snapshot = await resolveSnapshot(env, {
			loaderKey,
			expected
		}, validation, true, request.headers.get("x-open-compute-internal-token"));
		...
		let cold = false;
		const key = capability === 1 ? `workflow/${validation}/${loaderKey}/${expected}/${className}` : `workflow-v2/${validation}/${loaderKey}/${expected}/${className}/${body.versionDescriptorSha256}`;
		...
		const identity = {
			...
			instanceGeneration: body.instanceGeneration,
		...
		try {
			const result = await target.execute({
				externalInstanceId: body.externalInstanceId,
			...
			if (backend instanceof WorkflowRunControllerV2) {
				return Response.json({
					...await backend[finishWorkflowRun](result),
		...
		if (code === "ARTIFACT_INTEGRITY_ERROR" || code === "DEPLOYMENT_INVARIANT_VIOLATION") {
			return new Response(null, {
				status: 422,
				headers: { "x-open-compute-error-code": code === "ARTIFACT_INTEGRITY_ERROR" ? code : "WORKFLOW_INVARIANT_VIOLATION" }
			});
		}
