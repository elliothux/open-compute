...
function safeInteger(value) {
  if (!Number.isSafeInteger(value) || value < 0) typeError("R2_INVALID_OPTIONS");
  return value;
}

function etags(value) {
...
export class R2ObjectBody extends R2Object {
  ...
  async #consume(kind) {
    ...
    state.claimed = true;
    if (kind === "arrayBuffer") return state.response.arrayBuffer();
    if (kind === "text") return state.response.text();
    if (kind === "json") return state.response.json();
    if (kind === "blob") return state.response.blob();
    return new Uint8Array(await state.response.arrayBuffer());
  }

  bytes() { return this.#consume("bytes"); }
  arrayBuffer() { return this.#consume("arrayBuffer"); }
  text() { return this.#consume("text"); }
  json() { return this.#consume("json"); }
  blob() { return this.#consume("blob"); }
}

export class R2Bucket {
  constructor(raw) {
    if (!raw || typeof raw !== "object") typeError("R2_INTERNAL_PROTOCOL_ERROR");
