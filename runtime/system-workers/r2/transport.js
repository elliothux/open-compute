// Generated from runtime/src/r2/transport.ts by Rolldown. Do not edit.
...
function assertMetadata(value, bindingError) {
	if (!isRecord(value) || typeof value.key !== "string" || typeof value.etag !== "string" || typeof value.httpEtag !== "string" || typeof value.size !== "number" || !Number.isSafeInteger(value.size) || value.size < 0 || typeof value.uploaded !== "number" || !Number.isSafeInteger(value.uploaded) || value.version !== undefined && typeof value.version !== "string" || value.md5 !== undefined && typeof value.md5 !== "string" || value.storageClass !== "Standard" || !isRecord(value.httpMetadata) || !isRecord(value.customMetadata) || Object.values(value.customMetadata).some((item) => typeof item !== "string")) {
	...
	for (const [key, item] of Object.entries(value.httpMetadata)) {
		if (![
			...
			"cacheControl",
			"cacheExpiry"
		].includes(key) || item != null && (key === "cacheExpiry" ? typeof item !== "number" || !Number.isSafeInteger(item) : typeof item !== "string")) {
			throw bindingError("BINDING_PROTOCOL_ERROR");
		}
	}
	if (value.range != null) {
		if (!isRecord(value.range) || Object.entries(value.range).some(([key, item]) => ![
			...
			"length",
			"suffix"
		].includes(key) || item !== null && (typeof item !== "number" || !Number.isSafeInteger(item) || item < 0))) {
			throw bindingError("BINDING_PROTOCOL_ERROR");
		}
	}
}
...
export function makeR2TransportBase(bindingError, currentStartupGeneration, tokenHeader) {
	return class extends WorkerEntrypoint {
		...
		async #request(operation, body, permission, contentType = CONTENT_TYPE) {
			...
			const response = await this.env.BINDING_BACKEND.fetch(`http://binding-backend/internal/bindings/v1/r2/${props.bindingId}/${operation}`, {
				method: "POST",
				headers: {
		...
		async get(key, options) {
			const response = await this.#request("get", JSON.stringify({
				key,
				options
			}), "read", FRAME_CONTENT_TYPE);
			return response.status === 204 ? null : decodeFrame(response, bindingError);
		}
		async put(key, body, options) {
			const response = await this.#request("put", framedPutBody({
				key,
				options
			}, body, bindingError), "write", FRAME_CONTENT_TYPE);
			if (response.status === 204) return null;
			const meta = await response.json();
			assertMetadata(meta, bindingError);
			return meta;
		...
		async list(options) {
			...
			if (!isRecord(result) || !Array.isArray(result.objects) || typeof result.truncated !== "boolean" || result.cursor !== null && typeof result.cursor !== "string" || !stringArray(result.delimitedPrefixes)) throw bindingError("BINDING_PROTOCOL_ERROR");
			const objects = [];
