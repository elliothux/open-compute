// Generated from runtime/src/r2/facade.ts by Rolldown. Do not edit.
...
function assertKey(value) {
	...
	for (let i = 0; i < value.length; i++) {
		...
		if (code >= 55296 && code <= 56319) {
			...
			if (!(next >= 56320 && next <= 57343)) typeError("R2_KEY_INVALID");
...
export class R2Bucket {
	...
	async list(options = {}) {
		...
		const limit = input.limit == null ? 1e3 : safeInteger(input.limit);
		if (limit < 1 || limit > 1e3) typeError("R2_INVALID_OPTIONS");
		...
		return {
			...result,
			objects: (result.objects || []).map((meta) => new R2Object(meta))
		};
	}
...
function rawTransport(raw) {
	return raw !== null && typeof raw === "object" && "head" in raw && typeof raw.head === "function" && "get" in raw && typeof raw.get === "function" && "put" in raw && typeof raw.put === "function" && "delete" in raw && typeof raw.delete === "function" && "list" in raw && typeof raw.list === "function";
}
