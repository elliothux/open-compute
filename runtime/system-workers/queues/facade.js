// Generated from runtime/src/queues/facade.ts by Rolldown. Do not edit.
const producerState = new WeakMap();
const encoder = new TextEncoder();
const MAX_MESSAGE_BYTES = 128e3;
const MAX_BATCH_MESSAGES = 100;
const MAX_BATCH_BODY_BYTES = 256e3;
...
function options(value, allowed) {
	...
	const output = {};
	if (Object.prototype.hasOwnProperty.call(input, "delaySeconds") && input.delaySeconds !== undefined) {
		output.delaySeconds = delay(input.delaySeconds);
	}
	if (Object.prototype.hasOwnProperty.call(input, "contentType") && input.contentType !== undefined) {
...
function publicMetrics(input) {
	const value = object(input, "QUEUE_INVARIANT_VIOLATION");
	if (typeof value.backlogCount !== "number" || !Number.isSafeInteger(value.backlogCount) || value.backlogCount < 0 || typeof value.backlogBytes !== "number" || !Number.isSafeInteger(value.backlogBytes) || value.backlogBytes < 0 || value.oldestMessageTimestampMs !== null && (typeof value.oldestMessageTimestampMs !== "number" || !Number.isSafeInteger(value.oldestMessageTimestampMs))) {
...
export class QueueProducer {
	...
	async send(body, rawOptions) {
		...
		return response(await this.#state().raw.send(frame([message], undefined, 1)));
	}
	async sendBatch(iterable, rawOptions) {
		...
		return response(await this.#state().raw.sendBatch(frame(messages, normalized.delaySeconds, 2)));
	}
	async metrics() {
...
function rawTransport(raw) {
	return raw !== null && typeof raw === "object" && "send" in raw && typeof raw.send === "function" && "sendBatch" in raw && typeof raw.sendBatch === "function" && "metrics" in raw && typeof raw.metrics === "function";
}
