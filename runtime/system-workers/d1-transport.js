...
class FrameWriter {
  ...
  u8(value) { this.add(Uint8Array.of(value)); }
  u16(value) { const out = new Uint8Array(2); new DataView(out.buffer).setUint16(0, value); this.add(out); }
  u32(value) { const out = new Uint8Array(4); new DataView(out.buffer).setUint32(0, value); this.add(out); }
  i64(value) { const out = new Uint8Array(8); new DataView(out.buffer).setBigInt64(0, BigInt(value)); this.add(out); }
  f64(value) { const out = new Uint8Array(8); new DataView(out.buffer).setFloat64(0, value); this.add(out); }
  bytes(value) { this.u32(value.byteLength); this.add(value); }
...
export function makeD1TransportBase(bindingError, currentStartupGeneration, tokenHeader) {
  return class extends WorkerEntrypoint {
    ...
    async fetch() { throw bindingError("BINDING_PERMISSION_DENIED"); }
  };
}
