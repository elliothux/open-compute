...
export class WorkflowRunControllerV2 extends RpcTarget {
  ...
  async claimBatch(body) {
    ...
    try {
      ...
      if (reply?.errorCode || reply?.state === "suspended") return this.#verdict(reply);
      ...
      for (let i = 0; i < reply.steps.length; i++) {
        ...
        if (step.state !== "run") {
          ...
          if (!["complete", "failed", "suspended"].includes(step.state)) throw new Error("invalid state");
        ...
        if (typeof step.stepToken !== "string" || !/^[0-9a-f]{64}$/.test(step.stepToken)
            || !Number.isInteger(step.attempt) || step.attempt < 1 || step.attempt > 101
            || !Number.isSafeInteger(step.remainingMs) || step.remainingMs < 0 || step.remainingMs > 240000) {
          throw new Error("invalid grant");
        }
        let resolve;
        const result = new Promise(done => { resolve = done; });
        let acknowledge;
        const acknowledgment = new Promise(done => { acknowledge = done; });
        const grant = { stepToken: step.stepToken, attempt: step.attempt,
