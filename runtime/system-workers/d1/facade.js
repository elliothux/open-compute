// Generated from runtime/src/d1/facade.ts by Rolldown. Do not edit.
...
function sqlText(value) {
	...
	if (encoder.encode(value).byteLength > 1e5) typeError("D1_SQL_INVALID");
	return value;
}
...
function statement(owner, sql, params = []) {
	...
	statementState.set(result, Object.freeze({
		owner,
		sql,
...
function assertMeta(meta) {
	...
	if (Object.keys(meta).some((key) => !required.includes(key)) || required.some((key) => !Object.prototype.hasOwnProperty.call(meta, key)) || meta.served_by !== "open-compute-local" || meta.served_by_primary !== true || typeof meta.changed_db !== "boolean" || required.slice(2).filter((key) => key !== "changed_db").some((key) => typeof meta[key] !== "number" || !Number.isFinite(meta[key]))) {
		typeError("D1_INTERNAL_PROTOCOL_ERROR");
	}
	return meta;
}
...
function assertTransportResult(result) {
	if (!isRecord(result) || !Array.isArray(result.results)) typeError("D1_INTERNAL_PROTOCOL_ERROR");
	return result.results.map((entry) => {
		if (!isRecord(entry) || !Array.isArray(entry.columns) || !Array.isArray(entry.rows) || entry.columns.length > 100 || entry.columns.some((name) => typeof name !== "string")) {
			typeError("D1_INTERNAL_PROTOCOL_ERROR");
		}
...
export class D1PreparedStatement {
	...
	async raw(options = {}) {
		if (!isRecord(options) || Object.keys(options).some((key) => key !== "columnNames") || options.columnNames !== undefined && typeof options.columnNames !== "boolean") {
			typeError("D1_TYPE_ERROR");
		}
...
export class D1Database {
	...
	async exec(sql) {
		...
		if (!isRecord(result) || typeof result.count !== "number" || !Number.isSafeInteger(result.count) || result.count < 1 || typeof result.duration !== "number" || !Number.isFinite(result.duration)) typeError("D1_INTERNAL_PROTOCOL_ERROR");
		return {
			count: result.count,
	...
	withSession(constraint = "first-unconstrained") {
		...
		sessionState.set(session, Object.freeze({
