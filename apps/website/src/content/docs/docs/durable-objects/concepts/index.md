...
See [Alarms](/docs/durable-objects/alarms). `getAlarm` / `setAlarm` / `deleteAlarm` and the `alarm()` handler are supported. Scheduling still happens on this node.

## WebSocket hibernation

`state.acceptWebSocket`, `webSocketMessage` / `webSocketClose` / `webSocketError`, tags, and attachment serialize/deserialize are supported. Runtime details: [WebSockets](/docs/workers/runtime-apis/websockets). The object still lives on this one workerd. Cross-edge hibernation migration is not provided.
