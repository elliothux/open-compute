...
见 [Alarms](/docs/zh/durable-objects/alarms)。`getAlarm` / `setAlarm` / `deleteAlarm` 与 `alarm()` 处理函数均支持。调度仍在本机。

## WebSocket hibernation

`state.acceptWebSocket`、`webSocketMessage` / `webSocketClose` / `webSocketError`、tags、attachment serialize/deserialize 均支持。运行时细节见 [WebSockets](/docs/zh/workers/runtime-apis/websockets)。对象仍位于本机单个 workerd 进程，不提供跨边缘休眠迁移。
