...
与 [快速开始](/docs/zh/workers/get-started/) 使用同一份 hello-worker。
...
Worker 中的 KV API 与 [KV](/docs/zh/kv/) 对齐。数据位于本机一份 SQLite，不提供全球复制。
...
Cron 产品见 [Cron Triggers](/docs/zh/workers/configuration/cron-triggers)。handler 与 [Cloudflare scheduled()](https://developers.cloudflare.com/workers/runtime-apis/handlers/scheduled/) 对齐。
...
同一平台账户内 Worker 之间的 fetch / RPC，见 [bindings](/docs/zh/workers/runtime-apis/bindings)。
...
下一步：[配置](/docs/zh/workers/configuration/)、[Runtime APIs](/docs/zh/workers/runtime-apis/)。
