...
Same hello-worker as [Get started](/docs/workers/get-started/).
...
The Worker-side KV API matches [KV](/docs/kv/). Authority is a SQLite database on this node. Global replication is not provided.
...
The Cron product lives at [Cron Triggers](/docs/workers/configuration/cron-triggers). The handler matches [Cloudflare scheduled()](https://developers.cloudflare.com/workers/runtime-apis/handlers/scheduled/).
...
Fetch / RPC between Workers in the same platform account. See [bindings](/docs/workers/runtime-apis/bindings).
...
Next: [Configuration](/docs/workers/configuration/), [Runtime APIs](/docs/workers/runtime-apis/).
