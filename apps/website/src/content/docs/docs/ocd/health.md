...
| `GET /health/ready`  | `200` when the shared daemon entrance is serving       | Unreachable if the entrance is down | Shared entrance readiness, not instance readiness |
| `GET /health/status` | Instance JSON: `readiness`, `components`, `supervisor` | `401` if the Bearer does not match  | Inspect the selected instance                     |
...
`/health/ready` reports the one shared entrance. It does not aggregate instance health: one stopped or degraded instance does not make the daemon unready. Use the selected instance's `/health/status` on `<instance_id>.localhost` with an authorized Bearer, or `ocd instances`, for its own readiness and failure reason. Do not restart the daemon because one instance is degraded.

The object component on instance `/health/status` is `object_storage` for both Local and S3. States are `starting` / `healthy` / `degraded` / `failed` / `draining`. For Local, free-space thresholds are rechecked during maintenance; hard pressure refuses instance writes and readiness without changing daemon liveness.

The daemon listen address comes from `[server].public_bind` in `OCD_DIR/ocd.toml` (default `127.0.0.1:8787`). A separate `[server].admin_bind` is optional.
...
On readiness failure, follow the [incident handbook](/docs/ocd/incidents/) by symptom. Do not restart first and hope.
