...
This page covers running an **already issued** OS/CPU-matching `ocd` file as a long-running service: container, systemd, and launchd. It is not Worker code deployment (`ocd wrangler deploy`; see [Get started](/docs/get-started)). It does not cover building that file from source.
