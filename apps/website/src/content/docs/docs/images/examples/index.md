...
The chain is `input` / `transform` / `draw` / `output` / `response()` / `info()`. Options: [Guides](/docs/images/guides/). This is a bounded local raster transform binding, not hosted Cloudflare Images. Setup: [Get started](/docs/images/get-started/).
