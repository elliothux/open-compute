...
Use [`llms-small.txt`](https://open-compute.dev/llms-small.txt) instead when the agent has a smaller context window. [`llms.txt`](https://open-compute.dev/llms.txt) is the lightweight index of all available LLM documentation sets.
