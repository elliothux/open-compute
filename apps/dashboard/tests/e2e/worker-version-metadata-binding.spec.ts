...
test("version metadata binding adds, edits and deletes an exact deployed Version", async ({
  page,
}) => {
  ...
  const worker = `pw-version-metadata-${crypto.randomUUID().slice(0, 8)}`;
  ...
  try {
    ...
    const siblingVersion = await activeVersion();
    expect(siblingVersion.bindings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: "images", name: "IMAGES" }),
      ]),
    );

    await openBinding("Version metadata");
    const dialog = page.getByRole("dialog", {
      name: "Add version metadata binding",
    });
    await expect(
    ...
    await dialog
      .getByRole("textbox", { name: "Binding name" })
      .fill("CF_VERSION_METADATA");
    if (process.env.OPEN_COMPUTE_DASHBOARD_CAPTURE_VERSION_METADATA === "1") {
      await page.setViewportSize({ width: 1808, height: 1169 });
      await dialog.screenshot({
        path: resolve(
          import.meta.dirname,
          "../../../../dashboard-refactor/implementation-screenshots/worker-version-metadata-binding-add-desktop.png",
        ),
      });
      ...
      await dialog.screenshot({
        path: resolve(
          import.meta.dirname,
          "../../../../dashboard-refactor/implementation-screenshots/worker-version-metadata-binding-add-mobile.png",
        ),
      });
      await expect
    ...
    expect(added.id).not.toBe(siblingVersion.id);
    ...
    await page
      .getByRole("button", { name: "Edit CF_VERSION_METADATA" })
      .click();
    const edit = page.getByRole("dialog", {
      name: "Edit version metadata binding",
    });
    ...
    expect(
      edited.bindings.some((item) => item.name === "CF_VERSION_METADATA"),
    ).toBe(false);

    await page.getByRole("button", { name: "Delete VERSION_INFO" }).click();
    const confirmation = page.getByRole("alertdialog", {
      name: "Delete Version metadata binding?",
    });
    await confirmation
    ...
    expect(
      removed.bindings.some((item) => item.type === "version_metadata"),
    ).toBe(false);
