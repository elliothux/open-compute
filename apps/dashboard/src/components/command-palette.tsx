...
} from "../features/navigation/command-palette-atoms";
...
const destinations = [
  ...
  { label: "R2", to: "/r2" as const, keywords: "object storage bucket s3" },
  {
    ...
    to: "/durable-objects" as const,
    keywords: "do namespace object",
  },
  {
    label: "Queues",
    to: "/queues" as const,
    keywords: "message queue consumer",
  },
  {
    label: "Workflows",
    to: "/workflows" as const,
    keywords: "workflow definition instance",
  },
  {
    label: "Platform",
    to: "/platform" as const,
    keywords: "scheduler cache cron images",
  },
];
...
export function CommandPalette() {
  ...
  const [query, setQuery] = useAtom(commandPaletteQueryAtom);
  const close = useSetAtom(closeCommandPaletteAtom);
  ...
  return (
    <>
      ...
      >
        ...
        <span className="ring-kumo-line text-kumo-subtle hidden rounded px-1.5 py-0.5 text-xs ring md:inline">
          ⌘K
        </span>
      </Button>
      ...
      >
        <KumoCommandPalette.Input
          autoFocus
          placeholder="Jump to Workers, KV, Platform…"
        />
