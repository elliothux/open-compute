...
| [P18 公网 Gateway、DNS 与 TLS](p18-single-domain-public-gateway-acceptance.md)               | 本地实现完成；仍需具备公网 TCP 443、UDP/TCP 53 和专用测试域名的真实 DNS-01/HTTPS 资格                                     |
| [R1 单 daemon、多 Instance](r1-single-daemon-instances-acceptance.md)                        | 本地核心与 macOS launchd 隔离验收完成；Linux systemd、多真实 instance、备份恢复及正式发行布局仍待资格                     |
| [I102 Dynamic Worker 扩展产品](i102-dynamic-worker-binding-forwarding-acceptance.md)         | 首批 KV/D1/R2/Queue 已完成；其他已装配 facade 必须逐产品通过正式 pin 的真实行为矩阵后才能声明支持                         |
