...
每个单文件 platformd 从内嵌资源和 production registries 生成 machine-readable metadata：
...
单文件构建使用正式 pinned official workerd archive 和双重 SHA-256 verification。release metadata
由 production registries 和内嵌字节生成，不依赖可编辑的相邻 release.json；capabilities CLI 可离线读取身份。
...
P1 发布物是一个原生可执行文件，只有一个 service unit；完整当前契约见 [单二进制部署](../references/single-binary.md)：

```text
platformd   # 内嵌 pinned workerd、lock、模板、JS、默认配置、许可证和手册
```
...
- 首次 install 后 `config init -> config check -> run`；已有平台停机后才执行 `doctor --full`；
