...
| Dynamic Workers                              | `blocked`（23 已资格，2 实验项缺口） |    25 | 三个正式平台的 native fork；load/get、七类模块、scoped env/RPC、tail、facet、4/10 原生计数与 restart/delete 产品路径。W2 已原生执行 invocation CPU/subrequest 预算、128 MiB isolate 内存、1 s startup 与每 invocation 6 个并发出站连接，超限 isolate 被摘除并从 immutable Version 重建；`/internal/live` 功能性探活对卡死 generation 做确认式单次重启。custom limits 经 Wrangler/v4 → Version → snapshot → Loader → workerd 全链生效；当前认证日期的 Python 使用 formal-lock 固定并随 `ocd` gzip 内嵌的 Pyodide bundle；其它官方 child 日期/flag 组合不属于单 bundle 离线资格。macOS Intel 与 Windows 仅手动编译，不属于 release 资格；实验 trust/streaming tails 不开放 | `OC-WKR-LIMIT-001`                                |
...
自恢复已由用户 fork 实现并通过完整验收，见 [workerd W2](../implemented/w2-standard-limits.md)。
