...
4 个 custom-limit 成员已有 W2 原生执行器，但 Wrangler/v4 public 链和产品资格仍未完成；2 个
experimental-control 成员不在公开 W1 子集。
...
| Dynamic Workers                              | `blocked`（19 已资格，6 个缺口）     |    25 | 三个正式平台的 native fork；load/get、七类模块、scoped env/RPC、tail、facet、4/10 原生计数与 restart/delete 产品路径。W2 已原生执行 invocation CPU/subrequest 预算、128 MiB isolate 内存、1 s startup 与每 invocation 6 个并发出站连接，超限 isolate 被摘除并从 immutable Version 重建；`/internal/live` 功能性探活对卡死 generation 做确认式单次重启。custom limits 的内部 Version → snapshot → Loader → workerd 链已生效，但 Wrangler/v4 snake_case、toolchain、Settings API、entrypoint ceiling 和公开错误/产品验收仍在活动 W2 中，4 个 custom-limit 成员尚未资格化；当前认证日期的 Python 使用 formal-lock 固定并随 `ocd` gzip 内嵌的 Pyodide bundle；其它官方 child 日期/flag 组合不属于单 bundle 离线资格。macOS Intel 与 Windows 仅手动编译，不属于 release 资格；2 个实验 trust/streaming tails 不开放 | `OC-WKR-LIMIT-001`                                |
...
runtime unavailable，不宣称执行 CPU limit 或产生 `exceededCpu`。原生 limits、isolate 摘除和 supervisor
自恢复已由用户 fork 实现并通过运行时验收；完整 public limits 对齐仍在进行，见
[workerd W2](../w2-standard-limits.md)。
