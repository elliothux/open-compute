...
Browser Run 分别见 [P7](implemented/p7-workers-logs-realtime-tail.md)、[workerd W2](workerd/w2-standard-limits.md)和
...
- method 计入 workerd W1 subrequest/resource accounting；日志按 P7 做 redaction，不能记录 file/blob body 或 token plaintext。
...
- workerd W1 subrequest accounting 与 P7 logging。
...
- P7 与 workerd W1 集成完成，或相应调用在 capability inventory 中保持明确 planned 且不会伪装完整支持；
