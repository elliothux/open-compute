# P17：Cloudflare Containers 兼容设计
...
中的 Container upload、Containers control API 和 Durable Object Container runtime。P17 以固定 Cloudflare 公共合同、
...
P17 与 [P16 Browser Run](p16-browser-run.md) 共享单 executable 分发目标以及外部／嵌入式 runtime 的 ownership 边界：
...
context 上的 native capability，包含 Fetcher、Socket、streaming exec、monitor 和 lifecycle identity。P17 不用普通 HTTP
...
最后启动 rollout；后两步不是事务，Wrangler success 只表示 rollout 已启动。P17 必须兼容固定 Wrangler 可观察到的顺序和错误，
...
- 不保留 Wrangler 旧 `containers.configuration` 兼容路径；P17 只实现固定 Day 1 schema。
...
P17 实施时直接修改当前 Day 1 validator/schema/fixtures，不保留“旧版本拒绝、新版本接受”的历史分支。
...
P17 只保留两条有顺序的实现路线，不再把 Podman、containerd、crun/runc 或直接修改 workerd provider API 列为并行产品方案：
...
Worker code可能在 rollout 完成前访问旧 image instance。P17 因此必须持久化：
...
共享其 network namespace。P17 保留一个 authoritative egress path：
...
容器部署自身平台组件，不作为 P17 provider或API authority。后续发现的 WDL功能只有通过同一官方合同/测试后才可引用，不能因
...
Exit：若 native capability不能安全挂到 dynamic loaded DO/facet，P17保持 unsupported；不能退回JS模拟或把所有tenant共用一个
