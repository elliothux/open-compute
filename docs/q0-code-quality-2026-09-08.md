...
`crates/search`、`crates/document-parser`、`crates/images`、`packages/cloudflare-extension`、`apps/website`、
...
- [x] 将 `test/conformance/differential.ts` 按产品或协议领域拆分，但保留一个 differential entrypoint、统一 credential
      policy 和一份结果汇总；
...
- [x] 将非跨 crate API 收窄为 private 或 `pub(crate)`；本专项不改变产品 public API，也不引入 public API surface
      snapshot/check，避免把当前 pre-1.0 表面冻结成兼容合同；
