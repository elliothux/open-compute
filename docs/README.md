...
| [P11 ocd 安装、实例与本机运维体验](p11-ocd-operator-experience.md) | P11.1–P11.4 源码已落地；正式 runner 安装/OS service 冒烟与单轮 workspace Gate / coverage 验收待完成，完成前不归档 |
| [P12 Wrangler 项目开发与部署体验](p12-wrangler-project-workflow.md) | Day 1 产品合同与架构完成；待实现 local instance/remote context 选择、`ocd wrangler` 透明 launcher、安全凭据注入和 dev/staging/production workflow |
| [P13 面向用户的文档站重构](p13-documentation-site-restructure.md) | Day 1 信息架构、内容合同与实施方案完成；按 P11/P12 已实现后的产品形态重写站点，用户优先，新增 Develop/CLI 专章并把架构与贡献内容降到第二优先级 |
...
| [P15 Cloudflare Browser Run](p15-browser-run.md) | Day 1 合同与架构完成；标准 binding/Quick Actions/DevTools/CDP 通过 operator-owned 外部 Browser Provider 执行，受真实 stock-workerd/package/provider G0 阻断；正式 open-compute 发布仍是单个 `ocd` |
