...
全量平台矩阵与相关真实运行时 Gate：确定性用例完整一轮，审查登记的时序用例补两轮；
重复轮使用新进程和隔离数据目录，保留场景内正常运行与重启恢复。新增 P3 目标及其用例分类
尚待实现，本节不是已有可执行命令或已通过报告。
...
8. P3 最终验收（完整一轮、时序三轮）、相关平台回归与完整检查完成，报告包含逐轮实际用例、
   原始结果、失败记录及未遗留进程/资源的证据，不自动重试到通过。
