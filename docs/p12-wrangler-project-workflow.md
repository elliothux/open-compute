...
- 区分本机 `instance` 与远程 `context`，避免用本机 registry 假装远程控制平面；
...
| 开发机远程目标 | per-user `ocd` context registry | API base URL、account ID、deployer credential reference |

三者不能合并。`compute.toml` 不是 Worker 项目清单，`wrangler.jsonc` 不存储 daemon path 或 open-compute token，
context 也不复制 Worker bindings。普通 Worker repository 只需要 `wrangler.jsonc`；只有项目目录本身同时是本机平台运维目录时，
才需要 `compute.toml`。
...
## 3. 概念：instance、context 与 Wrangler environment

| 概念 | 例子 | 选择什么 | 是否可远程 |
| --- | --- | --- | --- |
| instance | `k7m2r` | 本机由某个 canonical config path 派生的 `ocd` 进程 | 否 |
| context | `company-prod` | 一个 API origin + account + credential reference | 是 |
| Wrangler environment | `dev`、`staging`、`production` | 同一 Worker project 内的名称、vars 和 bindings 投影 | 是 |

instance ID 是 P11 的本机运维身份，不发放给远程开发机作为伪集群 ID。context name 是开发者自定义的本机别名，
不进入服务端 identity。Wrangler environment 完全由上游 schema 解析；`ocd` 不重新实现其继承规则。

为避免在不同终端或 repository 中意外把命令发往 production，P12 不提供全局可变的“当前远程 context”。远程目标必须每次
传 `--context <name>`，或固定在项目的 package/CI script 中。本机唯一运行 instance 仍可按 P11 自动选择。

## 4. 命令面

```text
ocd context add <name> --api-base-url <url> --account-id <id> --token-file <path>
ocd context list [--json]
ocd context show <name> [--json]
ocd context test <name> [--json]
ocd context remove <name>

ocd wrangler [--context <name> | --instance <id> | --config <path>] [--project <dir>] <wrangler command> [arguments...]
```

`context add` 的 name 必须匹配 `[a-z][a-z0-9-]{0,31}`。`--context`、`--instance` 和 `--config` 三者互斥。
...
# 远程目标；context 与 Wrangler environment 是两个独立概念
ocd wrangler --context company-prod deploy --env production

# 从项目目录外调用
ocd wrangler --context company-prod --project /srv/workers/billing deploy --env production

# Wrangler 自己的 flag 在 command 后直接透传
ocd wrangler --context dev-server deploy --config ./wrangler.jsonc
```
...
`ocd wrangler` 按以下规则选择且只选择一个 target：

1. 显式 `--context <name>` 选择远程 target；
...
5. 不会从 context registry 隐式选择远程 target。

多个本机 instance 运行且未显式选择时，返回 P11 的稳定歧义错误和完整短 ID 列表；不使用 Worker name、
`wrangler.jsonc` 或端口猜测 instance。本机没有可选 instance 时明确提示 `--context`，不自动发往 Cloudflare。

本机 target 的 API base URL 只从已验证 instance descriptor/config 中的 admin listener 派生，不使用 tenant public
listener、Dashboard URL 或端口扫描。远程 context 同样必须指向运维者明确暴露并保护的 admin API origin。

## 6. 远程 context registry

context registry 是 per-user client configuration，不是 instance registry、`control.sqlite` 或 Worker project 数据。Linux 使用
`$XDG_CONFIG_HOME/open-compute/contexts.toml`（未设置时为 `~/.config/open-compute/contexts.toml`），macOS 使用
`~/Library/Application Support/open-compute/contexts.toml`。它使用 bounded schema、atomic write、fsync 和 owner-only 权限。
...
- `context show/list --json` 不读 token value，也不输出凭据摘要；
- `context remove` 只删除 context record，不删除外部 token file；
- 未知 schema、symlink、宽松权限、重名 context 和重复 `(api_base_url, account_id)` 都 fail closed。

`context test` 是显式网络命令：它读取 deployer token，调用 account discovery 和
...
1. 选择唯一 target；
2. 解析并安全读取该 target 的 deployer token reference；
3. 从本机 descriptor/context 得到 API base URL 和 account ID；
4. 解析 project-local Wrangler executable 并读取其精确版本；
5. 用 target capabilities 验证该 Wrangler 版本是当前 open-compute release 认证的精确 pin；
6. 打印一行不含 secret 的 target kind/name、origin、account 和 Wrangler version 摘要；
...
`CF_EMAIL`，避免另一套凭据或 base URL 参与 Wrangler precedence。它用选定 target 覆盖同名的三个现代变量，
...
`4.127.1` 是本文日期下 P6 认证的 pin；真实 authority 是 target 的 capabilities 与当前 P6 contract，不是本文中
...
```bash
ocd wrangler --context dev-server deploy --env dev
ocd wrangler --context dev-server tail --env dev
```
...
```bash
ocd wrangler --context company-prod versions upload --env production
ocd wrangler --context company-prod versions deploy <version-id>@100% --env production
```
...
1. 锁定并安装 target 认证的 Wrangler pin；
...
object authority、listener 或故障域，必须使用两个 P11 instance/两个远程 context，不能只依赖 `--env`。
...
开发者可以把远程 context 名显式固定在 package script：

```json
{
  "scripts": {
    "dev": "wrangler dev",
    "deploy:dev": "ocd wrangler --context dev-server deploy --env dev",
    "deploy:prod": "ocd wrangler --context company-prod deploy --env production",
    "logs:prod": "ocd wrangler --context company-prod tail --env production"
  }
}
```

context name 不是 secret，可以进入版本控制；context registry 和 token file 不进入 repository。
...
CI secret store 应只发放 deployer token，不使用 admin token。CI 必须锁定 Wrangler package/lockfile，打印非 secret 的 target origin、account、
Wrangler version 和部署结果 ID。`ocd wrangler` 在 CI 中可选；自动化不需为了部署强制创建开发机 context。

## 14. 失败语义

- target 不唯一、context 不存在、token file 不安全或 capabilities 不可达时，不启动 Wrangler；
- Wrangler 缺失或版本不是 target 认证的精确 pin 时，输出检测到的路径/版本与期望版本，不自动修复；
- target 返回 unsupported capability 时保留 Cloudflare-style error，wrapper 不删除 binding 或改写 config 后重试；
- Wrangler 退出后 `ocd` 返回同一 exit status；不把部分成功改写为成功；
- 请求中断不能推断服务端 mutation 已取消；用 Wrangler 列表/查询命令核对 Version、Deployment 和资源状态；
- context 失效不影响远程 daemon 运行，删除项目也不自动 remove context。
...
- support bundle 可列出 context name 和经现有策略清理的 origin，但不读 token file 或采集 Wrangler `.env`/`.dev.vars`；
- `context test` 的 audit 只记录 target name/origin、Wrangler pin 和 request ID；`ocd wrangler` 在 process replacement 前只记录
  target 和 launcher 结果，不记录 opaque argv 全文、环境值或无法观测的 child exit class。

## 16. 代码与文档所有权

- `crates/core`：bounded context name、normalized API base URL 和 target descriptor 值类型；
- `crates/service`：context registry、P11 selector 复用、Wrangler resolution/version check、child environment 与 process replacement；
- P6 capabilities：继续是 Wrangler pin 和 endpoint support 的唯一服务端 authority；
- P7：继续拥有 `wrangler tail` wire/runtime 合同，P12 只启动该命令；
- `packages/docs`：开发、context、CI、staging/production 和回退文档；
...
selector 和错误语义，并保持 context 与 instance registry 两个简单、不可互换的结构。

## 17. 实施顺序

### P12.1：context 与 target

- 实现 context value types、registry 和 add/list/show/test/remove；
...
- context name/URL/account/token-file 的正常与全部 fail-closed 路径；
- context registry 的 symlink、owner、mode、unknown schema、atomic write 和 crash recovery；
- `--context` / `--instance` / `--config` 互斥与 P11 0/1/N 本机选择；
...
- child 环境精确注入三个 Cloudflare target 值并移除冲突 legacy auth/base variables；
...
2. local instance 与 remote context 的选择、互斥、失败和输出合同与 P11/P12 一致；
...
7. CI 可不创建 context，直接使用标准环境变量和同一 Wrangler pin 部署；
8. 部署不重启 daemon，回退不修改 immutable Version，多项目/多环境不串 authority；
9. 中英文项目、context、CI、发布和故障处理文档与 CLI help 同步；
10. 静态检查、覆盖率和最终单轮 workspace Gate 按仓库政策通过，且无遗留 Wrangler/ocd 进程、listener 或 secret。

完成前，文档只能把 `ocd wrangler`、remote context 和上述项目 DX 标为 planned。当前已实现且可依赖的底层路径仍是 P6
