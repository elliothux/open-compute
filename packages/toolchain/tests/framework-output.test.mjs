...
async function fixture(t, wrangler = {}) {
  ...
  await writeFile(
    ...
    "import('./chunks/page-abc.js'); export default {};",
  );
  await writeFile(
    join(root, "dist", "server", "chunks", "page-abc.js"),
    "export const page = 1;",
  );
  await writeFile(
    join(root, "dist", "server", "unused.js"),
    "export const unused = true;",
  );
  await writeFile(
    join(root, "dist", "server", "qualification.txt"),
    "qualification\n",
  );
  await writeFile(
    join(root, "dist", "server", "metadata.json"),
    '{"not":"a module"}\n',
  );
  await writeFile(
  ...
  await writeFile(
    join(root, "dist", "client", "assets", "app-abc.js"),
    "globalThis.client = true;",
  );
  return {
    project: root,
    configPath: ".wrangler/deploy/config.json",
    name: "framework-local",
    vars: {},
    secrets: [],
    bindings: {},
    services: {},
...
test("imports generated server modules and client assets without rebundling or flattening", async (t) => {
  ...
  assert.deepEqual(
    output.worker.modules.map((module) => module.name),
    ["chunks/page-abc.js", "index.js", "qualification.txt", "unused.js"],
  );
  assert.match(
    new TextDecoder().decode(output.worker.modules[1].bytes),
    /chunks\/page-abc\.js/,
  );
...
test("accepts empty and redundant current-default flags without persisting selectors", async (t) => {
  for (const flags of [
    ...
    const project = await fixture(
      t,
      flags === undefined ? {} : { compatibility_flags: flags },
    );
...
test("reconciles provider identities while preserving local services and binding resource IDs", async (t) => {
  ...
  project.services = {
    SELF: {
      service: "local-companion",
      entrypoint: "Api",
      props: { mode: "local", nested: [1, true] },
    },
  };
...
test("rejects binding-shape drift, unsupported generated capabilities, auxiliary Workers, and links", async (t) => {
  ...
  await writeFile(
    join(project.project, ".wrangler", "deploy", "config.json"),
    JSON.stringify({
      configPath: "../../dist/server/wrangler.json",
  ...
  await symlink(
    ...
    join(project.project, "dist", "server", "linked.js"),
  );
