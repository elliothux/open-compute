...
test("bundles TS dependencies and lazy imports while preserving Worker named exports", async (t) => {
  ...
  const worker = await import(
    pathToFileURL(join(outputDirectory, compiled.mainModule)).href
  );
...
test("does not execute Worker code while compiling", async (t) => {
  const options = await project(t, {
    "index.ts":
      'throw new Error("tenant code executed during build"); export default {};',
  });
  const compiled = await compileWorker(options);
...
test("type checking and bundling choose the same workerd package export", async (t) => {
  const options = await project(t, {
    "index.ts":
      'import { kind } from "conditional"; const value: "workerd" = kind; export default value;',
...
test("type errors, excluded entries, and disabled checks cannot bypass validation", async (t) => {
  for (const [source, config] of [
    ...
    [
      ...
      { ...defaultConfig, include: ["other.ts"] },
    ],
    [
      'export const value: number = "wrong";',
      {
        ...defaultConfig,
        compilerOptions: { ...defaultConfig.compilerOptions, noCheck: true },
      },
    ],
    [
    ...
    const options = await project(
      t,
      { "index.ts": source, "other.ts": "export {};" },
      config,
    );
    await assert.rejects(
      compileWorker(options),
      /TypeScript validation|entry.*type.check/,
    );
  }
});
...
test("pinned @types/node compiles default node: builtins without a tenant flag", async (t) => {
  ...
  await writeFile(
    join(options.project, "modules.d.ts"),
    'declare module "cloudflare:sockets" { export function connect(address: string): unknown; }',
  );
  ...
  for (const specifier of [
    ...
    await writeFile(
      join(options.project, "modules.d.ts"),
      `declare module ${JSON.stringify(specifier)} { export const value: string; }`,
    );
...
test("entry and configuration paths cannot escape the selected project", async (t) => {
  ...
  await assert.rejects(
    compileWorker({ ...options, entry: outside.project + "/index.ts" }),
    /inside the project/,
  );
  await assert.rejects(
    compileWorker({ ...options, tsconfig: outside.project + "/tsconfig.json" }),
    /inside the project/,
  );
  await symlink(
    join(outside.project, "index.ts"),
    join(options.project, "linked.ts"),
  );
  await assert.rejects(
    compileWorker({ ...options, entry: "linked.ts" }),
    /inside the project/,
  );
  assert.equal(
    await readFile(join(outside.project, "index.ts"), "utf8"),
    "export default {};",
  );
});
