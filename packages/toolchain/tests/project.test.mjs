...
test("uses pinned Wrangler parsing and projects standard bindings", async (t) => {
  const { filename } = await fixture(t, {
    ...
    d1_databases: [
      { binding: "DB", database_name: "db", database_id: "d1-id" },
    ],
    durable_objects: {
  ...
  assert.deepEqual(Object.keys(project.services.CATALOG.props), [
    "constructor",
    "z",
  ]);
...
test("canonicalizes arbitrary Service props and enforces JSON depth and size", () => {
  const props = canonicalServiceProps(
    JSON.parse('{"z":1,"__proto__":{"ok":true},"a":[null,false]}'),
  );
  assert.deepEqual(Object.keys(props), ["__proto__", "a", "z"]);
  assert.equal(Object.hasOwn(props, "__proto__"), true);
  assert.throws(
    () => canonicalServiceProps({ value: Number.NaN }),
    /only JSON values/,
  );
  assert.throws(
    () => canonicalServiceProps({ value: new Date(0) }),
    /only JSON values/,
  );
  assert.throws(
    () => canonicalServiceProps({ value: "x".repeat(64 * 1024) }),
    /supported size/,
  );
  let nested = true;
...
test("loads the exact requested config when a directory contains multiple Wrangler configs", async (t) => {
  ...
  await writeFile(
    join(directory, ".wrangler", "deploy", "config.json"),
    JSON.stringify({
...
test("projects native Worker Loader declarations from JSONC and TOML", async (t) => {
  ...
  assert.deepEqual(
    (await loadProject(json.filename)).runtimeFeatures.workerLoaders,
    ["FIRST", "SECOND"],
  );
  const toml = await fixture(
    t,
    `name = "dynamic-parent"
main = "src/index.ts"
compatibility_date = "2026-09-08"
...
`,
  ...
  assert.deepEqual(
    (await loadProject(toml.filename)).runtimeFeatures.workerLoaders,
    ["LOADER"],
  );
});

test("rejects malformed, privileged, and duplicate Worker Loader declarations", () => {
  for (const worker_loaders of [
    null,
    {},
    [null],
    [{}],
    [{ binding: 1 }],
    [{ binding: "LOADER", namespace: "guessed" }],
    [{ binding: "LOADER", id: "system" }],
  ...
  assert.throws(() =>
    parseRuntimeFeatures(
      { worker_loaders: [{ binding: "TAKEN" }] },
      new Set(["TAKEN"]),
    ),
  );
  assert.throws(() =>
    parseRuntimeFeatures({
      worker_loaders: [{ binding: "AI" }],
      ai: { binding: "AI" },
    }),
  );
});
