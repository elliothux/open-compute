...
test("scans immutable bytes into sorted encoded paths and parses control files", async (t) => {
  ...
  await writeFile(
    ...
    "/old/:name /new/:name 308\n/rewrite /index.html 200\n",
  );
  const scanned = await scanAssets(root, config);
  assert.deepEqual(
    scanned.manifest.entries.map((entry) => entry.path),
    ["/index.html", "/static/%25%20%5Bname%5D.js"],
  );
  assert.equal(
    scanned.routing.headers[0].operations[0].name,
    "x-frame-options",
  );
  ...
  assert.equal(
    new TextDecoder().decode(await readAssetObject(source)),
    "<main>hello</main>",
  );
});
...
test("refuses symlinks, project roots, forbidden files, and post-scan changes", async (t) => {
  ...
  await assert.rejects(
    scanAssets(root, { ...config, directory: "." }),
    /dedicated directory/,
  );
});
...
test("rule parsers reject malformed lines instead of silently dropping them", () => {
  ...
  assert.throws(
    () => parseRedirects("/from https://example.com 999"),
    /invalid/,
  );
  assert.deepEqual(parseRedirects("/from /to"), [
...
test("ignore, Unicode, well-known paths, source-map policy, CRLF, and quotas are fixed", async (t) => {
  ...
  assert.deepEqual(
    hidden.manifest.entries.map((entry) => entry.path),
    ["/.well-known/security.txt", "/caf%C3%A9.html", "/ignored.keep"],
  );
  assert.equal(hidden.routing.headers[0].operations[0].value, "yes");
  const visible = await scanAssets(root, {
    ...config,
    publishSourceMaps: true,
  });
  assert.ok(
    visible.manifest.entries.some((entry) => entry.path === "/client.js.map"),
  );
