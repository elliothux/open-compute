  ...
  return content
    ...
    .map((line, index) => {
      if (line.length > 2_000)
        throw new Error(`${label} line ${index + 1} exceeds 2000 characters`);
      return line;
    });
}
...
export function parseHeaders(content: string): AssetHeaderRule[] {
  ...
  let current:
    | { pattern: string; operations: { name: string; value: string | null }[] }
    | undefined;
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (!/^\s/.test(raw)) {
      current = {
        pattern: pattern(raw.trim(), "_headers", index + 1),
        operations: [],
      };
      result.push(current);
    ...
    if (!current)
      throw new Error(`_headers line ${index + 1} has no rule pattern`);
    const operation = raw.trim();
    if (operation.startsWith("! ")) {
      const name = operation.slice(2).trim().toLowerCase();
      if (!HEADER_NAME.test(name))
        throw new Error(
          `_headers line ${index + 1} has an invalid header name`,
        );
    ...
    if (separator < 1)
      throw new Error(
        `_headers line ${index + 1} has an invalid header operation`,
      );
  ...
  for (const [index, rule] of result.entries()) {
    if (!rule.operations.length || rule.operations.length > 100) {
      throw new Error(
        `_headers rule ${index + 1} has an invalid operation count`,
      );
    }
    const names = new Set<string>();
    for (const operation of rule.operations) {
      if (names.has(operation.name))
        throw new Error(`_headers rule ${index + 1} repeats a header name`);
