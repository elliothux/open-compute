...
function ignored(path: string, rules: readonly string[]): boolean {
  ...
  for (const rule of rules) {
    ...
    if (expression.test(path) || expression.test(path.split("/").at(-1) ?? ""))
      value = !negative;
  }
  return value;
}
  ...
  try {
    ...
    try {
      ...
      if (!within(root, resolved))
        throw new Error(`${name} escapes the asset directory`);
      return new TextDecoder("utf-8", { fatal: true }).decode(
        await readBounded(file, 256 * 1024, name),
      );
    } finally {
      await file.close();
    }
...
export async function scanAssets(
  ...
  if (!within(project, directory) || directory === project)
    throw new Error(
      "assets.directory must be a dedicated directory inside the project",
    );
  ...
  const walk = async (path: string, logical: string): Promise<void> => {
    ...
    try {
      ...
      if (!within(directory, actual))
        throw new Error("asset directory traversal escaped its root");
      ...
      try {
        for await (const item of listing) {
          ...
          if (FORBIDDEN.test(childLogical))
            throw new Error(
              `asset output contains forbidden path: ${childLogical}`,
            );
          const child = resolve(path, item.name);
          if (item.isSymbolicLink())
            throw new Error(
              `asset output contains a symbolic link: ${childLogical}`,
            );
          if (item.isDirectory()) {
          ...
          if (!item.isFile())
            throw new Error(
              `asset output contains a special file: ${childLogical}`,
            );
          if (CONTROL_FILES.has(childLogical)) continue;
          if (!config.publishSourceMaps && childLogical.endsWith(".map"))
            continue;
          ...
          try {
            ...
            if (total > MAX_TOTAL_BYTES)
              throw new Error("asset output exceeds 512 MiB");
            if (entries.length >= MAX_FILES)
              throw new Error("asset output exceeds 20000 files");
            ...
            entries.push({
              ...
              contentType: contentType(childLogical),
            });
            ...
            } else if (objects.get(sha256)!.size !== bytes.byteLength)
              throw new Error("asset digest declares conflicting lengths");
          } finally {
            await file.close();
          }
        }
        ...
        try {
          ...
          /* `for await` closes an exhausted directory. */
        }
      }
    } finally {
      await handle.close();
  ...
  if (!entries.length)
    throw new Error("asset output contains no publishable files");
  if (
    entries.some(
  ...
  if (Buffer.byteLength(JSON.stringify(manifest)) > MAX_MANIFEST_BYTES)
    throw new Error("asset manifest exceeds 16 MiB");
  const headers = parseHeaders(
    (await readControl(directory, "_headers")) ?? "",
  );
  const redirects = parseRedirects(
    (await readControl(directory, "_redirects")) ?? "",
  );
  return {
    manifest,
    routing: {
...
export async function readAssetObject(
  ...
  try {
    ...
    if (bytes.byteLength !== source.size || digest !== source.sha256)
      throw new Error("asset changed after manifest generation");
    return bytes;
  } finally {
    await file.close();
  }
