...
function generatedBindings(
  ...
  if (
    !record(generatedVars) ||
    JSON.stringify(
      canonicalJson(generatedVars as Record<string, JsonValue>),
  ...
  const add = (name: string, declaration: GeneratedBinding): void => {
    if (declarations.has(name))
      throw new Error("generated framework binding names conflict");
    declarations.set(name, declaration);
  };
  for (const item of generatedArray(
    config.kv_namespaces,
    "generated framework KV bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "id", "preview_id", "remote"],
      "generated framework KV binding",
    );
    add(bindingName(item.binding, "generated framework KV binding name"), {
      type: "kv_namespace",
    });
  }
  for (const item of generatedArray(
    config.r2_buckets,
    "generated framework R2 bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "bucket_name", "preview_bucket_name"],
      "generated framework R2 binding",
    );
    add(bindingName(item.binding, "generated framework R2 binding name"), {
      type: "r2_bucket",
    });
  }
  for (const item of generatedArray(
    config.d1_databases,
    "generated framework D1 bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "database_id", "database_name", "preview_database_id"],
      "generated framework D1 binding",
    );
    add(bindingName(item.binding, "generated framework D1 binding name"), {
      type: "d1_database",
    });
  }
  if (config.durable_objects !== undefined) {
    if (!record(config.durable_objects))
      throw new Error(
        "generated framework Durable Object bindings are invalid",
      );
    onlyKeys(
      config.durable_objects,
      ["bindings"],
      "generated framework Durable Object config",
    );
    for (const item of generatedArray(
      config.durable_objects.bindings,
      "generated framework Durable Object bindings",
    )) {
      onlyKeys(
        item,
        ["name", "class_name", "script_name", "environment"],
        "generated framework Durable Object binding",
      );
      add(
        bindingName(
          item.name,
          "generated framework Durable Object binding name",
        ),
        {
          type: "do_namespace",
          className: className(
            item.class_name,
            "generated framework Durable Object class",
          ),
        },
      );
    }
  ...
  if (config.queues !== undefined) {
    if (!record(config.queues))
      throw new Error("generated framework Queue bindings are invalid");
    onlyKeys(
      config.queues,
      ["producers", "consumers"],
      "generated framework Queue config",
    );
    if (
      generatedArray(
        config.queues.consumers,
        "generated framework Queue consumers",
      ).length
    ) {
    ...
    for (const item of generatedArray(
      ...
      "generated framework Queue producers",
    )) {
      onlyKeys(
        item,
        ["binding", "queue"],
        "generated framework Queue producer",
      );
      add(
        bindingName(item.binding, "generated framework Queue producer binding"),
        { type: "queue_producer" },
      );
    }
  }
  for (const item of generatedArray(
    config.workflows,
    "generated framework Workflow bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "name", "class_name", "script_name"],
      "generated framework Workflow binding",
    );
    add(
      bindingName(item.binding, "generated framework Workflow binding name"),
      {
        type: "workflow",
        className: className(
          item.class_name,
          "generated framework Workflow class",
        ),
      },
    );
  }
  for (const item of generatedArray(
    config.vectorize,
    "generated framework Vectorize bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "index_name", "remote"],
      "generated framework Vectorize binding",
    );
    string(item.index_name, "generated framework Vectorize index name");
    if (item.remote !== undefined && typeof item.remote !== "boolean")
      throw new Error(
        "generated framework Vectorize remote selector is invalid",
      );
    add(
      bindingName(item.binding, "generated framework Vectorize binding name"),
      { type: "vectorize_index" },
    );
  }
  for (const item of generatedArray(
    config.ai_search_namespaces,
    "generated framework AI Search namespace bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "namespace", "remote"],
      "generated framework AI Search namespace binding",
    );
    string(item.namespace, "generated framework AI Search namespace");
    if (item.remote !== undefined && typeof item.remote !== "boolean")
      throw new Error(
        "generated framework AI Search namespace remote selector is invalid",
      );
    add(
      bindingName(
        item.binding,
        "generated framework AI Search namespace binding name",
      ),
  ...
  for (const item of generatedArray(
    config.ai_search,
    "generated framework AI Search instance bindings",
  )) {
    onlyKeys(
      item,
      ["binding", "instance_name", "remote"],
      "generated framework AI Search instance binding",
    );
    string(item.instance_name, "generated framework AI Search instance name");
    if (item.remote !== undefined && typeof item.remote !== "boolean")
      throw new Error(
        "generated framework AI Search instance remote selector is invalid",
      );
    add(
      bindingName(
        item.binding,
...
function emptyGeneratedDeclaration(value: unknown): boolean {
  ...
  if (record(value))
    return Object.values(value).every(emptyGeneratedDeclaration);
  return false;
}
...
function validateGeneratedConfig(config: Record<string, unknown>): void {
  ...
  if (config.no_bundle !== true)
    throw new Error(
      "generated framework config must preserve the prebuilt module graph",
    );
  if (!emptyGeneratedDeclaration(config.triggers))
    throw new Error("generated framework triggers are unsupported");
  if (!emptyGeneratedDeclaration(config.migrations))
    throw new Error("generated framework migrations are unsupported");
  if (
...
function serviceDeclarations(value: unknown): Record<string, WorkerService> {
  ...
  for (const item of value) {
    ...
    onlyKeys(
      item,
      ["binding", "service", "entrypoint", "props"],
      "generated framework service",
    );
    ...
    const entrypoint =
      item.entrypoint === undefined
        ? undefined
        : string(item.entrypoint, "generated framework service entrypoint");
    const props =
      item.props === undefined ? undefined : canonicalServiceProps(item.props);
    if (
      !/^[A-Za-z_][A-Za-z0-9_]{0,63}$/.test(binding) ||
      !/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(service) ||
      (entrypoint !== undefined &&
        !/^[A-Za-z_$][A-Za-z0-9_$]{0,127}$/.test(entrypoint)) ||
...
function reconciledServices(
  value: unknown,
  local: Record<string, WorkerService>,
  ...
  for (const [binding, declaration] of Object.entries(local)) {
    ...
    if (
      expected === undefined ||
      expected.entrypoint !== declaration.entrypoint ||
...
function moduleType(value: unknown): CompiledModuleType {
  switch (value) {
    ...
    case "CompiledWasm":
      return "wasm";
    default:
      throw new Error("framework module rule type is unsupported");
  }
}
...
function moduleRules(value: unknown): ModuleRule[] {
  ...
  if (!Array.isArray(value) || value.length > 32)
    throw new Error("framework module rules are invalid");
...
async function readModule(
  filename: string,
  name: string,
  type: CompiledModuleType,
): Promise<CompiledModule> {
  const file = await open(filename, constants.O_RDONLY | constants.O_NOFOLLOW);
  try {
    const info = await file.stat();
    if (!info.isFile() || info.size > MAX_MODULE_BYTES)
      throw new Error("framework module exceeds 4 MiB");
    const bytes = Buffer.alloc(info.size);
    let offset = 0;
    while (offset < bytes.length) {
      const { bytesRead } = await file.read(
    ...
    if (
      ...
      try {
        ...
        throw new Error("framework text module is not UTF-8");
      }
    }
    if (type === "json") {
      try {
        ...
        throw new Error("framework JSON module is invalid");
      }
    }
...
async function importModules(
    ...
    try {
      for await (const item of listing) {
        ...
        if (
          ...
          (filename === assetsDirectory || within(assetsDirectory, filename))
        )
          continue;
        const logical = relative(root, filename).split(sep).join("/");
        if (item.isSymbolicLink())
          throw new Error("framework output contains a symbolic link");
        if (item.isDirectory()) {
          await walk(filename);
          continue;
        }
        if (!item.isFile())
          throw new Error("framework output contains a special file");
        const type = classify(logical, rules);
        if (type === undefined) continue;
        if (modules.length >= MAX_MODULES)
          throw new Error("framework output exceeds 128 modules");
        const module = await readModule(filename, logical, type);
        total += module.bytes.byteLength;
        if (total > MAX_TOTAL_MODULE_BYTES)
          throw new Error("framework output exceeds 16 MiB of modules");
        modules.push(module);
      }
    } finally {
      try {
        ...
        /* Exhausted async iterators close themselves. */
      }
    }
  };
  await walk(root);
  ...
  if (
    !modules.some(
      (module) => module.name === mainModule && module.type === "esModule",
    )
  ) {
...
function routing(value: unknown, directory: string): AssetsProject {
  if (!record(value))
    throw new Error("generated framework assets config is invalid");
  onlyKeys(
    value,
    [
      "directory",
...
export async function importFrameworkOutput(
  project: WorkerProject,
): Promise<FrameworkOutput> {
  if (project.frameworkOutput === undefined)
    throw new Error("project has no frameworkOutput");
  const projectRoot = await realpath(project.project);
  const redirectPath = await outputPath(
    projectRoot,
    projectRoot,
    project.frameworkOutput,
    "frameworkOutput",
  );
  ...
  onlyKeys(
    ...
    "framework deploy config",
  );
  if (
    redirect.auxiliaryWorkers !== undefined &&
    (!Array.isArray(redirect.auxiliaryWorkers) ||
      redirect.auxiliaryWorkers.length !== 0)
  ) {
    throw new Error("framework auxiliary Workers are unsupported");
  }
  const configPath = await outputPath(
    projectRoot,
    dirname(redirectPath),
    redirect.configPath,
    "generated framework configPath",
  );
  ...
  const generatedRuntimeFeatures = hasGeneratedRuntimeFeatures
    ? parseRuntimeFeatures(
        config,
        new Set([
          ...Object.keys(project.vars),
          ...project.secrets,
          ...Object.keys(project.bindings),
          ...Object.keys(services),
          ...(project.assets?.binding === undefined
            ? []
  ...
  if (generatedRuntimeFeatures !== undefined) {
    const projectExplicit =
      project.runtimeFeatures.workerLoaders.length > 0 ||
      project.runtimeFeatures.cache.enabled ||
      Object.keys(project.runtimeFeatures.cache.entrypoints).length > 0 ||
      project.runtimeFeatures.images !== undefined ||
      project.runtimeFeatures.ai !== undefined ||
      project.runtimeFeatures.versionMetadata !== undefined;
    if (
      projectExplicit &&
      JSON.stringify(generatedRuntimeFeatures) !==
        JSON.stringify(project.runtimeFeatures)
    ) {
      throw new Error(
        "generated framework runtime features conflict with the project",
      );
    }
  }
  const outputRoot = await realpath(dirname(configPath));
  const main = await outputPath(
    projectRoot,
    outputRoot,
    config.main,
    "generated framework main",
  );
  let assets: AssetsProject | undefined;
  let assetsDirectory: string | undefined;
  if (config.assets !== undefined) {
    if (!record(config.assets))
      throw new Error("generated framework assets config is invalid");
    assetsDirectory = await outputPath(
      projectRoot,
      outputRoot,
      config.assets.directory,
      "generated framework assets directory",
    );
    if (within(assetsDirectory, main))
      throw new Error(
        "framework server entry is inside the client asset output",
      );
    const relativeAssets = relative(projectRoot, assetsDirectory);
    assets = routing(config.assets, relativeAssets.split(sep).join("/"));
  }
  const worker = await importModules(
    outputRoot,
    main,
    assetsDirectory,
    moduleRules(config.rules),
  );
  return {
    worker,
...
export function applyFrameworkOutput(
  project: WorkerProject,
