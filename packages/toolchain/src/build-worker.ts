...
export type CompiledModuleType =
  "esModule" | "commonJsModule" | "text" | "json" | "data" | "wasm";
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        ...
        if (error) {
          ...
          reject(
            new Error(
              `Worker TypeScript validation failed\n${diagnostics}\n${stderr}`.trim(),
              { cause: error },
            ),
          );
        } else if (
          !lines.some((line) => isAbsolute(line) && resolve(line) === entry)
        ) {
...
export async function compileWorker(
  ...
  for (const chunk of output.output) {
    if (chunk.type !== "chunk")
      throw new Error("unsupported emitted Worker resource");
    if (chunk.isEntry) {
      if (mainModule !== undefined)
        throw new Error("Worker must have one entry module");
  ...
  if (mainModule === undefined)
    throw new Error("Worker build did not emit an entry module");
