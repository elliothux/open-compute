...
function isSessionIssueResponse(value: unknown): value is SessionIssueResponse {
  if (value === null || typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
  return typeof record.session_token === "string"
...
export async function exchangeLoginCode(code: string): Promise<SessionIssueResponse> {
  const response = await fetch(new URL("/operator/session/exchange", window.location.origin), {
    method: "POST",
...
export async function mintSessionFromAdmin(adminToken: string): Promise<SessionIssueResponse> {
  const response = await fetch(new URL("/operator/session", window.location.origin), {
    method: "POST",
    credentials: "same-origin",
    headers: {
      "content-type": "application/json",
