...
test("derives its contract digest and operation catalog from the root OpenAPI authority", async () => {
  ...
  const declared = Object.entries(contract.paths)
...
test("reuses the official Cloudflare transport for extension operations", async () => {
  ...
  assert.equal(
    requests[0].url,
    "https://compute.example/client/v4/open-compute/capabilities",
  );
  assert.equal(
    ...
    fetch: async (url, init) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          errors: [],
          messages: [],
          result: { state: "paused", pending: 0, running: 0 },
        }),
        { headers: { "content-type": "application/json" } },
      );
    },
      ...
      return new Response(
        JSON.stringify({
          ...
          result: {
            ...
            name: "restored-db",
            kind: "d1_database",
            created_on: "2026-09-03T00:00:00Z",
          },
        }),
        { headers: { "content-type": "application/json" } },
      );
...
test("preserves official APIError and retry behavior", async () => {
    ...
    fetch: async () => {
      ...
      return new Response(
        JSON.stringify({
          ...
          errors: [],
          messages: [],
          result: { state: "ready", version: "dev", components: [] },
        }),
        { headers: { "content-type": "application/json" } },
      );
    },
  ...
  assert.equal(
    (await createOpenComputeExtension(retrying).system.status()).state,
    "ready",
  );
  assert.equal(attempts, 2);

  const failing = retrying.withOptions({
    maxRetries: 0,
    fetch: async () =>
      new Response(
        JSON.stringify({
          success: false,
          result: null,
          errors: [{ code: 9100000, message: "denied" }],
          messages: [],
        }),
  ...
  await assert.rejects(
    createOpenComputeExtension(failing).capabilities.get(),
    APIError,
  );
});

test("rejects path segments that URL normalization could consume", () => {
  const extension = createOpenComputeExtension(
    new Cloudflare({ apiToken: "test", maxRetries: 0 }),
  );
  for (const value of ["", ".", ".."]) {
    assert.throws(
      () => extension.workers.endpoints(value, "worker"),
      /invalid extension path segment/,
    );
    assert.throws(
      () => extension.workers.endpoints("account", value),
      /invalid extension path segment/,
    );
  }
});
