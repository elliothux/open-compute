...
test("typed socket authority preserves IPv6 record and string forms across private hops", () => {
  const record = socketAuthorityWire({
  ...
  assert.deepEqual(socketAddressFromWire(record, "2606:4700:4700::1111:443"), {
    hostname: "2606:4700:4700::1111",
    port: 443,
  });
...
test("private socket authority rejects confused, malformed, and extended wire values", () => {
  ...
  for (const value of [
    ...
    assert.throws(
      () => validateSocketAuthorityWire(value),
      /SOCKET_TUNNEL_INVALID/,
    );
  }
});
...
test("inbound CONNECT authority recovers workerd's unbracketed IPv6 form", async () => {
  assert.deepEqual(
    await inboundSocketTargetAddress({
      opened: Promise.resolve({ localAddress: "2606:4700:4700::1111:443" }),
    }),
    { hostname: "2606:4700:4700::1111", port: 443 },
  );
  assert.equal(
    await inboundSocketTargetAddress({
      opened: Promise.resolve({ localAddress: "example.com:443" }),
    }),
    "example.com:443",
  );
});
