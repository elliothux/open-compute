...
test("latest Vectorize surface normalizes pinned options and emits the bounded mutation frame", async () => {
    ...
    async call(operation, payload) {
      ...
      if (operation === "describe")
        return {
          vectorCount: 1,
          dimensions: 2,
          processedUpToDatetime: 3,
          processedUpToMutation: 4,
        };
      if (operation === "query" || operation === "queryById")
        return {
          matches: [{ id: "one", score: 0.9, values: [1, 2] }],
          count: 1,
        };
  ...
  assert.equal(
    (
      await binding.query(new Float64Array([1 / 3, 2]), {
        topK: 50,
        returnMetadata: true,
      })
    ).count,
...
test("Vectorize rejects unsupported options, invalid ranges, limits, and malformed backend success", async () => {
  ...
  const binding = new VectorizeBinding({
    ...
    async mutate() {
      return { mutationId: "ok" };
    },
  });
  await assert.rejects(
    binding.query([1], { topK: 51, returnValues: true }),
    /VECTORIZE_LIMIT_EXCEEDED/,
  );
  await assert.rejects(
    binding.query([1], { filter: {} }),
    /VECTORIZE_INPUT_INVALID/,
  );
  await assert.rejects(
    binding.query([1], { filter: { year: { $gt: 1, $gte: 2 } } }),
    /VECTORIZE_INPUT_INVALID/,
  );
