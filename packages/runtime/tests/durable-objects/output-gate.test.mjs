...
function sqlStorage() {
  ...
  const exec = (query, ...params) => {
    ...
    if (text.includes("UPDATE")) {
      ...
      const row = (tables.get("output") ?? []).find(
    ...
    if (text.includes("DELETE")) {
      ...
      tables.set(
        "output",
        (tables.get("output") ?? []).filter((row) => row.id !== id),
      );
      return {
        toArray() {
          return [];
...
test("awaited transaction mutation stages immediately and commit publishes once", async () => {
  ...
  await runWithOutputGate(gate, async () => {
    ...
    const first = await gate.schedule(
      "queue",
      "EVENTS",
      new Uint8Array([1]),
      async () => {
    ...
    const second = await gate.schedule(
      "queue",
      "EVENTS",
      new Uint8Array([2]),
      async () => {
...
test("older recovered intent blocks a newer committed closure", async () => {
  ...
  await assert.rejects(
    gate.exitTransaction("committed"),
    /DO_OUTPUT_GATE_RECOVERY_REQUIRED/,
  );
  assert.deepEqual(published, []);
  assert.equal(storage._tables.get("output")[0].attempt_count, 1);
  assert.equal(
    storage._tables.get("output")[0].last_error,
    "DO_OUTPUT_GATE_RECOVERY_REQUIRED",
  );
});
...
test("recover publishes remaining committed intents through the named publisher", async () => {
  ...
  const gate = new DoOutputGate(storage);
  await gate
    .schedule("queue", "EVENTS", new Uint8Array([9, 9]), async () => {
    ...
    gate.schedule(
      ...
      async (operationId) => {
        ...
        throw Object.assign(new Error("unavailable"), {
          stableCode: "QUEUE_STORAGE_UNAVAILABLE",
        });
      },
    ),
