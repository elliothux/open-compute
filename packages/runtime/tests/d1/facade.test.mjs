...
test("compiled D1 facade normalizes values and returns Cloudflare result shapes", async () => {
  ...
  assert.deepEqual(await db.exec("create table entries(value)"), {
    count: 1,
    duration: 0,
  });
...
test("compiled D1 bind follows the pinned workerd conversions and errors", async () => {
  ...
  await statement
    .bind(Number.NaN, Number.POSITIVE_INFINITY, Number.MAX_SAFE_INTEGER + 1)
    .all();
  assert.deepEqual(calls.at(-1).statements[0].params, [
    null,
    null,
    9007199254740992,
  ]);
...
test("compiled D1 raw and first preserve the pinned wrapper's permissive call behavior", async () => {
  ...
  assert.deepEqual(await prepared.raw({ columnNames: true }), [
    ["value", "value"],
    [1, 2],
  ]);
});
...
test("compiled D1 session trims constraints and observes opaque bookmarks", async () => {
  ...
  const db = new D1Database({
    async query(mode, statements, session) {
      if (session?.kind === 3 && session.bookmark === "future") {
        throw Object.assign(new Error("D1_SESSION_ERROR"), {
          stableCode: "D1_SESSION_ERROR",
        });
      }
      version += 1;
  ...
  await assert.rejects(
    db.withSession("future").prepare("select 1").all(),
    /D1_ERROR: D1_SESSION_ERROR/,
  );
});
  ...
  for (const sql of ["", "   "]) {
    await assert.rejects(
      ...
      (error) =>
        ...
        error.cause?.message ===
          "Error in line 1: : No SQL statements detected.",
    );
  }
  await assert.rejects(
    db.dump(),
...
test("compiled D1 facade maps private failures but rejects malformed results without exposing them", async () => {
  const failed = new D1Database({
    async query() {
      throw Object.assign(new Error("D1_LIMIT_ERROR"), {
        stableCode: "D1_LIMIT_ERROR",
      });
    },
    async exec() {
      throw Object.assign(new Error("D1_SQL_INVALID"), {
        stableCode: "D1_SQL_INVALID",
      });
    },
  });
  await assert.rejects(
    failed.prepare("select 1").all(),
    /D1_ERROR: D1_LIMIT_ERROR/,
  );
  await assert.rejects(
    failed.exec("select 1"),
    /D1_EXEC_ERROR: D1_SQL_INVALID/,
  );
  ...
  for (const malformed of [
    ...
    await assert.rejects(db.prepare("select 1").all(), {
      name: "TypeError",
      message: "D1_INTERNAL_PROTOCOL_ERROR",
    });
  }
});
