...
test("Images binding preserves official synchronous chaining and result response shape", async () => {
  ...
  const binding = new ImagesBinding({
    ...
    async info() {
      return { format: "png", fileSize: 4, width: 2, height: 2 };
    },
    async transform(session, options) {
      calls.push(["transform", session, options]);
    },
    async draw(session, input, options) {
      calls.push(["draw", session, input, options]);
    },
  ...
  const overlay = new ReadableStream({
    start(controller) {
      controller.close();
    },
  });
...
test("Images facade rejects unsupported public options before transport execution", async () => {
  ...
  assert.throws(
    () => binding.input(source()).transform({ sharpen: 1 }),
    /IMAGE_OPTION_UNSUPPORTED/,
  );
  assert.throws(
    () => binding.input(source(), { encoding: "base64" }),
    /IMAGE_OPTION_UNSUPPORTED/,
  );
  assert.throws(
    () => binding.input(source()).draw(source(), { right: 1 }),
    /IMAGE_OPTION_UNSUPPORTED/,
  );
  await assert.rejects(
    binding.input(source()).output({ format: "image/gif" }),
    /IMAGE_OPTION_UNSUPPORTED/,
  );
  assert.equal(
    calls,
    3,
    "only input session creation starts before chain validation",
  );
});
...
test("Images facade rejects malformed info and output protocol values", async () => {
  ...
  const malformedInfo = new ImagesBinding({
    ...
    async info() {
      return { format: "png", fileSize: -1, width: 2, height: 2 };
    },
    async transform() {},
    async draw() {},
    async output() {
  ...
  const wrongOutput = new ImagesBinding({
    ...
    async info() {
      return { format: "png", fileSize: 1, width: 1, height: 1 };
    },
    async transform() {},
    async draw() {},
    async output() {
