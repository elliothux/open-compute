...
const { durationMs, timestampMs } = await importRuntime(
  "workflows/duration.ts",
);

test("duration grammar and rounding match shared Rust fixtures", () => {
  const fixtures = JSON.parse(
    readFileSync(
      new URL("../fixtures/workflow-duration.json", import.meta.url),
      "utf8",
    ),
  );
  for (const { input, maximum, expected } of fixtures) {
    if (expected === null)
      assert.throws(
        () => durationMs(input, maximum),
        /WORKFLOW_DURATION_INVALID/,
        JSON.stringify(input),
      );
    else
      assert.equal(durationMs(input, maximum), expected, JSON.stringify(input));
  }
});
...
test("absolute timestamps stay integral and preserve past dates", () => {
  for (const value of [0, -1, 9007199254740991, -9007199254740991])
    assert.equal(timestampMs(value), value);
