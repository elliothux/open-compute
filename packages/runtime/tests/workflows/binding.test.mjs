...
test("durable facade resolves once and keeps only a private instance-scoped handle", async () => {
  ...
  assert.throws(
    () => instance.sendEvent({ type: "bad type" }),
  ...
  assert.throws(
    () => instance.sendEvent({ type: "x", payload: "x".repeat(1024 * 1024) }),
    /WORKFLOW_PAYLOAD_TOO_LARGE/,
  );
  const created = await binding.create({
    id: "fresh",
    retention: { successRetention: "1 hour" },
  });
  ...
  assert.deepEqual(calls.at(-1), [
    "create",
    { id: generated.id, payloadBase64: encodeWorkflowBase64(null) },
  ]);
  assert.deepEqual(await binding.deleteBatch(["order", "order"]), {
    deleted: [{ id: "order" }, { id: "order" }],
    errors: [],
  });
});
...
test("direct Workflow schedules use one deterministic instance and operation per logical slot", async () => {
  ...
  await assert.rejects(
    triggerWorkflowSchedule(binding, {
      cron: "",
