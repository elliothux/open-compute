...
globalThis.__openComputeCacheEnv = {
  __OPEN_COMPUTE_PRIVATE_CACHE: { default: transport },
};
...
test("automatic cache reports miss, hit, SWR refresh, SIE, bypass, and purge", async () => {
  ...
  const miss = await runtime.dispatch(
    () => {
      ...
      return new Response("origin", {
        headers: { "cache-control": "max-age=60", "cache-tag": "gate" },
      });
    },
  ...
  assert.deepEqual(calls.at(-1).at(-1), {
    status: "MISS",
    fenceGeneration: "3",
  });
  ...
  transport.lookup = {
    ...
    response: new Response("fallback", {
      headers: { "cf-cache-status": "STALE_IF_ERROR" },
    }),
  };
  ...
  const bypass = await runtime.dispatch(
    () =>
  ...
  assert.deepEqual(await runtime.context.purge({ tags: ["release"] }), {
    success: true,
    deleted: 2,
  });
});
...
test("automatic cache serves cached ranges and only bypasses an uncached partial origin response", async () => {
  ...
  const ctx = {
    waitUntil() {
      throw new Error("range response must not schedule a store");
    },
  };
  const runtime = automaticRuntime(true);
...
test("automatic cache availability follows the operator fail-open policy", async () => {
  ...
  transport.match = async () => {
    throw Object.assign(new Error("CACHE_UNAVAILABLE"), {
  ...
  try {
    ...
    assert.equal(bypass.headers.get("cf-cache-status"), "BYPASS");
    await assert.rejects(
      automaticRuntime(false).dispatch(
        () => new Response("origin"),
        request,
        ctx,
      ),
...
test("automatic cache never fail-opens protocol or integrity failures", async () => {
  ...
  try {
    for (const code of ["CACHE_PROTOCOL_ERROR", "CACHE_CORRUPT"]) {
      transport.match = async () => {
        throw Object.assign(new Error(code), { stableCode: code });
      };
      await assert.rejects(
    ...
    const hostile = {};
    Object.defineProperty(hostile, "stableCode", {
      get() {
  ...
  try {
    for (const lookup of [
      { status: "HIT", fenceGeneration: "8" },
      {
        status: "MISS",
        fenceGeneration: "8",
        response: new Response("unexpected"),
      },
