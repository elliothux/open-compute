...
const { QueueProducer } = await import(
  moduleUrl(
    await compileRuntime("queues/facade.ts", {
...
test("compiled Queue producer encodes v8 and keeps json/text/bytes exact", async () => {
  const calls = [];
  const queue = new QueueProducer(
    transport(async (operation, frame) => {
      calls.push({ operation, frame });
      return {
        backlogCount: 1,
        backlogBytes: frame?.byteLength ?? 0,
  ...
  await queue.sendBatch([{ body: "a", contentType: "text" }, { body: 1 }], {
    delaySeconds: 3,
  });
...
test("compiled Queue producer rejects unsupported, oversized, delayed, and detached values", async () => {
  ...
  await assert.rejects(queue.send("x", { contentType: "xml" }), {
    name: "TypeError",
    message: "QUEUE_CONTENT_TYPE_UNSUPPORTED",
  });
  await assert.rejects(queue.send(undefined), {
    name: "TypeError",
    message: "QUEUE_INVALID_MESSAGE",
  });
  await assert.rejects(queue.send("x", { delaySeconds: -1 }), {
    name: "Error",
    message: "QUEUE_DELAY_INVALID",
  });
  await assert.rejects(queue.send("x", { delaySeconds: 86_401 }), {
    name: "Error",
    message: "QUEUE_DELAY_INVALID",
  });
  await assert.rejects(
    queue.send(new Uint8Array(128_001), { contentType: "bytes" }),
    {
      name: "TypeError",
      message: "QUEUE_MESSAGE_TOO_LARGE",
    },
  );
  await assert.rejects(queue.sendBatch([]), {
    name: "TypeError",
    message: "QUEUE_INVALID_MESSAGE",
  });
  await assert.rejects(
    queue.sendBatch(Array.from({ length: 101 }, () => ({ body: "x" }))),
    {
      name: "Error",
      message: "QUEUE_BATCH_LIMIT_EXCEEDED",
    },
  );
  await assert.rejects(queue.send(Promise.resolve(1), { contentType: "v8" }), {
    name: "TypeError",
    message: "QUEUE_V8_UNSUPPORTED",
  });
  ...
  const doQueue = new QueueProducer(
    transport(async () => ({ backlogCount: 0, backlogBytes: 0 })),
    true,
  );
  await assert.rejects(doQueue.send("x"), {
    name: "TypeError",
...
test("awaited Durable Object Queue send stages, commits, finalizes, and removes its intent", async () => {
  ...
  const raw = {
    ...
    async finalize(operationId) {
      operations.push(["finalize", operationId]);
    },
    async metrics() {
