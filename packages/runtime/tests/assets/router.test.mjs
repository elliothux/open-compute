...
test("default router selects one path without invoking a speculative asset fetch", () => {
  assert.equal(
    routeDefaultHttp(snapshot(), request("/static/app.js")),
    "asset",
  );
  assert.equal(routeDefaultHttp(snapshot(), request("/api/data")), "worker");
  assert.equal(
    routeDefaultHttp(
      snapshot({ contentKind: "assets_only" }),
      request("/api/data"),
    ),
    "asset",
  );
  assert.equal(
    routeDefaultHttp(snapshot(), request("/static/app.js", { method: "POST" })),
    "worker",
  );
});
...
test("worker-first exclusions, pinned navigation, redirects, and encodings are fixed", () => {
  ...
  assert.equal(
    routeDefaultHttp(
      snapshot(),
      request("/missing", { headers: { "sec-fetch-mode": "navigate" } }),
    ),
    "asset",
  );
  ...
  redirected.assets.routing.redirects = [
    { from: "/old/:name", to: "/new/:name", status: 308 },
  ];
  assert.equal(routeDefaultHttp(redirected, request("/old/page")), "asset");
  assert.equal(
    routeDefaultHttp(snapshot(), request("/static%2Fapp.js")),
    "worker",
  );
});
