...
test("AI binding preserves direct and handle single/array overloads", async () => {
  ...
  assert.deepEqual(
    (await binding.toMarkdown().supported()).map((item) => item.extension),
    [".docx", ".pdf"],
  );
});
...
test("AI binding validates documents, options, limits, and backend response fail closed", async () => {
  ...
  for (const options of [
    ...
    await assert.rejects(
      binding.toMarkdown(file, options),
      /AI_OPTION_UNSUPPORTED/,
    );
  await assert.rejects(
    binding.toMarkdown({ name: "bad\0name", blob: file.blob }),
    /AI_DOCUMENT_INVALID/,
  );
  await assert.rejects(
    binding.toMarkdown({ name: "../manual.pdf", blob: file.blob }),
    /AI_DOCUMENT_INVALID/,
  );
  await assert.rejects(
    binding.toMarkdown({ name: "manual.pdf", blob: new Blob(["pdf"]) }),
    /AI_DOCUMENT_INVALID/,
  );
  await assert.rejects(
    binding.toMarkdown({ name: "empty", blob: new Blob([]) }),
    /AI_DOCUMENT_INVALID/,
  );
  await assert.rejects(
    binding.toMarkdown({
  ...
  await assert.rejects(
    binding.toMarkdown(Array.from({ length: 17 }, () => file)),
    /AI_BATCH_TOO_LARGE/,
  );
