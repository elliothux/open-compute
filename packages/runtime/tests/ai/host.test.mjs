...
`);
const { AiTransport } = await import(
  moduleUrl(
    await compileRuntime("ai/host.ts", {
...
test("AI transport sends only the private authorized Markdown Conversion wire contract", async () => {
    ...
    BINDING_BACKEND: {
      async fetch(url, init) {
        ...
        return Response.json({
          schemaVersion: 1,
          result: url.endsWith("/supported") ? [] : [{ ok: true }],
        });
      },
    },
  };
  ...
  assert.equal(
    calls[0].init.headers["x-open-compute-binding-token"],
    "private-token",
  );
  assert.equal(
    calls[0].init.headers["x-open-compute-descriptor-sha256"],
    props.descriptorSha256,
  );
  assert.equal(
    calls[0].init.headers["x-open-compute-startup-generation"],
    "generation",
  );
});

test("AI transport maps backend and malformed envelope failures to stable codes", async () => {
  const failed = new AiTransport(
    { props },
    {
      ...
      BINDING_BACKEND: {
        fetch: async () =>
          new Response(null, {
            status: 422,
            headers: { "x-open-compute-error-code": "DOCUMENT_ENCRYPTED" },
          }),
      },
    },
  );
  await assert.rejects(
    failed.supported(),
    (error) => error.stableCode === "DOCUMENT_ENCRYPTED",
  );
  const malformed = new AiTransport(
    { props },
    {
      BINDING_BACKEND_TOKEN: "token",
  ...
  await assert.rejects(
    malformed.supported(),
    (error) => error.stableCode === "AI_PROTOCOL_ERROR",
  );
});
