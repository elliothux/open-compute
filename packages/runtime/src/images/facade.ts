...
interface TransportDrawOptions {
  left?: number;
  top?: number;
  opacity?: number;
  repeat?: false;
...
interface TransportOutputOptions {
...
function stream(value: unknown): ReadableStream<Uint8Array> {
  if (!(value instanceof ReadableStream))
    throw new TypeError("IMAGE_INPUT_INVALID");
...
function transformOptions(value: unknown): TransportTransformOptions {
  ...
  return {
    ...
    ...(options.fit === undefined
      ? {}
      : { fit: options.fit as NonNullable<TransformOptions["fit"]> }),
    ...
    ...(flip === undefined
      ...
      : {
          flip:
            flip === "h" ? "horizontal" : flip === "v" ? "vertical" : "both",
        }),
...
function drawOptions(value: unknown): TransportDrawOptions {
  ...
  if (
    ...
    !["normal", "over"].includes(String(options.composite))
  ) {
    throw new TypeError("IMAGE_OPTION_UNSUPPORTED");
  }
  return {
    ...
    ...(options.composite === undefined
      ? {}
      : {
          blend: options.composite as NonNullable<
            TransportDrawOptions["blend"]
          >,
        }),
  };
}
  ...
  async info(
    ...
    if (
      Object.keys(info).some(
        (key) => !["format", "fileSize", "width", "height"].includes(key),
      ) ||
