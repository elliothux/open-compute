...
export class ImageTransport extends WorkerEntrypoint<
  BindingEnv,
  ImageTransportProps
> {
  #headers() {
  ...
  async input(body: ReadableStream<Uint8Array>): Promise<string> {
    const response = await this.#fetch("/internal/images/v1/input", {
      method: "POST",
      body,
    });
  ...
  async info(body: ReadableStream<Uint8Array>): Promise<unknown> {
    const response = await this.#fetch("/internal/images/v1/info", {
      method: "POST",
      body,
    });
    await expectBindingStatus(response, 200, "IMAGE_PROTOCOL_ERROR");
  ...
  async transform(session: string, options: unknown): Promise<void> {
    const response = await this.#fetch(
      `/internal/images/v1/session/${encodeURIComponent(session)}/transform`,
      {
        method: "POST",
  ...
  async draw(
    ...
    const response = await this.#fetch(
      `/internal/images/v1/session/${encodeURIComponent(session)}/draw`,
      {
        method: "POST",
        headers: {
          "content-type": "application/vnd.open-compute.image.v1+frame",
        },
  ...
  async output(session: string, options: unknown): Promise<Response> {
    const response = await this.#fetch(
      `/internal/images/v1/session/${encodeURIComponent(session)}/output`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(options),
