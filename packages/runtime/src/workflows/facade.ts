...
function createWire(value: unknown): WorkflowCreateWire {
  ...
  const id =
    value.id === undefined ? crypto.randomUUID() : instanceId(value.id);
  if (value.retention !== undefined)
    fields(value.retention, ["successRetention", "errorRetention"]);
  if (
    value.locationHint !== undefined &&
    (typeof value.locationHint !== "string" ||
      !LOCATION_HINTS.has(value.locationHint))
  ) {
    throw workflowError("WORKFLOW_METHOD_UNSUPPORTED");
  }
  return {
    ...(id === undefined ? {} : { id }),
    payloadBase64: encodeWorkflowBase64(
      value.params,
...
function restartOptions(value: unknown): {
  from?: {
    name: string;
    count?: number;
  ...
  if (
    value.from.count !== undefined &&
    (typeof value.from.count !== "number" ||
      !Number.isSafeInteger(value.from.count) ||
      value.from.count < 1 ||
      value.from.count > 1024)
  ) {
    throw workflowError("WORKFLOW_METHOD_UNSUPPORTED");
  }
  if (
    value.from.type !== undefined &&
    !["do", "sleep", "waitForEvent"].includes(String(value.from.type))
  ) {
    throw workflowError("WORKFLOW_METHOD_UNSUPPORTED");
  }
  return {
    from: {
      ...
      ...(value.from.type === undefined
        ? {}
        : { type: value.from.type as "do" | "sleep" | "waitForEvent" }),
    },
  };
}
...
type WorkflowIntent =
  ...
  | {
      op: "restart";
      instanceId: string;
      options: ReturnType<typeof restartOptions>;
    }
  | {
      op: "sendEvent";
      instanceId: string;
      event: { type: string; payloadBase64: string };
    };
...
class WorkflowInstance {
  ...
  #mutate(
    ...
    run: (id: string) => Promise<unknown> | unknown,
  ) {
    return this.#binding.dispatchIntent(intent, run, () => undefined);
  }
  pause(...args: unknown[]) {
    noArguments(args);
    return this.#mutate({ op: "pause", instanceId: this.id }, (id) =>
      this.#handle.pause(id),
    );
  }
  resume(...args: unknown[]) {
    noArguments(args);
    return this.#mutate({ op: "resume", instanceId: this.id }, (id) =>
      this.#handle.resume(id),
    );
  }
  ...
  delete(...args: unknown[]) {
    noArguments(args);
    return this.#mutate({ op: "delete", instanceId: this.id }, (id) =>
      this.#handle.delete(id),
    );
  }
}
...
export class WorkflowBinding {
  ...
  async #triggerSchedule(schedule: { cron: string; scheduledTime: number }) {
    ...
    const digest = new Uint8Array(
      await crypto.subtle.digest(
        ...
        new TextEncoder().encode(
          `${this.#name}\0${schedule.cron}\0${schedule.scheduledTime}`,
        ),
      ),
    );
    ...
    const operation = Array.from(uuid, (byte) =>
      byte.toString(16).padStart(2, "0"),
    ).join("");
  ...
  dispatchIntent(
    ...
    return gate.schedule<unknown>(
      "workflow",
      this.#name,
      encodeWorkflowValue(intent),
      (id) => Promise.resolve(run(id)),
      staged,
    );
  }
  ...
  async [FLUSH_OUTPUT](payload: Uint8Array, id: string) {
    ...
    if (intent.op === "create") {
      return new WorkflowInstance(
        await this.#transport.create(intent.create, id),
        this,
      );
    }
    if (intent.op === "createBatch") {
      return (await this.#transport.createBatch(intent.creates, id)).map(
        (result) => new WorkflowInstance(result, this),
      );
    }
    if (intent.op === "deleteBatch")
      return this.#transport.deleteBatch(intent.instanceIds, id);
    ...
    if (intent.op === "terminate")
      return resolved.handle.terminate(intent.options, id);
    if (intent.op === "restart")
      return resolved.handle.restart(intent.options, id);
    if (intent.op === "delete") return resolved.handle.delete(id);
    if (intent.op === "sendEvent")
      return resolved.handle.sendEvent(intent.event, id);
    throw workflowError("WORKFLOW_INVARIANT_VIOLATION");
  }
  ...
  deleteBatch(ids: unknown) {
    ...
    return this.dispatchIntent({ op: "deleteBatch", instanceIds }, (id) =>
      this.#transport.deleteBatch(instanceIds, id),
    );
  }
}
...
function rawTransport(raw: unknown): raw is WorkflowTransport {
  return (
    ...
    "deleteBatch" in raw &&
    typeof raw.deleteBatch === "function"
  );
}
