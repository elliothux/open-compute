...
export interface WorkflowProtocolError {
  errorCode: string;
  state?: never;
  steps?: never;
  ok?: never;
}
export interface WorkflowSuspended {
  state: "suspended";
  errorCode?: never;
  steps?: never;
  ok?: never;
...
export interface WorkflowClaimDeclaration extends WorkflowDeclaration {
  batchFirstOrdinal: number;
  batchSize: number;
}
...
export type WorkflowBatchReply =
  | WorkflowProtocolError
  | WorkflowSuspended
  | { steps: WorkflowBatchStep[]; state?: never; errorCode?: never };
export type WorkflowDrainReply =
  | WorkflowProtocolError
  | WorkflowSuspended
  | { ok: true; state?: never; errorCode?: never };
