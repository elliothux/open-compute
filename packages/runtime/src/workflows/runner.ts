...
function isCallback(value: unknown): value is WorkflowCallback {
...
function doConfig(value: unknown): {
  config: unknown;
  dynamicDelay: WorkflowDelayFunction | undefined;
} {
  if (
    ...
    Object.keys(value).some(
      (key) => !["retries", "timeout", "sensitive"].includes(key),
    )
  ) {
  ...
  if (retries !== undefined) {
    if (
      !object(retries) ||
      Object.keys(retries).some(
        (key) => !["limit", "delay", "backoff"].includes(key),
      )
    ) {
      throw workflowError("WORKFLOW_STEP_CONFIG_UNSUPPORTED");
    }
    if (typeof retries.delay === "function") {
      ...
      return {
        config: { ...value, retries: normalizedRetries },
        dynamicDelay: retries.delay as WorkflowDelayFunction,
      };
    }
  }
...
function rollbackOptions(value: unknown):
  ...
  if (
    !object(value) ||
    Object.keys(value).some(
      (key) => !["rollback", "rollbackConfig"].includes(key),
    ) ||
...
export function validateWorkflowClass(
  target: unknown,
): target is WorkflowClass {
  return (
...
export async function runWorkflow(
  ...
  const rpc = async <T extends { errorCode?: string | undefined }>(
    operation: () => Promise<T>,
  ): Promise<Exclude<T, WorkflowProtocolError>> => {
    let reply: T;
    try {
  ...
  const report = async (
    operation: () => Promise<WorkflowVerdict>,
  ): Promise<void> => {
    ...
    if (reply?.errorCode && reply.errorCode !== "WORKFLOW_STEP_STALE")
      reject(reply.errorCode);
  };
  const value = (
    reply: Exclude<WorkflowVerdict, WorkflowProtocolError>,
  ): unknown => {
    ...
    if (reply.state === "failed") {
      ...
      const error =
        reply.code === "WORKFLOW_NON_RETRYABLE"
          ? new NonRetryableError("Workflow step is not retryable")
          : workflowError(reply.code);
    ...
    try {
      return reply.outputBase64 === undefined
        ? undefined
        : decodeWorkflowBase64(reply.outputBase64);
    } catch {
      unknown = true;
      throw workflowError("WORKFLOW_RUNTIME_UNAVAILABLE");
    }
  ...
  const evaluateDynamicDelay = async (
    item: PendingStep,
    attempt: number,
    config: {
      retries: {
    ...
    try {
      const returned = await Promise.race([
        item.dynamicDelay(
          Object.freeze({ ctx: callbackContext(item, attempt, config), error }),
        ),
        new Promise<never>((_resolve, rejectDelay) => {
          timer = setTimeout(
            () =>
              rejectDelay(workflowError("WORKFLOW_STEP_CONFIG_UNSUPPORTED")),
            5_000,
          );
        }),
      ]);
  ...
  const resolveDynamicDelay = async (
    item: PendingStep,
    grant: {
      state: "resolve_delay";
      attempt: number;
  ...
  const descriptor = (
    ...
    return {
      ...
      nameCount,
      config,
      ...(rollbackConfig === undefined ? {} : { rollbackConfig }),
      rollbackStep: internal,
        ...
        batch.map(async (item, index) => {
          ...
          if (grant.state === "resolve_delay") {
            ...
            return {
              ok: true as const,
              value: value(
                verdict as Exclude<WorkflowVerdict, WorkflowProtocolError>,
              ),
            };
          }
          if (grant.state === "run") {
            ...
            const callback = (async () => {
              ...
              try {
                ...
                if (
                  ...
                  try {
                    if (error instanceof Error) callbackError = error;
                  } catch {
                    /* hostile proxy */
                  }
                  try {
              ...
              let outputBase64;
              try {
                outputBase64 = encodeWorkflowBase64(result);
              } catch (error) {
              ...
              await report(() =>
                backend.success({ ordinal: indexOrdinal, outputBase64 }),
              );
            })();
            callback.catch(() => {});
          }
          try {
            ...
            if (reply.state === "resolve_delay")
              reply = await resolveDynamicDelay(item, reply);
            ...
            if (item.descriptor.kind === "wait_event") {
              ...
              let payload;
              try {
                payload = decodeWorkflowBase64(reply.payloadBase64);
              } catch {
                unknown = true;
                throw workflowError("WORKFLOW_RUNTIME_UNAVAILABLE");
              }
              return {
                ...
                value: Object.freeze({
                  type: reply.type,
                  payload,
                  timestamp: new Date(reply.timestampMs),
                }),
              };
            }
            if (reply.state === "event") {
            ...
            if (item.descriptor.kind === "do") {
              const replay =
                grant.state === "complete" && grant.config !== undefined
                  ? grant.config
                  : replayConfig(item.descriptor.config);
              const attempt =
                grant.state === "complete" && grant.attempt !== undefined
                  ? grant.attempt
                  : 1;
  ...
  const enqueue = (
    ...
    dynamicDelay?: WorkflowDelayFunction,
    ...
    const base = new Promise<unknown>((yes, no) => {
      resolve = yes;
      rejectResult = no;
    });
    base.catch(() => {});
  ...
  const step = Object.freeze({
    do(
      name: string,
      config: unknown,
      callbackOrRollback?: WorkflowCallback | unknown,
      rollbackArgument?: unknown,
      ...extra: unknown[]
    ): Promise<unknown> {
      ...
      if (isCallback(config)) {
        ...
        callback = isCallback(callbackOrRollback)
          ? callbackOrRollback
          : undefined;
      ...
      try {
        ...
        reject("WORKFLOW_STEP_CONFIG_UNSUPPORTED");
      }
      return enqueue(
        "do",
        name,
    ...
    sleepUntil(name: string, timestamp: unknown, ...extra: unknown[]) {
      if (extra.length) reject("WORKFLOW_STEP_CONFIG_UNSUPPORTED");
      return enqueue("sleep_until", name, {
  ...
  const executeRollbacks = async (): Promise<void> => {
    ...
    frontier = [];
    const registrations = [...rollbacks.values()].sort(
  ...
  try {
    ...
    const result = await instance.run(
      Object.freeze({
        payload: decodeWorkflowBase64(event.payloadBase64),
        ...
        ...(event.schedule === undefined
          ? {}
          : { schedule: Object.freeze({ ...event.schedule }) }),
      }),
      step,
    );
    flush();
  ...
  return suspended
    ...
    : failure
      ? { outcome: "errored", errorCode: failure, finalOrdinal: ordinal }
      : {
          outcome: "complete",
