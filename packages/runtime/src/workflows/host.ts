...
function assertActivation(
  ...
  if (
    ...
    !Number.isSafeInteger(value.instanceGeneration) ||
  ...
  if (value.schedule !== undefined) {
    if (
      ...
      typeof value.schedule.cron !== "string" ||
      value.schedule.cron.length < 1 ||
      value.schedule.cron.length > 256 ||
      typeof value.schedule.scheduledTime !== "number" ||
      !Number.isSafeInteger(value.schedule.scheduledTime) ||
      value.schedule.scheduledTime < 0
    ) {
      throw new Error("invalid activation");
    }
  }
...
export async function handleWorkflow(
  ...
  try {
    ...
    if (
      !/^[A-Za-z_$][A-Za-z0-9_$]{0,127}$/.test(className) ||
    ...
    const target = validation
      ? loaded.getEntrypoint<LoadedWorkflow>("__OpenComputeWorkflow")
      : observedEntrypoint<LoadedWorkflow>(
          loaded,
          env.WORKER_LOADER_FACTORY,
          ctx,
          snapshot.observability,
          "__OpenComputeWorkflow",
        );
    ...
    if (typeof body.activationBudgetMs !== "number")
      throw new Error("invalid activation budget");
    const backend = new WorkflowRunController(
      env,
      {
    ...
    if (
      ...
      code === "VERSION_INVARIANT_VIOLATION"
    ) {
