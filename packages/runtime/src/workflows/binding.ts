...
export function readWorkflowStatus(value: unknown): WorkflowStatus {
  ...
  if (value.outputBase64 !== undefined)
    result.output = decodeWorkflowBase64(value.outputBase64);
...
class WorkflowInstanceTransport extends RpcTarget implements WorkflowHandle {
  ...
  pause(operationId: string) {
    return this.#request("pause", {}, operationId);
  }
  resume(operationId: string) {
    return this.#request("resume", {}, operationId);
  }
  ...
  delete(operationId: string) {
    return this.#request("delete", {}, operationId);
  }
  sendEvent(
...
class WorkflowPendingInstanceTransport
  extends RpcTarget
  implements WorkflowHandle
{
  #env: BindingEnv;
  #props: WorkflowBindingProps;
  #externalId: string;
  ...
  async #handle() {
    this.#resolved ??= request(this.#env, this.#props, "get", {
      id: this.#externalId,
    }).then((result) => {
      if (typeof result.instanceId !== "string")
        throw bindingError("WORKFLOW_RUNTIME_UNAVAILABLE");
      return new WorkflowInstanceTransport(
        this.#env,
        this.#props,
        result.instanceId,
      );
  ...
  async pause(id: string) {
    return (await this.#handle()).pause(id);
  }
  async resume(id: string) {
    return (await this.#handle()).resume(id);
  }
  async terminate(options: { rollback?: boolean }, id: string) {
    return (await this.#handle()).terminate(options, id);
  }
  async restart(
    options: {
      from?: {
  ...
  async delete(id: string) {
    return (await this.#handle()).delete(id);
  }
...
> {
  #resolved(result: Record<string, unknown>) {
    ...
    return {
      id: result.id,
      handle: new WorkflowInstanceTransport(
        this.env,
        this.ctx.props,
        result.instanceId,
      ),
  ...
  async #resolve(operation: string, body: object, operationId?: string) {
    ...
    return this.#resolved(
      await request(this.env, props, operation, body, operationId),
    );
  }
  resolve(id: string) {
    return {
      id,
      handle: new WorkflowPendingInstanceTransport(
        this.env,
        this.ctx.props,
        id,
      ),
  ...
  create(body: WorkflowCreateWire, operationId: string) {
    return this.#resolve("create", body, operationId);
  }
  get(id: string) {
  ...
  async createBatch(body: WorkflowCreateWire[], operationId: string) {
    ...
    if (
      ...
      result.instances.length !== body.length
    ) {
      throw bindingError("WORKFLOW_RUNTIME_UNAVAILABLE");
    }
