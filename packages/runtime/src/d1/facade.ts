...
interface StatementState extends D1StatementDto {
  owner: object;
}
...
function publicError(
  prefix: "D1_ERROR" | "D1_EXEC_ERROR" | "D1_TYPE_ERROR",
  message: string,
): never {
  ...
  if (
    ...
    for (const key of ["stableCode", "message"]) {
      ...
      if (
        ...
        /^[A-Z][A-Z0-9_]{1,127}$/.test(descriptor.value)
      )
...
function queryError(error: unknown): never {
  ...
  if (
    ...
    error.message === "D1_INTERNAL_PROTOCOL_ERROR"
  )
    throw error;
...
function execError(error: unknown): never {
  if (error instanceof Error && error.message.startsWith("D1_EXEC_ERROR:"))
    throw error;
...
function arrayLikeView(
  value: ArrayBufferView,
): value is ArrayBufferView & ArrayLike<unknown> {
...
function normalizeValue(value: unknown): D1Value {
  ...
  if (Array.isArray(value)) {
    if (
      value.every((item) => typeof item === "number" && item >= 0 && item < 256)
    ) {
  ...
  if (ArrayBuffer.isView(value)) {
    ...
    if (
      items.every(
        (item): item is number =>
          typeof item === "number" && item >= 0 && item < 256,
      )
    ) {
...
function statement(
  ...
  statementState.set(
    result,
    Object.freeze({ owner, sql, params: Object.freeze(params) }),
  );
  return result;
}
...
function objectRow(
  ...
  values: QueryResult["rows"][number],
): Record<string, null | string | number | number[]> {
...
function sessionWire(owner: object): D1SessionWire {
  ...
  if (session.observedBookmark)
    return { kind: 3, bookmark: session.observedBookmark };
...
function observeBookmark(
  ...
  if (
    session.observedVersion === null ||
    stateVersion > session.observedVersion
  ) {
...
async function terminal(
  ...
  try {
    ...
    if (state.sql.trim().length === 0)
      publicError("D1_ERROR", "No SQL statements detected.");
    const session = sessionState.has(state.owner);
    const response = await transportFor(state.owner).query(
      mode,
      [dto(state)],
      sessionWire(state.owner),
    );
...
function batchOwner(owner: object, values: unknown): StatementState[] {
  if (!Array.isArray(values)) {
    publicError(
      "D1_ERROR",
      "Malformed input: expected an array of prepared statements",
    );
  }
  if (values.length === 0)
    publicError("D1_ERROR", "No SQL statements detected.");
...
async function runBatch(owner: object, values: unknown) {
  try {
    ...
    const response = await transportFor(owner).query(
      "batch",
      states.map(dto),
      sessionWire(owner),
    );
    const decoded = assertQueryResponse(response, session);
    if (decoded.results.length !== states.length)
      typeError("D1_INTERNAL_PROTOCOL_ERROR");
...
export class D1DatabaseSession {
  ...
  getBookmark() {
    ...
    return (
      state.observedBookmark ??
      (state.constraint === "bookmark" ? state.pendingBookmark : null)
    );
  }
}
...
export class D1Database {
  ...
  async dump(): Promise<ArrayBuffer> {
    if (!databaseState.has(this)) typeError("D1_INTERNAL_PROTOCOL_ERROR");
    throw new Error("D1_DUMP_ERROR: Status + 400", {
      cause: new Error("Status 400"),
    });
  }
}
