  ...
  switch (reader.u8()) {
    ...
    case 2: {
      ...
      if (!Number.isFinite(value))
        throw reader.bindingError("D1_INTERNAL_PROTOCOL_ERROR");
      return value;
    }
    case 3:
      return reader.text();
    case 4:
      return Uint8Array.from(reader.bytesValue());
    default:
      throw reader.bindingError("D1_INTERNAL_PROTOCOL_ERROR");
  }
}
...
function encodeQuery(
  ...
  session: D1SessionWire = { kind: 0 },
) {
  const modes = { all: 1, run: 2, raw: 3, batch: 4 };
  if (
    statements.length > 0xffff ||
    statements.some((statement) => statement.params.length > 0xffff)
  ) {
...
function decodeQuery(buffer: ArrayBuffer, bindingError: BindingError) {
  ...
  if (decoder.decode(reader.take(4)) !== "D1R1")
    throw bindingError("D1_INTERNAL_PROTOCOL_ERROR");
  ...
  for (let resultIndex = 0; resultIndex < count; resultIndex++) {
    ...
    for (let index = 0; index < columnCount; index++)
      columns.push(reader.text());
    const rowCount = reader.u32();
    const rows = [];
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
      const row = [];
      for (let index = 0; index < columnCount; index++)
        row.push(readValue(reader));
      rows.push(row);
    }
  ...
  return {
    results,
    bookmark: bookmarkText.length === 0 ? null : bookmarkText,
    stateVersion,
  };
}

export function makeD1TransportBase(
  bindingError: BindingError,
  ...
  return class extends WorkerEntrypoint<BindingEnv, ResourceBindingProps> {
    ...
    async query(
      ...
      const response = await this.#request(
        "query",
        encodeQuery(mode, statements, session),
        "query",
        FRAME_CONTENT_TYPE,
      );
    ...
    async exec(sql: string): Promise<unknown> {
      ...
      if (
        !response.headers.get("content-type")?.startsWith(JSON_CONTENT_TYPE)
      ) {
        throw bindingError("D1_INTERNAL_PROTOCOL_ERROR");
      }
