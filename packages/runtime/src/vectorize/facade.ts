interface VectorizeTransport {
  ...
  mutate(
    ...
    frame: ReadableStream<Uint8Array>,
  ): Promise<unknown>;
}
...
function fail(code = "VECTORIZE_INPUT_INVALID"): never {
...
function metadata(value: unknown): Record<string, VectorizeVectorMetadata> {
  ...
  const result = Object.fromEntries(
    Object.entries(value).map(([key, item]) => [key, metadataValue(item)]),
  );
  if (encoder.encode(JSON.stringify(result)).byteLength > METADATA_BYTES)
    fail("VECTORIZE_LIMIT_EXCEEDED");
...
function vector(value: unknown): VectorizeVector {
  ...
  return {
    ...
    values: vectorValues(item.values),
...
function filter(value: unknown): VectorizeVectorMetadataFilter {
  ...
  for (const [field, raw] of Object.entries(value)) {
    ...
    if (
      ...
      output[field] =
        raw === null ? null : typeof raw === "number" ? finiteNumber(raw) : raw;
      continue;
    }
    ...
    if (
      entries.length === 2 &&
      !(
        names.some((name) => name === "$gt" || name === "$gte") &&
        names.some((name) => name === "$lt" || name === "$lte")
      )
    )
      fail();
  ...
  if (encoder.encode(JSON.stringify(output)).byteLength > 2048)
    fail("VECTORIZE_LIMIT_EXCEEDED");
...
function queryOptions(value: unknown): VectorizeQueryOptions {
  ...
  if (
    ...
    fail("VECTORIZE_LIMIT_EXCEEDED");
  if (
    options.returnValues !== undefined &&
    typeof options.returnValues !== "boolean"
  )
    fail();
  if (
    options.returnMetadata !== undefined &&
    typeof options.returnMetadata !== "boolean" &&
    !["all", "indexed", "none"].includes(String(options.returnMetadata))
  )
    fail();
  if (
    (options.returnValues === true ||
      options.returnMetadata === true ||
      options.returnMetadata === "all") &&
    (topK as number) > 50
  )
    fail("VECTORIZE_LIMIT_EXCEEDED");
  return {
    ...
    ...(options.namespace === undefined
      ? {}
      : { namespace: boundedString(options.namespace) }),
    ...(options.returnValues === undefined
      ? {}
      : { returnValues: options.returnValues as boolean }),
    ...(options.returnMetadata === undefined
      ? {}
      : {
          returnMetadata:
            options.returnMetadata === true
              ...
              : options.returnMetadata === false
                ? "none"
                : (options.returnMetadata as VectorizeMetadataRetrievalLevel),
        }),
...
function mutationFrame(
  ...
  const number = (bytes: number, value: number): void => {
    ...
    if (bytes === 2) view.setUint16(0, value, false);
    else view.setUint32(0, value, false);
    push(raw);
  };
  const header = encoder.encode(
    JSON.stringify({ operation, schemaVersion: 1 }),
  );
  push(encoder.encode("OCVZ"));
  number(2, 1);
  number(4, header.byteLength);
  push(header);
  number(4, vectors.length);
  for (const item of vectors) {
    const id = encoder.encode(item.id);
    number(2, id.byteLength);
    push(id);
    const namespace =
      item.namespace === undefined ? undefined : encoder.encode(item.namespace);
    number(2, namespace?.byteLength ?? 0xffff);
    if (namespace) push(namespace);
    const meta =
      item.metadata === undefined
        ? undefined
        : encoder.encode(JSON.stringify(item.metadata));
    number(4, meta?.byteLength ?? 0xffffffff);
    if (meta) push(meta);
    const values = vectorValues(item.values);
    number(2, values.length);
...
export class VectorizeBinding {
  ...
  async describe(): Promise<VectorizeIndexInfo> {
    return described(await this.#transport.call("describe", {}));
  }
  async query(
    input: VectorFloatArray | number[],
    options?: VectorizeQueryOptions,
  ): Promise<VectorizeMatches> {
    return matches(
      await this.#transport.call("query", {
        vector: vectorValues(input),
  ...
  async queryById(
    ...
    return matches(
      await this.#transport.call("queryById", {
        vectorId: boundedString(id),
        options: queryOptions(options),
      }),
    );
  }
  async insert(vectors: VectorizeVector[]): Promise<VectorizeAsyncMutation> {
    const value = batch(vectors);
    return mutation(
      await this.#transport.mutate("insert", mutationFrame("insert", value)),
    );
  }
  async upsert(vectors: VectorizeVector[]): Promise<VectorizeAsyncMutation> {
    const value = batch(vectors);
    return mutation(
      await this.#transport.mutate("upsert", mutationFrame("upsert", value)),
    );
  }
  async deleteByIds(value: string[]): Promise<VectorizeAsyncMutation> {
    return mutation(
      await this.#transport.call("deleteByIds", { ids: ids(value) }),
    );
  }
