...
export class VectorizeTransport extends WorkerEntrypoint<
  BindingEnv,
...
> {
  ...
  call(operation: string, payload: unknown): Promise<unknown> {
    if (
      !["describe", "query", "queryById", "deleteByIds", "getByIds"].includes(
        operation,
      )
    )
      throw bindingError("VECTORIZE_PROTOCOL_ERROR");
    return this.#response(
      "/internal/vectorize/v1/call",
  ...
  mutate(
    ...
    if (
      ...
      !(frame instanceof ReadableStream)
    )
      throw bindingError("VECTORIZE_PROTOCOL_ERROR");
    return this.#response(
      `/internal/vectorize/v1/mutate/${operation}`,
      frame,
      "application/vnd.open-compute.vectorize.v1+frame",
    );
  }
}
