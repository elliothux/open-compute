...
export function socketAuthorityWire(
...
export function validateSocketAuthorityWire(
  value: unknown,
): SocketAuthorityWire {
  ...
  if (
    ...
    item.address.length <= 1_024 &&
    !CONTROL.test(item.address)
  ) {
...
export function socketAddressFromWire(
  ...
  if (
    ...
    authorityText(authority) !== observedAuthority
  ) {
    throw failure("SOCKET_TUNNEL_INVALID");
  }
  return authority.kind === "string"
...
export async function inboundSocketAddress(socket: Socket): Promise<string> {
  ...
  if (
    typeof info.localAddress !== "string" ||
    info.localAddress.length < 3 ||
    info.localAddress.length > 1024 ||
    /[\x00-\x1f\x7f]/.test(info.localAddress)
  ) {
    throw failure("SOCKET_TUNNEL_INVALID");
  }
...
export async function inboundSocketTargetAddress(
  socket: Socket,
): Promise<SocketAddress | string> {
...
export async function tunnelSockets(
  left: Socket,
  right: Socket,
): Promise<void> {
