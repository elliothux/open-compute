...
function assertMetadata(
  value: unknown,
  bindingError: BindingError,
): asserts value is R2Metadata {
  if (
    !isRecord(value) ||
    typeof value.key !== "string" ||
    typeof value.etag !== "string" ||
    typeof value.httpEtag !== "string" ||
    ...
    (value.storageClass !== "Standard" &&
      value.storageClass !== "InfrequentAccess") ||
  ...
  if (value.httpMetadata != null) {
    if (!isRecord(value.httpMetadata))
      throw bindingError("BINDING_PROTOCOL_ERROR");
  ...
  if (value.customMetadata != null) {
    if (
      ...
      Object.values(value.customMetadata).some(
  ...
  if (value.range != null) {
    if (
      ...
      Object.entries(value.range).some(
        ([key, item]) =>
...
function framedBody(
  ...
  if (!(stream instanceof ReadableStream))
    throw bindingError("R2_INVALID_OPTIONS");
  const encoded = new TextEncoder().encode(JSON.stringify(header));
  if (encoded.byteLength > MAX_METADATA_BYTES)
    throw bindingError("R2_METADATA_TOO_LARGE");
...
async function decodeFrame(
  response: Response,
  bindingError: BindingError,
): Promise<{ meta: R2Metadata; body?: ReadableStream<Uint8Array> }> {
  ...
  const frame: unknown = JSON.parse(
    new TextDecoder().decode(await exact(metadataLength)),
  );
...
export function makeR2TransportBase(
  bindingError: BindingError,
  currentStartupGeneration: () => string,
  tokenHeader: string,
) {
  return class extends WorkerEntrypoint<BindingEnv, ResourceBindingProps> {
    ...
    async get(key: string, options: R2GetOptions) {
      ...
      return response.status === 204
        ? null
        : decodeFrame(response, bindingError);
    }
    ...
    async put(
      ...
      const response = await this.#request(
        "put",
        framedBody({ key, options }, body, bindingError),
        "write",
        FRAME_CONTENT_TYPE,
      );
      if (response.status === 204) return null;
    ...
    async list(options: R2ListOptions): Promise<R2ListResult> {
      ...
      if (
        ...
        (result.truncated
          ...
          : result.cursor != null) ||
        !stringArray(result.delimitedPrefixes)
      )
        throw bindingError("BINDING_PROTOCOL_ERROR");
    ...
    async uploadPart(
      ...
      if (
        !isRecord(result) ||
        typeof result.partNumber !== "number" ||
