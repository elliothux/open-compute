...
class Encoder {
  ...
  u32(value: number): void {
    this.writer.u32(value, () => this.tooLarge());
  }
  f64(value: number): void {
    this.writer.f64(value, () => this.tooLarge());
  }
  raw(value: Uint8Array): void {
    this.writer.bytesOf(value, () => this.tooLarge());
  }
  ...
  value(value: unknown, depth: number): void {
    switch (typeof value) {
      case "undefined":
        this.u8(TAG.UNDEFINED);
        return;
      case "boolean":
        this.u8(value ? TAG.TRUE : TAG.FALSE);
        return;
      case "number":
        this.u8(TAG.NUMBER);
        this.f64(value);
  ...
  object(value: object, depth: number): void {
    ...
    if (prototype === ObjectPrototype) {
      this.plain(value, TAG.OBJECT, depth);
      return;
    }
    if (prototype === null) {
      this.plain(value, TAG.NULL_OBJECT, depth);
      return;
    }
    if (prototype === DateCtor.prototype) {
      this.date(value);
      return;
    }
    if (prototype === RegExpCtor.prototype) {
      this.regexp(value);
      return;
    }
    if (prototype === MapCtor.prototype) {
      this.map(value as Map<unknown, unknown>, depth);
      return;
    }
    if (prototype === SetCtor.prototype) {
      this.set(value as Set<unknown>, depth);
      return;
    }
    if (prototype === ArrayBufferCtor.prototype) {
      this.arrayBuffer(value as ArrayBuffer);
      return;
    }
    if (prototype === DataView.prototype) {
      this.dataView(value as DataView, depth);
      return;
    }
    const typed = typedArrayIndex(value);
    if (typed !== undefined) {
      this.typedArray(value as ArrayBufferView, typed, depth);
      return;
    }
    const error = errorKindOf(value);
    if (error !== undefined) {
      this.error(value, error, depth);
      return;
    }
    this.unsupported();
  }
  array(value: unknown[], depth: number): void {
    ...
    for (const key of extras) {
      const descriptor = dataDescriptor(value, key);
      if (descriptor === undefined || descriptor === "accessor")
        this.unsupported();
      this.string(key);
      this.value(descriptor.value, depth + 1);
    }
  ...
  dataView(value: DataView, depth: number): void {
    ...
    if (!(buffer instanceof ArrayBufferCtor) || unsafeBuffer(buffer))
      this.unsupported();
  ...
  error(value: object, kind: number, depth: number): void {
    ...
    const message =
      nativeString(value, "message", () => this.unsupported()) ?? "";
    const cause = dataDescriptor(value, "cause");
    const errors =
      kind === ERROR_KIND.AGGREGATE
        ? dataDescriptor(value, "errors")
        : undefined;
    const code =
      kind === ERROR_KIND.DOM ? dataDescriptor(value, "code") : undefined;
    if (cause === "accessor" || errors === "accessor" || code === "accessor")
      this.unsupported();
    let flags = 0;
    if (cause !== undefined) flags |= ERROR_CAUSE;
    if (errors !== undefined) flags |= ERROR_ERRORS;
    if (code !== undefined) {
      if (typeof code.value !== "number" || !Number.isSafeInteger(code.value))
        this.unsupported();
      flags |= ERROR_CODE;
    }
    this.u8(TAG.ERROR);
...
function ownString(
  ...
  if (descriptor === "accessor" || typeof descriptor.value !== "string")
    unsupported();
