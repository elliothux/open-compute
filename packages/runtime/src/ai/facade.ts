...
function parseOptions(value: unknown): WireConversionOptions {
  ...
  if (request.gateway !== undefined || request.extraHeaders !== undefined)
    throw new TypeError("AI_OPTION_UNSUPPORTED");
  if (request.conversionOptions === undefined) return {};
  const conversion = exact(request.conversionOptions, [
    "output",
    "html",
    "docx",
    "image",
  ...
  if (conversion.html !== undefined) {
    ...
    if (html.hostname !== undefined) {
      if (
        typeof html.hostname !== "string" ||
        html.hostname.length > 2048 ||
        /[\u0000-\u001f\u007f]/.test(html.hostname)
      )
        throw new TypeError("AI_OPTION_UNSUPPORTED");
      let base: URL;
    ...
    if (
      html.cssSelector !== undefined &&
      (typeof html.cssSelector !== "string" ||
        !html.cssSelector.length ||
        html.cssSelector.length > 512 ||
        /[\u0000-\u001f\u007f{}@\\]/.test(html.cssSelector) ||
        html.cssSelector.split(",").length > 16)
    )
      throw new TypeError("AI_OPTION_UNSUPPORTED");
    result.html = {
      ...(html.hostname === undefined
        ? {}
        : { hostname: html.hostname as string }),
      ...(html.cssSelector === undefined
        ? {}
        : { cssSelector: html.cssSelector as string }),
    };
  }
  if (conversion.pdf !== undefined) {
    const pdf = exact(conversion.pdf, ["images", "metadata"]);
    if (
      pdf.images !== undefined ||
...
async function wireDocument(value: unknown): Promise<WireDocument> {
  ...
  if (
    ...
    file.blob.size > MAX_FILE_BYTES ||
    file.blob.size < 1 ||
    file.blob.type.length > 128 ||
    !/^[A-Za-z0-9!#$&^_.+-]+\/[A-Za-z0-9!#$&^_.+-]+$/.test(file.blob.type)
  ) {
    throw new TypeError(
      file.blob instanceof Blob && file.blob.size > MAX_FILE_BYTES
...
function response(value: unknown, expectedName: string): ConversionResponse {
  ...
  if (value.format === "error") {
    if (
      Object.keys(value).some(
        (key) => !["id", "name", "mimeType", "format", "error"].includes(key),
      ) ||
  ...
  if (
    ...
    Object.keys(value).some(
      (key) =>
        !["id", "name", "mimeType", "format", "tokens", "data"].includes(key),
    ) ||
...
class ToMarkdownService {
  ...
  async transform(
    ...
    if (input.length > MAX_BATCH_FILES)
      throw new TypeError("AI_BATCH_TOO_LARGE");
    let bytes = 0;
    ...
    const result = raw.map((item, index) =>
      response(item, documents[index]!.name),
    );
  ...
  async supported(): Promise<SupportedFileFormat[]> {
    ...
    for (const value of raw) {
      if (
        !record(value) ||
        Object.keys(value).some(
          (key) => !["extension", "mimeType"].includes(key),
        ) ||
        typeof value.extension !== "string" ||
        !/^\.[a-z0-9]{1,16}$/.test(value.extension) ||
        typeof value.mimeType !== "string" ||
        !/^[A-Za-z0-9!#$&^_.+-]+\/[A-Za-z0-9!#$&^_.+-]+$/.test(value.mimeType)
      ) {
        throw new TypeError("AI_PROTOCOL_ERROR");
      }
