...
function invalid(): never {
  throw Object.assign(new Error("VERSION_INVARIANT_VIOLATION"), {
    stableCode: "VERSION_INVARIANT_VIOLATION",
  });
}
...
export function assertSnapshot(
  ...
  if (
    ...
    typeof value.cachePolicy.enabled !== "boolean" ||
    typeof value.cachePolicy.failOpen !== "boolean" ||
    typeof value.cachePolicy.crossVersionCache !== "boolean" ||
    !record(value.cachePolicy.entrypoints)
  )
    invalid();
  for (const policy of Object.values(
    value.cachePolicy.entrypoints as Record<string, unknown>,
  )) {
    if (
      !record(policy) ||
      typeof policy.enabled !== "boolean" ||
      typeof policy.crossVersionCache !== "boolean"
    )
      invalid();
  }
  ...
  if (value.observability !== undefined) {
    ...
    if (
      ...
      (item.deploymentId !== undefined &&
        typeof item.deploymentId !== "string") ||
      typeof item.routeGeneration !== "number" ||
      !Number.isSafeInteger(item.routeGeneration) ||
      item.routeGeneration < 1 ||
      typeof item.observabilityGeneration !== "number" ||
      !Number.isSafeInteger(item.observabilityGeneration) ||
      item.observabilityGeneration < 1 ||
      typeof item.enabled !== "boolean" ||
      typeof item.logsEnabled !== "boolean" ||
      typeof item.headSamplingRate !== "number" ||
      !Number.isFinite(item.headSamplingRate) ||
      item.headSamplingRate < 0 ||
      item.headSamplingRate > 1 ||
      typeof item.invocationLogs !== "boolean" ||
  ...
  if (
    ...
    (!record(value.imagesBinding) ||
      ...
      typeof value.imagesBinding.descriptorSha256 !== "string")
  )
    invalid();
  if (
    value.aiBinding !== undefined &&
    (!record(value.aiBinding) ||
      typeof value.aiBinding.name !== "string" ||
      typeof value.aiBinding.descriptorSha256 !== "string")
  )
    invalid();
  if (
    value.versionMetadataBinding !== undefined &&
    (!record(value.versionMetadataBinding) ||
      typeof value.versionMetadataBinding.name !== "string" ||
      ...
      (value.versionMetadataBinding.tag !== undefined &&
        typeof value.versionMetadataBinding.tag !== "string"))
  )
    invalid();
  if (
    value.assetBinding !== undefined &&
    (!record(value.assetBinding) || typeof value.assetBinding.name !== "string")
  )
    invalid();
  if (value.assets !== undefined) {
    if (
      !record(value.assets) ||
      !record(value.assets.manifest) ||
      !record(value.assets.routing) ||
      value.assets.manifest.schemaVersion !== 1 ||
      !Array.isArray(value.assets.manifest.entries) ||
      value.assets.routing.schemaVersion !== 1
    )
      invalid();
  ...
  if (value.contentKind === "assets_only" && value.assets === undefined)
    invalid();
  ...
  for (const binding of value.bindings as unknown[]) {
    if (
      ...
      typeof binding.capabilityVersion !== "number" ||
    ...
    switch (binding.kind) {
      ...
      case "queue_producer":
        if (
          typeof binding.queueId !== "string" ||
          typeof binding.queueLifecycleGeneration !== "number" ||
          !Number.isSafeInteger(binding.queueLifecycleGeneration)
        )
          invalid();
        break;
      case "kv_namespace":
      ...
      case "ai_search_instance":
        if (
          ...
          !Number.isSafeInteger(binding.resourceSpecGeneration) ||
          !record(binding.permissions) ||
          typeof binding.permissions.read !== "boolean" ||
          typeof binding.permissions.write !== "boolean" ||
          (binding.namespacePrefix !== undefined &&
            typeof binding.namespacePrefix !== "string") ||
          (binding.namespaceNameKey !== undefined &&
            typeof binding.namespaceNameKey !== "string")
        )
          invalid();
        break;
      default:
  ...
  for (const service of value.services as unknown[]) {
    if (
      ...
      (service.props !== undefined &&
        (!record(service.props) ||
          !jsonValue(service.props, 0) ||
          new TextEncoder().encode(JSON.stringify(service.props)).byteLength >
            64 * 1024))
    )
      invalid();
  }
