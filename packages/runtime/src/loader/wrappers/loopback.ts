...
import type { Environment, EnvironmentWrapper, TenantConstructor } from "./runtime.js";
...
export function createLoopbackEntrypoint(
  tenant: Environment, wrapEnv: EnvironmentWrapper,
  wrap: (target: unknown, environment: EnvironmentWrapper, name?: string) => TenantConstructor,
  ...
  for (const [name, target] of Object.entries(tenant)) {
    if (!alreadyWrapped.includes(name) && typeof target === "function"
  ...
  return class extends WorkerEntrypoint {
    constructor(ctx: ExecutionContext, env: Environment) {
      ...
      if (options === null || typeof options !== "object") throw new Error("invalid loopback entrypoint");
      const name: unknown = Reflect.get(options, "name");
      const target = typeof name === "string" ? constructors.get(name) : undefined;
      ...
      if (!(instance instanceof WorkerEntrypoint)) throw new Error("invalid loopback entrypoint");
      return instance;
    }
  };
}

function serviceExport(source: object, native: Callable, name: string): Callable {
  ...
  const scoped = (options: unknown): unknown => {
    ...
    const scopedOptions = new Proxy({}, {
      get(_target, property) {
        return property === "props" ? { name, props: props === undefined ? {} : props }
  ...
  return new Proxy(native, {
    apply(_target, _receiver, args) { return scoped(args[0]); },
    get(_target, property) {
      if (!unscoped) {
...
export function tenantExports(source: object): object {
  ...
  for (const property of Reflect.ownKeys(source)) {
    ...
    values.set(property, loopbackNames.has(property) && callable(native) && !ownedExports.has(native)
      ? serviceExport(source, native, property) : native);
    enumerable.set(
      property,
  ...
  const exposed = (property: PropertyKey): unknown => {
    ...
    const bound = new Proxy(value, {
      apply(target, _receiver, args) {
        ...
        if (!loopbackNames.has(property) && result !== null
