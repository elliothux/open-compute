...
interface AlarmRow extends AlarmProjection {
  id: 1;
  inFlight: boolean;
  lastErrorCode: string | null;
  updatedAtMs: number;
}
interface AlarmTransaction {
  mutated: boolean;
  rolledBack: boolean;
}
...
type AlarmSqlRow = Record<string, SqlStorageValue> & {
  row_token: string;
...
class TenantSqlStorage implements SqlStorage {
  ...
  setMaxPageCountForTest(count: number) {
    this.#raw.setMaxPageCountForTest(count);
  }

  get databaseSize() {
    return this.#raw.databaseSize;
  }
}
...
function assertTenantSql(query: string) {
  if (INTERNAL_SQL.test(query)) {
    throw new Error(
      "SQLITE_AUTH: internal Durable Object storage is not accessible",
    );
  }
}
...
function exactDelete(storage: DurableObjectStorage, rowToken: string) {
  ensureTable(storage);
  storage.sql.exec(
    `DELETE FROM ${TABLE} WHERE id = 1 AND row_token = ?`,
    rowToken,
  );
}
...
function setAlarm(
  storage: DurableObjectStorage,
  index: AlarmIndexCapability,
  transaction: AlarmTransaction | null,
  scope: StorageScope,
  value: unknown,
) {
...
function deleteAlarm(
  ...
  if (scope.syncDepth > 0) {
    throw new TypeError(
      "deleteAlarm() is not supported inside transactionSync()",
    );
  }
  return (async () => {
    const row = readRow(storage);
...
async function deleteAllStorage(
  ...
  for (const object of objects) {
    ...
    storage.sql.exec(
      `DROP ${type.toUpperCase()} IF EXISTS ${quoteSqlIdentifier(name)}`,
    );
  }
}

async function flushTransaction(
  index: AlarmIndexCapability,
  initial: AlarmRow | null,
  final: AlarmRow | null,
) {
  try {
    if (final) {
...
function wrapStorage<T extends DurableObjectStorage | DurableObjectTransaction>(
  storage: T,
  index: AlarmIndexCapability,
  transaction: AlarmTransaction | null,
  rootStorage: DurableObjectStorage,
  gate: DoOutputGate,
  scope: StorageScope,
): T {
  return new Proxy(storage, {
    get(target, property) {
      ...
      if (property === "setAlarm") {
        return (value: unknown, _options?: unknown) =>
          setAlarm(alarmStorage, index, transaction, scope, value);
      }
      if (property === "getAlarm") {
        return (_options?: unknown) =>
          getAlarm(alarmStorage, index, transaction);
      }
      if (property === "deleteAlarm") {
        return (_options?: unknown) =>
          deleteAlarm(alarmStorage, index, transaction, scope);
      }
      if (property === "transactionSync") {
        if (!("transactionSync" in target))
          throw new TypeError("nested transactionSync() is unsupported");
      ...
      if (property === "transaction") {
        if (!("transaction" in target))
          throw new TypeError("nested transaction() is unsupported");
        return async (
          ...
          try {
            ...
            if (
              committed.value?.mutated &&
              !sameProjection(initial, committed.value.final)
            ) {
              await flushTransaction(index, initial, committed.value.final);
            }
            return result;
          } catch (error) {
            if (!txnCommitted && attemptStarted)
              await gate.exitTransaction("failed");
            throw error;
          }
        };
      }
      ...
      if (property === "deleteAll") {
        if (!("transaction" in target))
          throw new TypeError(
            "deleteAll() inside a transaction is unsupported",
          );
        return async (...args: unknown[]) => {
          if (args.length > 1)
            throw new TypeError(
              "deleteAll() accepts at most one options argument",
            );
          ...
          try {
            ...
            if (row) {
              try {
                ...
                /* activation repair owns convergence */
              }
            }
            throw error;
          }
...
export function prepareDurableObjectContext(
...
export function activateDurableObjectAlarm(
...
export async function dispatchDurableObjectAlarm(
  instance: unknown,
  handler: unknown,
  prepared: PreparedAlarm,
  payload: unknown,
  ...
  if (
    ...
    typeof payload.rowToken !== "string" ||
    !ROW_TOKEN.test(payload.rowToken) ||
  ...
  try {
    ...
    if (!current || current.rowToken !== payload.rowToken)
    ...
    try {
      ...
      /* dispatcher response repairs it */
    }
    return {
      outcome: "retry",
      scheduledTimeMs,
