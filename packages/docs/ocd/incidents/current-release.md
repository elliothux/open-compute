...
Allowed mutation: after operator confirmation, stop the service, use a verified binary of the **same** release, and restore an authenticated snapshot into an explicitly new directory using [fresh-host restore](/ocd/incidents/fresh-host). Source release, config policy, master key, object backend kind, authority fingerprint, and full schema identity must match first. Do not overwrite, downgrade, self-repair, or empty the existing directory. Building a release, downloading a runtime, and replacing the binary still need separate approval.
