...
Trigger: original data-dir is gone, or a full disaster-recovery drill. Blast radius is the entire platform; R2 sees the selected object authority's current state. Local mode requires a complete independently protected object root, including `format.json`, restored at a configured path disjoint from the empty target data-dir. Partial roots and Local↔S3 migration are not supported.
