...
`ocd` 必须 ready。namespace 创建需要已有的 Worker id 和 class 名：先部署带 class 的 Worker，再创建 namespace，再绑回去重新 `oc run`。
...
本机平台控制面，不是 Cloudflare REST / `client.v4`。body 是 camelCase：`workerId`、`className`。
...
把 default export 改成 `env.COUNTER.idFromName("global")` 再 `get` / `fetch`。CLI 是 `oc`，不是 Wrangler。下一步：[概念](/durable-objects/concepts/)。
