...
This page covers running an **already issued** OS/CPU-matching `platformd` file as a long-running service: container, systemd, and launchd. It does not cover building that file from source.

Shared contract: one `platformd`, one absolute-path config, one writable executable data-dir, and external S3. Never embed credentials in images, units, plists, or release archives; use env/file refs from config. Restart on process exit or `/health/live` failure, **never** on `/health/ready` 503.
...
| Role | Path |
| --- | --- |
| Binary | `/opt/open-compute/platformd` |
...
- `Type=simple`, `ExecStart=/opt/open-compute/platformd --config /etc/open-compute/config.toml run`
- `KillMode=control-group`, `KillSignal=SIGTERM`, `TimeoutStopSec=30`. platformd owns the workerd child; kill the whole group.
...
Plist: `examples/launchd/io.opencompute.platformd.plist`.

- `Label`: `io.opencompute.platformd`
- Arguments: `/opt/open-compute/platformd --config /usr/local/etc/open-compute/config.toml run`
...
- Logs: `/usr/local/var/log/open-compute/platformd.out.log` and `.err.log`
...
- The build context contains exactly one native Linux release file named `platformd`; CPU architecture must match.
...
- Mount operator config read-only at `/etc/open-compute/config.toml`. Generate it with `platformd config init --data-dir /var/lib/open-compute`, then set S3 and listeners.
