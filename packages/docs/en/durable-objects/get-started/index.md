...
Local platform control plane, not Cloudflare REST / `client.v4`. The body is camelCase: `workerId`, `className`.
...
Change the default export to `env.COUNTER.idFromName("global")` then `get` / `fetch`. The CLI is `oc`, not Wrangler. Next: [Concepts](/en/durable-objects/concepts/).
