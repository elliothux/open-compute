...
Follow [Install and first start](/en/ocd/get-started) until `ocd` is running. This page assumes `GET /health/ready` already returns 200. `oc run` does not start another workerd; it activates the Worker on the already-running local platform.
...
Next: [Directory](/en/directory). To run `ocd` as a service, see [Operate](/en/ocd/).
