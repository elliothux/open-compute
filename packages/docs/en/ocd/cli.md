...
Print the versioned product and release contract. `--json` emits `schema_version`, `release`, `runtime`, `products`, `limits`. How to read it: [Compatibility](/en/platform/compatibility).
...
Default is read-only. `--full` authorizes an S3 canary and a temporary workerd compile/start/stop. `--json` emits a versioned report. See [Health checks](/en/ocd/health).
...
All require `--config`; all accept `--json`. Procedures: [Backup and retention](/en/ocd/backup) and the [incident handbook](/en/ocd/incidents/).
...
`support-bundle`: `--output` must be an absolute path that does not exist and is not a symlink. `scheduler recover-corrupt` is only for an alarm-only directory whose control plane is verifiable and that has no Queue/Cron/Workflow authority. Full stop conditions: [incident handbook](/en/ocd/incidents/).

`ocd worker bundle` is an offline developer tool: versioned build JSON on stdin, canonical bundle on stdout. It is not part of installing the daemon. Worker programming: [Get started](/en/get-started) and [Compatibility](/en/platform/compatibility).
