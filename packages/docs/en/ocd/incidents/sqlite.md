...
Do not edit SQLite, WAL, SHM, or migration tables directly. The allowed mutation is a [fresh-host restore](/en/ocd/incidents/fresh-host) from the newest verified full-platform snapshot. Only an alarm-only directory whose control plane is verifiable and that has no Queue, Cron activation, Workflow history, operation, or version may use `scheduler recover-corrupt --backup-name scheduler-corrupt-20260826` to rebuild the projection. A Workflow catalog also blocks rebuild because the corrupt file may still hold purge receipts; see [Scheduler recovery](/en/ocd/incidents/scheduler).
