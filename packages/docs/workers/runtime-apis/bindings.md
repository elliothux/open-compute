...
KV / R2 / D1 / DO / Queue / Workflow / Assets / Images 的成员签名见各产品文档。配置语法见 [configuration · bindings](/workers/configuration/bindings)。
...
| Service Bindings | 跨地域 placement / 全球 service discovery | 仅限本平台；默认/具名 fetch 与 RPC；目标准入、deployment pin、capability 生命周期与恢复均在本节点，失败则 fail closed |
