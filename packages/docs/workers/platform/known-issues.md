...
| 项目文件 | wrangler.jsonc（可含注释） | `open-compute.json`；未知字段拒绝；无 `compatibilityDate`、无完整 Wrangler 产品键、无 jsonc 注释 |
| 控制面 | Cloudflare 控制面 | 本节点 `ocd` HTTP API（`/v1/account`、`/v1/accounts/.../workers`） |
| workers.dev | 是 | 不提供 |
