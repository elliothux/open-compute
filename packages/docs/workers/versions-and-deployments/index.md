...
One deploy: create or reuse a Worker → encode an immutable bundle → validate the runtime → activate (promote). Authority is local SQLite and one supervised runtime generation. `oc deploy` is this path locally; `oc deploy` is remote HTTPS.
...
A failed validation does not change the current active deployment. Promotion / rollback change the active pointer; they do not mutate a ready deployment's bytes. Preview is the local URL printed by `oc deploy`.
