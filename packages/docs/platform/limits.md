...
数字上限来自**正在运行的**二进制：`ocd capabilities --json` 的 `limits`。那是配置里冻结的产品数值上限，**不含密钥**。
...
固定版本的开源 `workerd` 独立进程不执行 Cloudflare 托管环境的 request-scoped CPU、subrequest 或 simultaneous-connection 配额。`LimitEnforcer` 的 subrequest 记账是空操作，`getLimitsExceeded()` 始终报告未超限。不要从其它 `limits` 字段推断这些配额已生效。
