...
| 主题                                                   | 行为                                                      | 文档                                                   |
| ------------------------------------------------------ | --------------------------------------------------------- | ------------------------------------------------------ |
| 出站 `fetch` / sockets / `node:net`                    | 共用开源 workerd 的 `Network(allow=["public"])`           | [TCP sockets](/zh/workers/runtime-apis/tcp-sockets)    |
...
| Dynamic Worker 显式 `limits`                           | 原生拒绝，包括空对象；默认 CPU/内存/子请求预算执行属于 W2 | [Bindings](/zh/workers/runtime-apis/bindings)          |
...
| Cron                                                   | UTC 五字段，以及文档所述本机扩展                          | [Cron 触发器](/zh/workers/configuration/cron-triggers) |
...
| 主题                                    | 行为                                                  | 文档                                    |
| --------------------------------------- | ----------------------------------------------------- | --------------------------------------- |
| Durable Objects 位置                    | 本机单个 workerd 进程                                 | [Durable Objects](/zh/durable-objects/) |
| location hint / jurisdiction / 全球迁移 | 不产生地理调度效果                                    |                                         |
| Queues 存储                             | 本机 `scheduler.sqlite`；投递语义为 at-least-once     | [Queues](/zh/queues/)                   |
| Queues 全局 FIFO                        | 不提供                                                |                                         |
| 无法识别的 native dispatch              | 可能保留消息 lease；后续投递可能使用同一 attempt 编号 |                                         |
| Workflows                               | 步骤状态位于本机 SQLite                               | [Workflows](/zh/workflows/)             |
...
| 主题                                                     | 行为                                                     | 文档                                                                             |
| -------------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------- |
| Cache                                                    | Workers Cache 与 Cache API 仅在本机                      | [Workers Cache](/zh/workers/cache/)、[Cache API](/zh/workers/runtime-apis/cache) |
...
| 运行中的精确值                                           | `ocd capabilities --json`，见[限制](/zh/platform/limits) |                                                                                  |
| Images                                                   | 对本机请求体中的图像做变换，不是 Cloudflare 托管 Images  | [Images](/zh/images/)                                                            |
