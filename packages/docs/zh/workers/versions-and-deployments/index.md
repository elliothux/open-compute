...
一次部署：创建或复用 Worker → 编码不可变 bundle → 校验 runtime → 激活（promote）。部署状态位于本机 SQLite；`ocd` 监督当前 workerd 进程。`oc deploy` 使用本地路径；`oc deploy` 使用远端 HTTPS。
...
校验失败不改变当前 active。promotion / rollback 只切换 active 指针，不修改已 ready 的部署内容。预览即为 `oc deploy` 打印的本机 URL。
