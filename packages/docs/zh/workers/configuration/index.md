...
`wrangler@4.127.1/config-schema.json` 是唯一项目语法 authority。`ocd wrangler` 不解析项目；离线 `oc build` / `oc types` 工具直接调用 Wrangler 的 config/environment resolver，不维护第二套 grammar。
...
框架 adapter 保留用户的 `wrangler.jsonc`，并生成标准 `.wrangler/deploy/config.json` redirect，指向生成的 Wrangler 配置。项目内 Wrangler 拥有部署；`oc build` / `oc types` 只保留离线 build 与类型生成职责。
