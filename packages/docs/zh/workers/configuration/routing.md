...
`oc deploy` 在已运行的平台上激活部署，然后打印该 Worker 的默认 `platform_path` URL（平台 origin + path prefix）。默认 origin 是 `http://127.0.0.1:8787`。
...
| 预览 URL | `*.workers.dev` / Cloudflare preview | `oc deploy` 打印的本机 URL |
