...
创建 database，在 `open-compute.json` 中绑定，再用 `oc` 运行 Worker。`oc run` 不会再起一个 workerd。若平台尚未就绪，见 [ocd 上手](/zh/ocd/get-started)。
...
CLI 为 `oc`，不是 Wrangler。下一步：[概念](/zh/d1/concepts/)、[指南](/zh/d1/guides/)。
