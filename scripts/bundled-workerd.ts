...
export function compressWorkerd(binary: Uint8Array): Buffer {
  if (process.versions.bun !== "1.3.14") throw new Error("workerd archives require Bun 1.3.14");
  if (binary.length > maxBinary) throw new Error("bundled workerd exceeds the size bound");
...
async function directory(path: string): Promise<void> {
  ...
  if (!(await lstat(path)).isDirectory()) throw new Error("workerd cache path must be a physical directory");
}

/** Verify the checked-in binary and materialize only its exact pinned compressed bytes. */
export async function bundledWorkerdArchive(repository: string, pin: Pin): Promise<string> {
  const source = join(repository, "share", "workerd", pin.target);
  for (const path of [join(repository, "share"), join(repository, "share/workerd"), source]) {
    if (!(await lstat(path)).isDirectory()) throw new Error("bundled workerd path must be a physical directory");
  }
  const binaryPath = join(source, "workerd");
  const metadata = await lstat(binaryPath);
  if (!metadata.isFile() || metadata.size > maxBinary || (metadata.mode & 0o111) === 0) {
    throw new Error("bundled workerd must be a bounded executable regular file");
  }
  const binary = await readFile(binaryPath);
  if (digest(binary) !== pin.binarySha256) {
    throw new Error("bundled workerd SHA-256 mismatch; hydrate Git LFS files and verify the formal pin");
  }
  let cache = repository;
  for (const part of [".temp", "workerd-build", pin.target, pin.archiveSha256]) {
  ...
  try { await verify(); return destination; }
  catch (error) {
  ...
  try {
    try { await file.writeFile(archive); await file.chmod(0o444); await file.sync(); }
