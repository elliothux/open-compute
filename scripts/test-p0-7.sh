#!/bin/sh
# Run the stock-workerd P0.7 Durable Objects matrix in three fresh processes, then P0.6-P0.2.
set -eu
...
if [ -z "$workerd" ] || [ ! -f "$workerd" ]; then
  echo "OPEN_COMPUTE_TEST_WORKERD is missing; the P0.7 Gate refuses to skip" >&2
  exit 1
fi
