#!/bin/sh
# Run the stock-workerd P0.4 KV matrix in three fresh processes, then P0.3/P0.2.
set -eu
...
if [ -z "$workerd" ] || [ ! -f "$workerd" ]; then
  echo "OPEN_COMPUTE_TEST_WORKERD is missing; the P0.4 Gate refuses to skip" >&2
  exit 1
fi
...
round=1
while [ "$round" -le 3 ]; do
  printf 'P0.4 fresh-process round %s/3\n' "$round"
  cargo test -p open-compute-service --features test-support \
    --test p0_4_kv_gate -- --test-threads=1 --nocapture
  round=$((round + 1))
done

./scripts/test-p0-3.sh
